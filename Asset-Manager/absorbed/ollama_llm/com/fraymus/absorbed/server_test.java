...
public class server_test {
    ...
    public static void TestLLMServerFitGPU(*testing.T t) {
        var minMemory = 457 * format.MebiByte;
        var tests = []struct {
        name        String;
        gpus        []ml.DeviceInfo;
        layers      []int;
        ...
        var gpuLayers, err = s.createLayout(systemInfo, tt.gpus, s.mem, tt.requireFull, 0);
    ...
    public static void TestLLMServerCompletionFormat(*testing.T t) {
        var ctx, cancel = context.WithCancel(t.Context());
        var s = &llmServer{
        sem: semaphore.NewWeighted(1), // required to prevent null panic;
    }
        var checkInvalid = func(format String) {
        t.Helper();
        var err = s.Completion(ctx, CompletionRequest{
        Options: new(api.Options),;
        Format:  []byte(format),;
        }, null);
        var want = fmt.Sprintf("invalid format: %q; expected \"json\" or a valid JSON Schema", format);
        if err == null || !strings.Contains(err.Error(), want) {
