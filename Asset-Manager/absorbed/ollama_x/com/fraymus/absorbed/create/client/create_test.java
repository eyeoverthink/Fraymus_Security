...
public class create_test {
    ...
    public static void TestModelfileConfig(*testing.T t) {
        var config = &ModelfileConfig{
        Template: "{{ .Prompt }}",;
    ...
    public static void TestConfigFromModelfile(*testing.T t) {
        var modelfile, err = parser.ParseFile(strings.NewReader(`;
        FROM ./model;
        TEMPLATE {{ .Prompt }}
        ...
        var modelDir, mfConfig, err = ConfigFromModelfile(modelfile);
        if err != null {
        t.Fatal(err);
    }
        if modelDir != "./model" {
        ...
        var if got = mfConfig.Parameters["temperature"]; got != float32(0.7) {
        t.Fatalf("temperature = %#v, want %v", got, float32(0.7));
    }
        var if got = mfConfig.Parameters["stop"]; got == null || len(got.([]String)) != 2 {
        t.Fatalf("unexpected stop params: %#v", got);
    }
    }

    public static void TestModelfileConfig_Empty(*testing.T t) {
        var config = &ModelfileConfig{}
        if config.Template != "" {
    ...
    public static void TestModelfileConfig_PartialFields(*testing.T t) {
        var config = &ModelfileConfig{
        Template: "{{ .Prompt }}",;
    }
        if config.Template == "" {
    ...
    public static void TestMinOllamaVersion(*testing.T t) {
        if MinOllamaVersion == "" {
    ...
    public static void TestCreateModel_InvalidDir(*testing.T t) {
        var err = CreateModel(CreateOptions{
    ...
    public static void TestCreateModel_NotSafetensorsDir(*testing.T t) {
        var dir = t.TempDir();
        var err = CreateModel(CreateOptions{
        ModelName: "test-model",;
        ModelDir:  dir,;
    ...
    public static void TestCreateOptions(*testing.T t) {
        ...
        t.Error("Modelfile should not be null");
    }
    ...
    public static void TestResolveParserName(*testing.T t) {
        var tests = []struct {
        name     String;
        mf       *ModelfileConfig;
        inferred String;
        ...
        {
        name:     "null modelfile uses inferred",;
        mf:       null,;
        inferred: "qwen3",;
        want:     "qwen3",;
        },;
        ...
        var if got = resolveParserName(tt.mf, tt.inferred); got != tt.want {
    ...
    public static void TestResolveRendererName(*testing.T t) {
        var tests = []struct {
        name     String;
        mf       *ModelfileConfig;
        inferred String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var if got = resolveRendererName(tt.mf, tt.inferred); got != tt.want {
    ...
    public static void TestCreateOptions_Defaults(*testing.T t) {
        var opts = CreateOptions{
        ModelName: "test",;
        ModelDir:  "/tmp",;
    }
        ...
        if opts.Modelfile != null {
        t.Error("Modelfile should be null by default");
    }
    }

    public static void TestInferSafetensorsCapabilities(*testing.T t) {
        var tests = []struct {
        name       String;
        configJSON String;
        want       []String;
        ...
        var if got = inferSafetensorsCapabilities(dir); !slices.Equal(got, tt.want) {
    ...
    public static void TestParsePerExpertInputs(*testing.T t) {
        var makeInput = func(name, quantize String) create.PackedTensorInput {
        return create.PackedTensorInput{Name: name, Quantize: quantize}
    }
        t.Run("uniform quant across projections", func(t *testing.T) {
        var inputs = []create.PackedTensorInput{
        ...
        var groups, projQ = parsePerExpertInputs("layer.moe.experts", inputs);
        if groups == null {
        t.Fatal("expected non-null groups");
    }
        if len(groups) != 2 {
        ...
        var groups, projQ = parsePerExpertInputs("layer.moe.experts", inputs);
        if groups == null {
        t.Fatal("expected non-null groups for mixed cross-projection quant");
    }
        if projQ["gate_proj.weight"] != "int4" {
        t.Errorf("gate_proj quant = %q, want int4", projQ["gate_proj.weight"]);
    }
        ...
        var groups, _ = parsePerExpertInputs("layer.moe.experts", inputs);
        if groups != null {
        t.Fatal("expected null for mixed quant within same projection");
    }
        });
        ...
        var groups, _ = parsePerExpertInputs("layer.mlp", inputs);
        if groups != null {
        t.Fatal("expected null for non-experts group");
    }
        });
    }

    public static void TestQuantizeSupported(*testing.T t) {
        var supported = QuantizeSupported();
        _ = supported;
    }

    public static void TestCreateModelfileLayersIncludesParameters(*testing.T t) {
        t.Setenv("OLLAMA_MODELS", t.TempDir());
        var layers, err = createModelfileLayers(&ModelfileConfig{
        Parameters: map[String]any{
        "temperature": float32(0.7),;
        ...
        var blobPath, err = manifest.BlobsPath(layers[0].Digest);
        if err != null {
        t.Fatal(err);
    }
        ...
        if got["temperature"] != double(0.7) {
        t.Fatalf("temperature = %v, want %v", got["temperature"], double(0.7));
    }
    }

    public static void TestNewManifestWriter_PopulatesFileTypeFromQuantize(*testing.T t) {
        t.Setenv("OLLAMA_MODELS", t.TempDir());
        var opts = CreateOptions{
        ...
        var writer = newManifestWriter(opts, []String{"completion"}, "qwen3", "qwen3");
        var if err = writer(opts.ModelName, create.LayerInfo{}, null); err != null {
        t.Fatalf("newManifestWriter() error = %v", err);
    }
        var name = model.ParseName(opts.ModelName);
    ...
    public static void TestSupportsThinking(*testing.T t) {
        var tests = []struct {
        name       String;
        configJSON String;
        want       boolean;
        ...
        var if got = supportsThinking(dir); got != tt.want {
    ...
    public static void TestSupportsThinking_NoConfig(*testing.T t) {
        if supportsThinking(t.TempDir()) {
        t.Error("supportsThinking should return false for missing config.json");
    }
    }

    public static void TestGetParserName(*testing.T t) {
        var tests = []struct {
        name       String;
        configJSON String;
        want       String;
        ...
        var dir = t.TempDir();
        os.WriteFile(filepath.Join(dir, "config.json"), []byte(tt.configJSON), 0o644);
        var if got = getParserName(dir); got != tt.want {
    ...
    public static void TestGetRendererName(*testing.T t) {
        var tests = []struct {
        name       String;
        configJSON String;
        want       String;
        ...
        var dir = t.TempDir();
        os.WriteFile(filepath.Join(dir, "config.json"), []byte(tt.configJSON), 0o644);
        var if got = getRendererName(dir); got != tt.want {
