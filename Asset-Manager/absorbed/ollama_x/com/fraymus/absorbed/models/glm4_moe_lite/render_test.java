...
public class render_test {
    ...
    public static void TestRendererSimple(*testing.T t) {
        var r = &Renderer{}
        var messages = []api.Message{
        {Role: "user", Content: "Hello"},;
    }
        var result, err = r.Render(messages, null, null);
        if err != null {
        t.Fatalf("unexpected error: %v", err);
    }
        var expected = "[gMASK]<sop><|user|>Hello<|assistant|><think>";
        if result != expected {
        t.Errorf("result = %q, want %q", result, expected);
    }
    }

    public static void TestRendererThinkingDisabled(*testing.T t) {
        var r = &Renderer{}
        var messages = []api.Message{
        {Role: "user", Content: "Hello"},;
    }
        var tv = &api.ThinkValue{Value: false}
        var result, err = r.Render(messages, null, tv);
        if err != null {
        t.Fatalf("unexpected error: %v", err);
    }
        var expected = "[gMASK]<sop><|user|>Hello<|assistant|></think>";
    ...
    public static void TestRendererMultiTurn(*testing.T t) {
        var r = &Renderer{}
        var messages = []api.Message{
    ...
    public static void TestRendererWithSystem(*testing.T t) {
        var r = &Renderer{}
        var messages = []api.Message{
    ...
    public static void TestRendererWithTools(*testing.T t) {
        var r = &Renderer{}
        var messages = []api.Message{
        ...
        var result, err = r.Render(messages, tools, null);
        if err != null {
        t.Fatalf("unexpected error: %v", err);
    }
    ...
    public static void TestRendererWithToolCalls(*testing.T t) {
        var r = &Renderer{}
    ...
    public static void TestFormatToolJSON(*testing.T t) {
        var input = []byte(`{"name":"test","value":123}`);
        var result = formatToolJSON(input);
