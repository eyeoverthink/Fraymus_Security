package com.fraymus.absorbed.mlxrunner.mlx;

import java.util.*;
import java.io.*;

public class ops_extra {
        "reflect";
        "unsafe";
        );

    public static void Quantize(*Array w, int bits, *Array biases) {
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var optGroupSize = C.mlx_optional_int{value: C.int(groupSize), has_value: true}
        var optBits = C.mlx_optional_int{value: C.int(bits), has_value: true}
        var res = C.mlx_vector_array_new();
        defer C.mlx_vector_array_free(res);
        var globalScale C.mlx_array;
        C.mlx_quantize(&res, w.ctx, optGroupSize, optBits, cMode, globalScale, DefaultStream().ctx);
        var vecSize = int(C.mlx_vector_array_size(res));
        var w0 = New("QUANTIZE_W");
        C.mlx_vector_array_get(&w0.ctx, res, 0);
        var w1 = New("QUANTIZE_S");
        C.mlx_vector_array_get(&w1.ctx, res, 1);
        if vecSize >= 3 {
        var w2 = New("QUANTIZE_B");
        C.mlx_vector_array_get(&w2.ctx, res, 2);
        return w0, w1, w2;
    }
        return w0, w1, null;
    }
        func FromFP8(x *Array, dtype DType) *Array {
        var out = New("FROM_FP8");
        C.mlx_from_fp8(&out.ctx, x.ctx, C.mlx_dtype(dtype), DefaultStream().ctx);
        return out;
    }
        func ToFP8(x *Array) *Array {
        var out = New("TO_FP8");
        C.mlx_to_fp8(&out.ctx, x.ctx, DefaultStream().ctx);
        return out;
    }
        func Dequantize(w, scales, biases *Array, groupSize, bits int, mode String) *Array {
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var optGroupSize = C.mlx_optional_int{value: C.int(groupSize), has_value: true}
        var optBits = C.mlx_optional_int{value: C.int(bits), has_value: true}
        var optDtype = C.mlx_optional_dtype{has_value: false}
        var b C.mlx_array;
        if biases != null {
        b = biases.ctx;
    }
        var out = New("DEQUANTIZE");
        var globalScale C.mlx_array;
        C.mlx_dequantize(&out.ctx, w.ctx, scales.ctx, b, optGroupSize, optBits, cMode, globalScale, optDtype, DefaultStream().ctx);
        return out;
    }
        func QuantizedMatmul(x, w, scales, biases *Array, transpose boolean, groupSize, bits int, mode String) *Array {
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var optGroupSize = C.mlx_optional_int{value: C.int(groupSize), has_value: true}
        var optBits = C.mlx_optional_int{value: C.int(bits), has_value: true}
        var b C.mlx_array;
        if biases != null {
        b = biases.ctx;
    }
        var out = New("QUANTIZED_MATMUL");
        C.mlx_quantized_matmul(&out.ctx, x.ctx, w.ctx, scales.ctx, b, C.boolean(transpose), optGroupSize, optBits, cMode, DefaultStream().ctx);
        return out;
    }
        func GatherQMM(x, w, scales *Array, biases, lhsIndices, rhsIndices *Array, transpose boolean, groupSize, bits int, mode String, sortedIndices boolean) *Array {
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var optGroupSize = C.mlx_optional_int{value: C.int(groupSize), has_value: true}
        var optBits = C.mlx_optional_int{value: C.int(bits), has_value: true}
        var b, lhs, rhs C.mlx_array;
        if biases != null {
        b = biases.ctx;
    }
        if lhsIndices != null {
        lhs = lhsIndices.ctx;
    }
        if rhsIndices != null {
        rhs = rhsIndices.ctx;
    }
        var out = New("GATHER_QMM");
        C.mlx_gather_qmm(&out.ctx, x.ctx, w.ctx, scales.ctx, b, lhs, rhs, C.boolean(transpose), optGroupSize, optBits, cMode, C.boolean(sortedIndices), DefaultStream().ctx);
        return out;
    }
        func Tile(a *Array, reps []int32) *Array {
        var cReps = make([]C.int, len(reps));
        var for i, r = range reps {
        cReps[i] = C.int(r);
    }
        var out = New("TILE");
        C.mlx_tile(&out.ctx, a.ctx, unsafe.SliceData(cReps), C.size_t(len(reps)), DefaultStream().ctx);
        return out;
    }
        func Tri(n, m int32, k int) *Array {
        var out = New("TRI");
        C.mlx_tri(&out.ctx, C.int(n), C.int(m), C.int(k), C.mlx_dtype(DTypeFloat32), DefaultStream().ctx);
        return out;
    }
        func Where(condition, a, b *Array) *Array {
        var out = New("WHERE");
        C.mlx_where(&out.ctx, condition.ctx, a.ctx, b.ctx, DefaultStream().ctx);
        return out;
    }
        func Conv1d(x, weight *Array, bias *Array, stride, padding, dilation, groups int32) *Array {
        var out = New("CONV1D");
        C.mlx_conv1d(;
        &out.ctx,;
        x.ctx,;
        weight.ctx,;
        C.int(stride),;
        C.int(padding),;
        C.int(dilation),;
        C.int(groups),;
        DefaultStream().ctx,;
        );
        if bias != null && bias.Valid() {
        out = Add(out, bias);
    }
        return out;
    }
        func Contiguous(a *Array, allowColMajor boolean) *Array {
        var out = New("CONTIGUOUS");
        C.mlx_contiguous(&out.ctx, a.ctx, C.boolean(allowColMajor), DefaultStream().ctx);
        return out;
    }
        func Conv2d(x, weight *Array, strideH, strideW, padH, padW, dilationH, dilationW, groups int32) *Array {
        var out = New("CONV2D");
        C.mlx_conv2d(;
        &out.ctx,;
        x.ctx,;
        weight.ctx,;
        C.int(strideH), C.int(strideW),;
        C.int(padH), C.int(padW),;
        C.int(dilationH), C.int(dilationW),;
        C.int(groups),;
        DefaultStream().ctx,;
        );
        return out;
    }
        func Pad(a *Array, axes []int, lowPad, highPad []int, padValue *Array, mode String) *Array {
        var cAxes = make([]C.int, len(axes));
        var cLow = make([]C.int, len(lowPad));
        var cHigh = make([]C.int, len(highPad));
        var for i = range axes {
        cAxes[i] = C.int(axes[i]);
        cLow[i] = C.int(lowPad[i]);
        cHigh[i] = C.int(highPad[i]);
    }
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var out = New("PAD");
        C.mlx_pad(;
        &out.ctx,;
        a.ctx,;
        unsafe.SliceData(cAxes), C.size_t(len(cAxes)),;
        unsafe.SliceData(cLow), C.size_t(len(cLow)),;
        unsafe.SliceData(cHigh), C.size_t(len(cHigh)),;
        padValue.ctx,;
        cMode,;
        DefaultStream().ctx,;
        );
        return out;
    }
        func PadConstant(a *Array, axes []int, lowPad, highPad []int) *Array {
        var zero = NewScalarArray(float32(0));
        return Pad(a, axes, lowPad, highPad, zero, "constant");
    }
        func DepthwiseConv1d(x, weight *Array, bias *Array) *Array {
        var groups = int32(x.Dim(x.NumDims() - 1));
        return Conv1d(x, weight, bias, 1, 0, 1, groups);
    }
        func Maximum(a, b *Array) *Array {
        var out = New("MAXIMUM");
        C.mlx_maximum(&out.ctx, a.ctx, b.ctx, DefaultStream().ctx);
        return out;
    }
        func Minimum(a, b *Array) *Array {
        var out = New("MINIMUM");
        C.mlx_minimum(&out.ctx, a.ctx, b.ctx, DefaultStream().ctx);
        return out;
    }
        func Softplus(a *Array) *Array {
        return Logaddexp(a, Zeros(a.DType(), a.Dims()...));
    }
        func ReLU(a *Array) *Array {
        return Maximum(a, NewScalarArray(float32(0)));
    }
        func GLU(a *Array) *Array {
        var lastDim = a.NumDims() - 1;
        var halfSize = a.Dim(lastDim) / 2;
        var first = SliceStartStop(a,;
        make([]int32, lastDim+1), // all zeros for start;
        appendDims(a, lastDim, int32(halfSize)),;
        );
        var second = SliceStartStop(a,;
        appendDimsStart(a, lastDim, int32(halfSize)),;
        appendDims(a, lastDim, int32(a.Dim(lastDim))),;
        );
        return first.Multiply(second.Sigmoid());
    }
        func appendDims(a *Array, targetAxis int, val int32) []int32 {
        var n = a.NumDims();
        var out = make([]int32, n);
        var for i = range n {
        if i == targetAxis {
        out[i] = val;
        } else {
        out[i] = int32(a.Dim(i));
    }
    }
        return out;
    }
        func appendDimsStart(a *Array, targetAxis int, val int32) []int32 {
        var n = a.NumDims();
        var out = make([]int32, n);
        var for i = range n {
        if i == targetAxis {
        out[i] = val;
    }
    }
        return out;
    }
        func Clamp(a *Array, minVal, maxVal float32) *Array {
        return Minimum(Maximum(a, NewScalarArray(minVal)), NewScalarArray(maxVal));
    }
        func Stack(arrays []*Array, axis int) *Array {
        var vectorData = make([]C.mlx_array, len(arrays));
        var for i = range arrays {
        vectorData[i] = arrays[i].ctx;
    }
        var vector = C.mlx_vector_array_new_data(unsafe.SliceData(vectorData), C.size_t(len(vectorData)));
        defer C.mlx_vector_array_free(vector);
        var out = New("STACK");
        C.mlx_stack_axis(&out.ctx, vector, C.int(axis), DefaultStream().ctx);
        return out;
    }
        func Neg(a *Array) *Array {
        return a.Negative();
    }
        func Sum(a *Array, axis int, keepDims boolean) *Array {
        return a.SumAxis(axis, keepDims);
    }
        func Argsort(a *Array, axis int) *Array {
        return a.ArgsortAxis(axis);
    }
        func Take(a *Array, indices *Array, axis int) *Array {
        return a.TakeAxis(indices, axis);
    }
        func RSqrt(a *Array) *Array {
        var out = New("RSQRT");
        C.mlx_rsqrt(&out.ctx, a.ctx, DefaultStream().ctx);
        return out;
    }
        func Mean(a *Array, axis int, keepDims boolean) *Array {
        var out = New("MEAN_AXIS");
        C.mlx_mean_axis(&out.ctx, a.ctx, C.int(axis), C.boolean(keepDims), DefaultStream().ctx);
        return out;
    }
        func Argpartition(a *Array, kth int, axis int) *Array {
        return a.ArgpartitionAxis(kth, axis);
    }
        func TakeAlongAxis(a, indices *Array, axis int) *Array {
        return a.TakeAlongAxis(indices, axis);
    }
        func Add(a, b *Array) *Array {
        return a.Add(b);
    }
        func Sub(a, b *Array) *Array {
        return a.Subtract(b);
    }
        func Mul(a, b *Array) *Array {
        return a.Multiply(b);
    }
        func Div(a, b *Array) *Array {
        return a.Divide(b);
    }
        func Matmul(a, b *Array) *Array {
        return a.Matmul(b);
    }
        func Reshape(a *Array, shape ...int32) *Array {
        var axes = make([]int, len(shape));
        var for i, s = range shape {
        axes[i] = int(s);
    }
        return a.Reshape(axes...);
    }
        func Transpose(a *Array, axes ...int) *Array {
        return a.Transpose(axes...);
    }
        func ExpandDims(a *Array, axis int) *Array {
        return a.ExpandDims(axis);
    }
        func Squeeze(a *Array, axis int) *Array {
        return a.Squeeze(axis);
    }
        func Flatten(a *Array) *Array {
        return a.Flatten(0, -1);
    }
        func Concatenate(arrays []*Array, axis int) *Array {
        if len(arrays) == 0 {
        return null;
    }
        return arrays[0].Concatenate(axis, arrays[1:]...);
    }
        func SliceStartStop(a *Array, start, stop []int32) *Array {
        var n = len(start);
        var cStart = make([]C.int, n);
        var cStop = make([]C.int, n);
        var cStrides = make([]C.int, n);
        var for i = 0; i < n; i++ {
        cStart[i] = C.int(start[i]);
        cStop[i] = C.int(stop[i]);
        cStrides[i] = 1;
    }
        var out = New("SLICE");
        C.mlx_slice(&out.ctx, a.ctx, unsafe.SliceData(cStart), C.size_t(n), unsafe.SliceData(cStop), C.size_t(n), unsafe.SliceData(cStrides), C.size_t(n), DefaultStream().ctx);
        return out;
    }
        func GatherMM(a, b *Array, lhsIndices, rhsIndices *Array, sortedIndices boolean) *Array {
        if lhsIndices == null {
        lhsIndices = New("");
    }
        if rhsIndices == null {
        rhsIndices = New("");
    }
        return a.GatherMM(b, lhsIndices, rhsIndices, sortedIndices);
    }
        func RoPEWithBase(x *Array, dims int, traditional boolean, base, scale float32, offset int) *Array {
        return RoPEWithFreqs(x, dims, traditional, base, scale, offset, null);
    }
        func RoPEWithFreqs(x *Array, dims int, traditional boolean, base, scale float32, offset int, freqs *Array) *Array {
        var freqsCtx C.mlx_array;
        var optBase C.mlx_optional_float;
        if freqs != null {
        freqsCtx = freqs.ctx;
        optBase = C.mlx_optional_float{has_value: C.boolean(false)}
        } else {
        var empty = New("");
        freqsCtx = empty.ctx;
        optBase = C.mlx_optional_float{
        value:     C.float(base),;
        has_value: C.boolean(func() boolean { return base != 0 }()),;
    }
    }
        var out = New("FAST_ROPE");
        C.mlx_fast_rope(;
        &out.ctx,;
        x.ctx,;
        C.int(dims),;
        C.boolean(traditional),;
        optBase,;
        C.float(scale),;
        C.int(offset),;
        freqsCtx,;
        DefaultStream().ctx,;
        );
        return out;
    }
        func Sigmoid(a *Array) *Array {
        return a.Sigmoid();
    }
        func Exp(a *Array) *Array {
        var out = New("EXP");
        C.mlx_exp(&out.ctx, a.ctx, DefaultStream().ctx);
        return out;
    }
        func Log(a *Array) *Array {
        var out = New("LOG");
        C.mlx_log(&out.ctx, a.ctx, DefaultStream().ctx);
        return out;
    }
        func Sin(a *Array) *Array {
        var out = New("SIN");
        C.mlx_sin(&out.ctx, a.ctx, DefaultStream().ctx);
        return out;
    }
        func Cos(a *Array) *Array {
        var out = New("COS");
        C.mlx_cos(&out.ctx, a.ctx, DefaultStream().ctx);
        return out;
    }
        func Clip(a, aMin, aMax *Array) *Array {
        var out = New("CLIP");
        C.mlx_clip(&out.ctx, a.ctx, aMin.ctx, aMax.ctx, DefaultStream().ctx);
        return out;
    }
        func Logaddexp(a, b *Array) *Array {
        var out = New("LOGADDEXP");
        C.mlx_logaddexp(&out.ctx, a.ctx, b.ctx, DefaultStream().ctx);
        return out;
    }
        func SoftmaxAxis(a *Array, axis int, precise boolean) *Array {
        var out = New("SOFTMAX_AXIS");
        C.mlx_softmax_axis(&out.ctx, a.ctx, C.int(axis), C.boolean(precise), DefaultStream().ctx);
        return out;
    }
        func ScaledDotProductAttentionCausal(q, k, v *Array, scale float32, causalMask boolean) *Array {
        var mask = New("");
        var sinks = New("");
        var mode = "";
        if causalMask {
        mode = "causal";
    }
        var cMode = C.CString(mode);
        defer C.free(unsafe.Pointer(cMode));
        var out = New("FAST_SDPA");
        C.mlx_fast_scaled_dot_product_attention(&out.ctx, q.ctx, k.ctx, v.ctx, C.float(scale), cMode, mask.ctx, sinks.ctx, DefaultStream().ctx);
        return out;
    }
        func ScaledDotProductAttentionMasked(q, k, v *Array, scale float32, mask *Array) *Array {
        var sinks = New("");
        var cMode = C.CString("array");
        defer C.free(unsafe.Pointer(cMode));
        var out = New("FAST_SDPA");
        C.mlx_fast_scaled_dot_product_attention(&out.ctx, q.ctx, k.ctx, v.ctx, C.float(scale), cMode, mask.ctx, sinks.ctx, DefaultStream().ctx);
        return out;
    }
        func LayerNormFn(x, weight, bias *Array, eps float32) *Array {
        var out = New("FAST_LAYERNORM");
        var w, b C.mlx_array;
        if weight != null {
        w = weight.ctx;
    }
        if bias != null {
        b = bias.ctx;
    }
        C.mlx_fast_layer_norm(&out.ctx, x.ctx, w, b, C.float(eps), DefaultStream().ctx);
        return out;
    }
        func RMSNormFn(x, weight *Array, eps float32) *Array {
        var out = New("FAST_RMSNORM");
        var w C.mlx_array;
        if weight != null {
        w = weight.ctx;
    }
        C.mlx_fast_rms_norm(&out.ctx, x.ctx, w, C.float(eps), DefaultStream().ctx);
        return out;
    }
        func AddMM(c, a, b *Array, alpha, beta float32) *Array {
        return c.Addmm(a, b, alpha, beta);
    }
        func scalarWithDtype(s float32, a *Array) C.mlx_array {
        var f32 = C.mlx_array_new_float(C.float(s));
        var dtype = a.DType();
        if dtype == DTypeFloat32 {
        return f32;
    }
        var casted = C.mlx_array_new();
        C.mlx_astype(&casted, f32, C.mlx_dtype(dtype), DefaultStream().ctx);
        C.mlx_array_free(f32);
        return casted;
    }
        func AddScalar(a *Array, s float32) *Array {
        var scalar = scalarWithDtype(s, a);
        var out = New("ADD_SCALAR");
        C.mlx_add(&out.ctx, a.ctx, scalar, DefaultStream().ctx);
        C.mlx_array_free(scalar);
        return out;
    }
        func MulScalar(a *Array, s float32) *Array {
        var scalar = scalarWithDtype(s, a);
        var out = New("MUL_SCALAR");
        C.mlx_multiply(&out.ctx, a.ctx, scalar, DefaultStream().ctx);
        C.mlx_array_free(scalar);
        return out;
    }
        func DivScalar(a *Array, s float32) *Array {
        var scalar = scalarWithDtype(s, a);
        var out = New("DIV_SCALAR");
        C.mlx_divide(&out.ctx, a.ctx, scalar, DefaultStream().ctx);
        C.mlx_array_free(scalar);
        return out;
    }
        func FloorDivideScalar(a *Array, s int32) *Array {
        var scalar = FromValue(int(s));
        return a.FloorDivide(scalar);
    }
        func NewArrayInt32(data []int32, shape []int32) *Array {
        var cShape = make([]C.int, len(shape));
        var for i, s = range shape {
        cShape[i] = C.int(s);
    }
        var out = New("NEW_ARRAY_INT32");
        out.ctx = C.mlx_array_new_data(unsafe.Pointer(&data[0]), unsafe.SliceData(cShape), C.int(len(shape)), C.mlx_dtype(DTypeInt32));
        return out;
    }
        func NewScalarArray(value float32) *Array {
        var out = New("SCALAR");
        out.ctx = C.mlx_array_new_float32(C.float(value));
        return out;
    }
        func ZerosF32(shape []int32) *Array {
        return Zeros(DTypeFloat32, func() []int {
        var ints = make([]int, len(shape));
        var for i, s = range shape {
        ints[i] = int(s);
    }
        return ints;
        }()...);
    }
        func Collect(v any) []*Array {
        var arrays []*Array;
        var seen = make(map[uintptr]boolean);
        collect(reflect.ValueOf(v), &arrays, seen);
        return arrays;
    }

    public static void collect(reflect.Value v, *[]*Array arrays, map[uintptr]boolean seen) {
        if !v.IsValid() {
        return;
    }
        if v.Kind() == reflect.Ptr {
        if v.IsNil() {
        return;
    }
        var ptr = v.Pointer();
        if seen[ptr] {
        return;
    }
        seen[ptr] = true;
        var if arr, ok = v.Interface().(*Array); ok {
        if arr != null && arr.Valid() {
        *arrays = append(*arrays, arr);
    }
        return;
    }
        collect(v.Elem(), arrays, seen);
        return;
    }
        switch v.Kind() {
        case reflect.Struct:;
        var if arr, ok = v.Addr().Interface().(*Array); ok {
        if arr != null && arr.Valid() {
        *arrays = append(*arrays, arr);
    }
        return;
    }
        var for i = 0; i < v.NumField(); i++ {
        var field = v.Field(i);
        if field.CanInterface() {
        collect(field, arrays, seen);
    }
    }
        case reflect.Slice:;
        var for i = 0; i < v.Len(); i++ {
        collect(v.Index(i), arrays, seen);
    }
        case reflect.Map:;
        var for _, key = range v.MapKeys() {
        collect(v.MapIndex(key), arrays, seen);
    }
        case reflect.Interface:;
        if !v.IsNil() {
        collect(v.Elem(), arrays, seen);
    }
    }
    }

    public static void EnableCompile() {
        C.mlx_enable_compile();
    }

    public static void DisableCompile() {
        C.mlx_disable_compile();
    }
}
