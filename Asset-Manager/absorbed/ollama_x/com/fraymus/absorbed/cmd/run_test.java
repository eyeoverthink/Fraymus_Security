...
public class run_test {
    ...
    public static void TestIsLocalModel(*testing.T t) {
        var tests = []struct {
        name      String;
        modelName String;
        expected  boolean;
        ...
        var result = isLocalModel(tt.modelName);
    ...
    public static void TestIsLocalServer(*testing.T t) {
        var tests = []struct {
        name     String;
        host     String;
        expected boolean;
    ...
    public static void TestTruncateToolOutput(*testing.T t) {
        var localLimitOutput = make([]byte, 20000)   // > 4k tokens (16k chars);
        var defaultLimitOutput = make([]byte, 50000) // > 10k tokens (40k chars);
        var for i = range localLimitOutput {
        localLimitOutput[i] = 'a';
    }
        var for i = range defaultLimitOutput {
        ...
        t.Run(tt.name, func(t *testing.T) {
        t.Setenv("OLLAMA_HOST", tt.host);
        var result = truncateToolOutput(tt.output, tt.modelName);
        if tt.shouldTrim {
        var maxLen = tt.expectedLimit * charsPerToken;
