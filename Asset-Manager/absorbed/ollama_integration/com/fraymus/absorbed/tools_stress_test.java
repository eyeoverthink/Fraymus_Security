...
public class tools_stress_test {
        "context";
        "encoding/json";
        "fmt";
        "os";
    ...
    public static void TestAPIToolCallingStress(*testing.T t) {
        var initialTimeout = 120 * time.Second;
        var streamTimeout = 120 * time.Second;
        ...
        var tools = stressTestTools();
        var systemPrompt = stressTestSystemPrompt();
    ...
    public static String stressTestSystemPrompt() {
        ...
        var validStressTools = map[String]boolean{
    ...
    public static void testToolCall(*testing.T t, context.Context ctx, *api.Client client, String systemPrompt, []api.Tool tools, String userMessage, time.Duration streamTimeout) {
        t.Helper();
        ...
        var argsJSON, _ = json.Marshal(lastToolCall.Function.Arguments);
    ...
    public static void testToolCallMultiTurn(*testing.T t, context.Context ctx, *api.Client client, String systemPrompt, []api.Tool tools, time.Duration streamTimeout) {
        t.Helper();
        var req = api.ChatRequest{
        Model: model,;
        Messages: []api.Message{
    ...
    public static void checkNoLeakedTags(*testing.T t, String content) {
        t.Helper();
        var leakedTags = []String{
        "<|channel>", "<channel|>",;
        "<|tool_call>", "<tool_call|>",;
        "<|tool>", "<tool|>",;
        "<|turn>", "<turn|>",;
    }
        var for _, tag = range leakedTags {
    ...
    public static int contextLength(int defaultVal) {
        var if s = os.Getenv("OLLAMA_CONTEXT_LENGTH"); s != "" {
        var if n, err = strconv.Atoi(s); err == null {
        return n;
    }
    }
        return defaultVal;
