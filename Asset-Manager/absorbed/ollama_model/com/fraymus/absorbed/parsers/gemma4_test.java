...
public class gemma4_test {
    ...
    public static void TestGemma4Parser(*testing.T t) {
        var tests = []struct {
        name              String;
        input             String;
        ...
        var parser = &Gemma4Parser{hasThinkingSupport: true}
        parser.Init(null, tt.lastMessage, &api.ThinkValue{Value: tt.thinkingEnabled});
    ...
    public static void TestGemma4Parser_Streaming(*testing.T t) {
        var parser = &Gemma4Parser{hasThinkingSupport: true}
    ...
    public static void TestGemma4Parser_StreamingToolCall(*testing.T t) {
        var parser = &Gemma4Parser{hasThinkingSupport: false}
        ...
        var for i, chunk = range chunks {
        var done = i == len(chunks)-1;
        var content, _, toolCalls, err = parser.Add(chunk, done);
        if err != null {
        t.Fatalf("Add() error on chunk %d: %v", i, err);
    }
        finalContent.WriteString(content);
    ...
    public static void TestGemma4Parser_IgnoresExtraToolCallCloseTags(*testing.T t) {
        var tests = []struct {
        name            String;
        chunks          []String;
        expectedContent String;
        ...
        var if diff = cmp.Diff(tt.expectedContent, finalContent.String()); diff != "" {
        t.Errorf("content mismatch (-want +got):\n%s", diff);
    }
        var if diff = cmp.Diff(expectedToolCalls, finalToolCalls, argsComparer); diff != "" {
        t.Errorf("tool calls mismatch (-want +got):\n%s", diff);
    ...
    public static void TestGemma4Parser_IgnoresToolResponseBoundaryAfterToolCall(*testing.T t) {
        var tests = []struct {
        name            String;
        chunks          []String;
        expectedContent String;
    ...
    public static void TestGemma4Parser_StreamingSplitThinkingTag(*testing.T t) {
        var tests = []struct {
        name             String;
        chunks           []String;
        expectedContent  String;
    ...
    public static void TestGemma4ArgsToJSON(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        expected String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var result = gemma4ArgsToJSON(tt.input);
    ...
    public static void TestRepairGemma4MissingStringDelimiter(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        expected String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got = repairGemma4MissingStringDelimiter(tt.input);
        if got != tt.expected {
    ...
    public static void TestRepairGemma4MissingObjectClose(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        expected String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got = repairGemma4MissingObjectClose(tt.input);
        if got != tt.expected {
    ...
    public static void TestRepairGemma4SingleQuotedValues(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        expected String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got = repairGemma4SingleQuotedValues(tt.input);
        if got != tt.expected {
    ...
    public static void TestRepairGemma4RawTerminalStringValue(*testing.T t) {
        var numberProps = api.NewToolPropertiesMap();
        numberProps.Set("content", api.ToolProperty{Type: api.PropertyType{"number"}});
        var numberTool = api.Tool{
        Type: "function",;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got, ok = repairGemma4RawTerminalStringValue(tt.input, tt.toolName, tt.tools);
        if ok != tt.expectedOK {
    ...
    public static void TestGemma4RepairCandidates(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        toolName String;
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got = gemma4RepairCandidates(tt.input, tt.toolName, tt.tools);
        ...
        var for _, argName = range argNames {
    ...
    public static void TestGemma4Parser_HasToolSupport(*testing.T t) {
    ...
    public static void TestGemma4Parser_HasThinkingSupport(*testing.T t) {
        var parser = &Gemma4Parser{hasThinkingSupport: true}
        if !parser.HasThinkingSupport() {
        t.Error("Gemma4Parser with thinking support should report it");
    }
        var parser2 = &Gemma4Parser{hasThinkingSupport: false}
        if parser2.HasThinkingSupport() {
        t.Error("Gemma4Parser without thinking support should not report it");
    }
    }

    public static void TestParseGemma4ToolCall_InvalidRawQuotedEscape(*testing.T t) {
        var _, err = parseGemma4ToolCall(`call:open_file{path:"C:\users\bob\file.txt"}`, null);
        if err == null {
        t.Fatal("expected parseGemma4ToolCall to reject malformed raw-quoted JSON escapes");
    }
    }

    public static void TestParseGemma4ToolCall_QuotedScalarsStayStrings(*testing.T t) {
        var toolCall, err = parseGemma4ToolCall(`call:foo{n:<|"|>1<|"|>,b:<|"|>true<|"|>,z:<|"|>null<|"|>}`, null);
        if err != null {
        ...
        var if diff = cmp.Diff(want, toolCall, argsComparer); diff != "" {
        t.Fatalf("quoted scalar handling differed from the reference implementation (-want +got):\n%s", diff);
    }
    }

    public static void TestParseGemma4ToolCall_UnquotedScalarsKeepStructuredTypes(*testing.T t) {
        var toolCall, err = parseGemma4ToolCall(`call:foo{n:1,b:true,z:null}`, null);
        if err != null {
        t.Fatalf("parseGemma4ToolCall returned error: %v", err);
    }
    ...
    public static void TestParseGemma4ToolCall_ReferenceImplementationExample(*testing.T t) {
        var toolCall, err = parseGemma4ToolCall(`call:get_current_temperature{detail_level:0,location:<|"|>Paris, France<|"|>,unit:<|"|>celsius<|"|>}`, null);
        if err != null {
        t.Fatalf("parseGemma4ToolCall returned error: %v", err);
    }
        var want = api.ToolCall{
    ...
    public static void TestParseGemma4ToolCall_RepairsIssue15315Examples(*testing.T t) {
        ...
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var got, err = parseGemma4ToolCall(tt.content, tt.tools);
        if err != null {
        t.Fatalf("parseGemma4ToolCall returned error: %v", err);
    }
        var if diff = cmp.Diff(tt.want, got, argsComparer); diff != "" {
    ...
    public static void TestParseGemma4ToolCall_RepairsMultipleProperties(*testing.T t) {
        var tests = []struct {
        name    String;
        content String;
        tools   []api.Tool;
    ...
    public static void TestParseGemma4ToolCall_DoesNotRepairNonTerminalUnclosedGemmaString(*testing.T t) {
        var _, err = parseGemma4ToolCall(`call:example{first:<|"|>one,second:<|"|>two<|"|>}`, []api.Tool{
    ...
    public static void TestParseGemma4ToolCall_RawQuotedStructuralString(*testing.T t) {
        var got, err = parseGemma4ToolCall(`call:foo{q:"a,b:c"}`, null);
        if err != null {
        t.Fatalf("parseGemma4ToolCall returned error: %v", err);
    }
        var want = api.ToolCall{
        ...
        var if diff = cmp.Diff(want, got, argsComparer); diff != "" {
        t.Fatalf("tool call mismatch (-want +got):\n%s", diff);
    }
    }
}
