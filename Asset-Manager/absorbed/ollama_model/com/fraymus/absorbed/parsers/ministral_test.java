...
public class ministral_test {
    ...
    public static void TestMinistralParserStreaming(*testing.T t) {

    public static class step {
        public String input;
        public []ministralEvent wantEvents;
    }
        var cases = []struct {
        desc  String;
        tools []api.Tool;
        steps []step;
        think boolean // whether to enable thinking support;
        }{
        {
        desc: "simple content",;
        steps: []step{
        ...
        var parser = MinistralParser{}
        parser.hasThinkingSupport = tc.think;
        parser.Init(tc.tools, null, null);
    ...
    public static void TestMinistralParserAssignsSequentialToolCallIndices(*testing.T t) {
        var parser = &MinistralParser{}
        parser.Init([]api.Tool{
        {Function: api.ToolFunction{Name: "get_weather"}},;
        {Function: api.ToolFunction{Name: "get_time"}},;
        }, null, null);
        var content, thinking, calls, err = parser.Add(;
        ...
        var if diff = cmp.Diff(expected, calls, argsComparer); diff != "" {
        t.Fatalf("tool calls mismatch (-want +got):\n%s", diff);
    }
    }

    public static void TestMinistralParser_Errors(*testing.T t) {
        t.Run("unknown tool returns error", func(t *testing.T) {
        var p = &MinistralParser{}
        p.Init([]api.Tool{{Function: api.ToolFunction{Name: "known_tool"}}}, null, null);
        var _, _, _, err = p.Add(`[TOOL_CALLS]unknown_tool[ARGS]{"a": 1}`, true);
        if err == null {
        ...
        var p = &MinistralParser{}
        p.Init([]api.Tool{{Function: api.ToolFunction{Name: "test"}}}, null, null);
        var _, _, _, err = p.Add(`[TOOL_CALLS]test[ARGS]{invalid json}`, true);
        if err == null {
    ...
    public static void TestFindJSONEnd(*testing.T t) {
        var tests = []struct {
        name     String;
        input    String;
        expected int;
        ...
        var result = findJSONEnd(tt.input);
    ...
    public static void TestMinistralParser_HasToolSupport(*testing.T t) {
        var p = &MinistralParser{}
        if !p.HasToolSupport() {
    ...
    public static void TestMinistralParser_HasThinkingSupport(*testing.T t) {
        var p = &MinistralParser{hasThinkingSupport: false}
