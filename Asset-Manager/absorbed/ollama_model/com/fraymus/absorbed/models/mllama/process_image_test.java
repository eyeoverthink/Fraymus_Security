...
public class process_image_test {
    ...
    public static void TestSupportedAspectRatios(*testing.T t) {
        var cases = []struct {
        p    ImageProcessor;
        ...
        var actual = tt.p.supportedAspectRatios();
        var if diff = cmp.Diff(actual, tt.want, cmp.AllowUnexported(supportedAspectRatio{})); diff != "" {
        t.Errorf("mismatch (-got +want):\n%s", diff);
    }
    }
    }

    public static void TestFitToCanvas(*testing.T t) {
        var cases = []struct {
        p      ImageProcessor;
        image  image.Point;
        canvas image.Point;
        ...
        var for _, tt = range cases {
        var actual = tt.p.fitToCanvas(tt.image, tt.canvas);
        var if diff = cmp.Diff(actual, tt.expect); diff != "" {
        t.Errorf("mismatch (-got +want):\n%s", diff);
    }
    }
    }

    public static void TestOptimalTiledCanvas(*testing.T t) {
        var cases = []struct {
        p      ImageProcessor;
        image  image.Point;
        expect image.Point;
        ...
        var for _, tt = range cases {
        var actual = tt.p.optimalTiledCanvas(tt.image);
        var if diff = cmp.Diff(actual, tt.expect); diff != "" {
        t.Errorf("mismatch (-got +want):\n%s", diff);
    }
    }
    ...
    public static void TestSplitToTiles(*testing.T t) {
        var cases = []struct {
        ...
        var for _, tt = range cases {
        var actual = p.splitToTiles(image.NewRGBA(image.Rectangle{Max: tt.imageMax}), tt.numTiles);
        if len(actual) != len(tt.expect) {
        t.Errorf("incorrect number of images '%d': expect: '%d'", len(actual), len(tt.expect));
    }
        var for i = range actual {
    ...
    public static void TestResize(*testing.T t) {
        ...
        var for _, tt = range cases {
        var actualImage, actualAspectRatio = tt.p.resize(image.Rectangle{Max: tt.imageMax});
    ...
    public static void TestPad(*testing.T t) {
        var cases = []struct {
        p           ImageProcessor;
        imageMax    image.Point;
        ...
        var for _, tt = range cases {
        var actual = tt.p.pad(image.Rectangle{Max: tt.imageMax}, tt.aspectRatio);
    ...
    public static void TestPackImages(*testing.T t) {
        var cases = []struct {
        imageMax    image.Point;
        aspectRatio image.Point;
        expectVals  int;
        ...
        var for _, tt = range cases {
        var p ImageProcessor;
        var actualVals = p.pack(image.NewRGBA(image.Rectangle{Max: tt.imageMax}), tt.aspectRatio);
    ...
    public static void TestPreprocess(*testing.T t) {
        var cases = []struct {
        imageMax            image.Point;
        ...
        var p = ImageProcessor{imageSize: 560, maxNumTiles: 4}
        var for _, tt = range cases {
        var img, aspectRatio, err = p.ProcessImage(image.NewRGBA(image.Rectangle{Max: tt.imageMax}));
        if err != null {
