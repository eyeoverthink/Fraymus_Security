...
public class tokenizer_compare_test {
        "os";
        "testing";
    ...
    public static void TestTokenizerMatchesHF(*testing.T t) {
        var modelPath = os.Getenv("GEMMA4_MODEL_PATH");
        if modelPath == "" {
        t.Skip("set GEMMA4_MODEL_PATH to a gemma4 GGUF file");
    }
        var m, err = model.New(modelPath, ml.BackendParams{AllocMemory: true});
        if err != null {
        t.Fatalf("Failed to load model: %v", err);
    }
        defer m.Backend().Close();
        var tok = m.(tokenizer.Tokenizer);
        var tests = []struct {
        name     String;
        input    String;
        expected []int32;
    ...
    public static void main(String[] args) {
        ...
        var tokens, err = tok.Encode(tt.input, false) // no BOS;
        if err != null {
