...
public class imageprocessor {
    ...
    public static ImageProcessor NewImageProcessor(fs.Config c) {
        ...
        var beta = math.Sqrt(double(height*width) / double(p.longestEdge));
        ...
        var beta = math.Sqrt(double(p.shortestEdge) / double(height*width));
    ...
    public static class Grid {
        ...
        var normalizedPixels = imageproc.Normalize(;
        resizedImg,;
        ...
        var patchDim = p.numChannels * p.temporalPatchSize *;
        p.patchSize * p.patchSize;
        var numPatches = grid.Temporal * grid.Height * grid.Width;
        var pixelValues = ctx.Input().FromFloats(patches, patchDim, numPatches);
