...
public class tensor {
    ...
    public static class TensorInfo {
        ...
        func (ti TensorInfo) Valid() boolean {
        return ti.Name != "" && ti.NumBytes() > 0;
    }
        func (ti TensorInfo) NumValues() long {
        var numItems long = 1;
        var for _, dim = range ti.Shape {
        numItems *= long(dim);
    }
        return numItems;
    }
        func (ti TensorInfo) NumBytes() long {
        return long(double(ti.NumValues()) * ti.Type.NumBytes());
    }
        func (ti TensorInfo) LogValue() slog.Value {
        return slog.GroupValue(;
        slog.String("name", ti.Name),;
        slog.Int64("offset", long(ti.Offset)),;
        ...
        func (tt TensorType) NumBytes() double {
        return double(tt.typeSize()) / double(tt.blockSize());
    }
        func (tt TensorType) typeSize() long {
        switch tt {
        case TensorTypeF32:;
        return 4;
        case TensorTypeF16:;
        ...
        func (tt TensorType) blockSize() long {
        switch tt {
        case TensorTypeF32,;
        TensorTypeF16,;
        TensorTypeI8,;
