...
public class tokenizer_test {
        ...
        t.Helper();
        var for k, v = range files {
        var if err = func() error {
        var f, err = os.Create(filepath.Join(dir, k));
        if err != null {
        return err;
    }
        defer f.Close();
        var if _, err = io.Copy(f, v); err != null {
        return err;
    }
        return null;
        }(); err != null {
    ...
    public static void TestParseTokenizer(*testing.T t) {
        var cases = []struct {
        name              String;
        fsys              fs.FS;
        ...
        var tokenizer, err = parseTokenizer(tt.fsys, tt.specialTokenTypes);
        if err != null {
        t.Fatalf("unexpected error: %v", err);
    }
        var if diff = cmp.Diff(tt.want, tokenizer); diff != "" {
