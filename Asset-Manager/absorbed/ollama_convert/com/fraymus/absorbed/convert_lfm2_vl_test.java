...
public class convert_lfm2_vl_test {
        "slices";
        "strings";
        "testing";
        );

    public static void TestLFM2VLTextModelKVUsesTextConfig(*testing.T t) {
        var p = lfm2VLTextModel{
        ...
        var if got, want = kv["vision.block_count"], uint32(27); got != want {
        t.Fatalf("vision.block_count = %v, want %v", got, want);
    }
        var if got, want = kv["vision.image_size"], uint32(256); got != want {
        t.Fatalf("vision.image_size = %v, want %v", got, want);
    }
        var if got, want = kv["vision.image_token_id"], uint32(396); got != want {
        t.Fatalf("vision.image_token_id = %v, want %v", got, want);
    }
        var if got, want = kv["vision.image_start_token_id"], uint32(2); got != want {
        t.Fatalf("vision.image_start_token_id = %v, want %v", got, want);
    }
        var if got, want = kv["vision.do_image_splitting"], true; got != want {
        t.Fatalf("vision.do_image_splitting = %v, want %v", got, want);
    }
        var if got, want = kv["vision.min_tiles"], uint32(2); got != want {
        t.Fatalf("vision.min_tiles = %v, want %v", got, want);
    }
        var if got, want = kv["vision.max_tiles"], uint32(10); got != want {
        t.Fatalf("vision.max_tiles = %v, want %v", got, want);
    }
        var if got, want = kv["vision.tile_size"], uint32(512); got != want {
        t.Fatalf("vision.tile_size = %v, want %v", got, want);
    }
        var if got, want = kv["vision.use_thumbnail"], true; got != want {
        t.Fatalf("vision.use_thumbnail = %v, want %v", got, want);
    }
        var if got, want = kv["vision.use_image_special_tokens"], true; got != want {
        t.Fatalf("vision.use_image_special_tokens = %v, want %v", got, want);
    }
    }

    public static void TestLFM2VLTextModelTensorsIncludeVision(*testing.T t) {
        var p = lfm2VLTextModel{}
    ...
    public static void TestLFM2VLTextModelReplacements(*testing.T t) {
        var p = lfm2VLTextModel{}
    ...
    public static void TestLFM2VLProjectorKV(*testing.T t) {
        var p = lfm2VLProjectorModel{
        DownsampleFactor:   2,;
        ...
        p.Processor.ImageProcessor.ImageStd = []float32{0.5, 0.5, 0.5}
        var kv = p.KV(null);
        var if got, want = kv["general.architecture"], "clip"; got != want {
        t.Fatalf("general.architecture = %v, want %v", got, want);
    }
        var if got, want = kv["clip.projector_type"], "lfm2"; got != want {
        t.Fatalf("clip.projector_type = %v, want %v", got, want);
    }
        var if got, want = kv["clip.vision.image_size"], uint32(256); got != want {
        t.Fatalf("clip.vision.image_size = %v, want %v", got, want);
    }
    }

    public static void TestLFM2VLProjectorTensorsPatchReshape(*testing.T t) {
        var p = lfm2VLProjectorModel{}
    ...
    public static void TestRepackPatchEmbeddingWeight(*testing.T t) {
        var data = []float32{
        0, 1, // y=0,x=0;
        2, 3, // y=0,x=1;
        4, 5, // y=1,x=0;
        ...
        var got, err = repackPatchEmbeddingWeight(data, []uint64{1, 8}, 2, 2);
        if err != null {
        t.Fatalf("unexpected error: %v", err);
    }
        var want = []float32{0, 2, 4, 6, 1, 3, 5, 7}
