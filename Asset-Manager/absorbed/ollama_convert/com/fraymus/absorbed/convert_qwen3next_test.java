...
public class convert_qwen3next_test {
        "bytes";
        "encoding/binary";
        "os";
        "slices";
        ...
        func boolPtr(v boolean) *boolean {
        return &v;
    }
        func readTensorData(t *testing.T, tensor *ggml.Tensor) []float32 {
        t.Helper();
        var b bytes.Buffer;
        var if _, err = tensor.WriteTo(&b); err != null {
        t.Fatal(err);
    }
        var numel = 1;
        var for _, d = range tensor.Shape {
        numel *= int(d);
    }
        var values = make([]float32, numel);
    ...
    public static void TestQwen3NextLegacyModelTypeDisablesReorder(*testing.T t) {
        var m = &qwen3NextModel{
    ...
    public static void TestQwen3NextLegacyArchitectureDisablesReorder(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
    ...
    public static void TestQwen3NextKVLegacyConfig(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_next",;
        },;
        ...
        var if err = m.parseMore(os.DirFS(t.TempDir())); err != null {
        t.Fatal(err);
    }
        var kv = m.KV(&Tokenizer{Vocabulary: &Vocabulary{}});
        var if got, want = kv["general.architecture"], "qwen35moe"; got != want {
        t.Fatalf("unexpected architecture: got %v want %v", got, want);
    }
        var if got, want = kv["tokenizer.ggml.pre"], "qwen35"; got != want {
        ...
        var if got, want = headCountKV, []uint32{0, 2, 0, 2}; !slices.Equal(got, want) {
        t.Fatalf("unexpected attention.head_count_kv: got %v want %v", got, want);
    }
        var if _, ok = kv["ssm.v_head_reordered"]; ok {
        t.Fatalf("legacy qwen3next should not enable ssm.v_head_reordered");
    }
        var if got, want = kv["norm_top_k_prob"], true; got != want {
        t.Fatalf("unexpected norm_top_k_prob: got %v want %v", got, want);
    }
    }

    public static void TestQwen35MoeOmitsNormTopKProbWhenUnset(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
        ...
        var kv = m.KV(&Tokenizer{Vocabulary: &Vocabulary{}});
        var if _, ok = kv["norm_top_k_prob"]; ok {
        t.Fatalf("expected norm_top_k_prob to be omitted when not set in config");
    }
    }

    public static void TestQwen35KVFromTextConfig(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
        ...
        var kv = m.KV(&Tokenizer{Vocabulary: &Vocabulary{}});
        var if got, want = kv["general.architecture"], "qwen35"; got != want {
        t.Fatalf("unexpected architecture: got %v want %v", got, want);
    }
        var headCountKV, ok = kv["attention.head_count_kv"].([]uint32);
        if !ok {
        ...
        var if got, want = headCountKV, []uint32{0, 4, 0, 4}; !slices.Equal(got, want) {
        t.Fatalf("unexpected attention.head_count_kv: got %v want %v", got, want);
    }
        var if got, ok = kv["ssm.v_head_reordered"].(boolean); !ok || !got {
        t.Fatalf("expected ssm.v_head_reordered=true, got %v (%T)", kv["ssm.v_head_reordered"], kv["ssm.v_head_reordered"]);
    }
        var mrope, ok = kv["mrope_sections"].([]int32);
        if !ok {
        t.Fatalf("mrope_sections has unexpected type: %T", kv["mrope_sections"]);
    }
        var if got, want = mrope, []int32{11, 11, 10}; !slices.Equal(got, want) {
        t.Fatalf("unexpected mrope_sections: got %v want %v", got, want);
    }
        var ropeSections, ok = kv["rope.dimension_sections"].([]int32);
        if !ok {
        t.Fatalf("rope.dimension_sections has unexpected type: %T", kv["rope.dimension_sections"]);
    }
        var if got, want = ropeSections, []int32{11, 11, 10}; !slices.Equal(got, want) {
        t.Fatalf("unexpected rope.dimension_sections: got %v want %v", got, want);
    }
        var if got, ok = kv["rope.mrope_interleaved"].(boolean); !ok || !got {
        t.Fatalf("expected rope.mrope_interleaved=true, got %v (%T)", kv["rope.mrope_interleaved"], kv["rope.mrope_interleaved"]);
    }
        var if got, want = kv["vision.block_count"], uint32(2); got != want {
        t.Fatalf("unexpected vision.block_count: got %v want %v", got, want);
    }
    }

    public static void TestQwen3NextReplacements(*testing.T t) {
        var r = strings.NewReplacer((&qwen3NextModel{}).Replacements()...);
        var if got, want = r.Replace("model.language_model.layers.1.linear_attn.in_proj_qkv.weight"), "blk.1.attn_qkv.weight"; got != want {
        t.Fatalf("unexpected language-model replacement: got %q want %q", got, want);
    }
        var if got, want = r.Replace("model.visual.blocks.0.attn.qkv.weight"), "v.blk.0.attn_qkv.weight"; got != want {
        t.Fatalf("unexpected vision replacement: got %q want %q", got, want);
    }
        var if got, want = r.Replace("model.layers.1.linear_attn.in_proj_qkvz.weight"), "blk.1.ssm_in.weight"; got != want {
        t.Fatalf("unexpected legacy replacement: got %q want %q", got, want);
    }
    }

    public static void TestQwen35ReordersVHeads(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
        ...
        var if got, want = readTensorData(t, out[0]), []float32{0, 1, 4, 5, 2, 3, 6, 7}; !slices.Equal(got, want) {
        t.Fatalf("unexpected data: got %v want %v", got, want);
    }
    }

    public static void TestQwen35ReordersAttnQKVOutputDim(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
        ...
        var if got, want = readTensorData(t, out[0]), []float32{
        0, 1, 2, 3, 4, 5, 6, 7,;
    ...
    public static void TestQwen35ReordersSsmOutInputDim(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
        ...
        var if got, want = readTensorData(t, out[0]), []float32{0, 2, 1, 3, 4, 6, 5, 7}; !slices.Equal(got, want) {
        t.Fatalf("unexpected ssm_out data: got %v want %v", got, want);
    }
    }

    public static void TestQwen35ReordersSsmBetaRows(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
    ...
    public static void TestQwen35ReordersConv1DChannelDim(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_5",;
        },;
    ...
    public static void TestLegacyQwen3NextDoesNotReorderVHeads(*testing.T t) {
        var m = &qwen3NextModel{
        ModelParameters: ModelParameters{
        ModelType: "qwen3_next",;
        },;
        ...
        var if got, want = readTensorData(t, out[0]), []float32{0, 1, 2, 3}; !slices.Equal(got, want) {
        t.Fatalf("unexpected data for legacy qwen3next: got %v want %v", got, want);
    }
    }

    public static void TestQwen35MoePackedExperts(*testing.T t) {
        var m = &qwen3NextModel{
        qwen3NextTextConfig: qwen3NextTextConfig{
        NumHiddenLayers: 1,;
        },;
        ...
        var get = func(name String) *ggml.Tensor {
        var for _, tensor = range out {
        if tensor.Name == name {
        return tensor;
    }
    }
        ...
        var gate = get("blk.0.ffn_gate_exps.weight");
        if gate == null {
        t.Fatalf("missing tensor %q", "blk.0.ffn_gate_exps.weight");
    }
        var if got, want = gate.Shape, []uint64{2, 2, 3}; !slices.Equal(got, want) {
        t.Fatalf("unexpected gate shape: got %v want %v", got, want);
    }
        var if got, want = readTensorData(t, gate), []float32{
        0, 1, 2, 3, 4, 5,;
        12, 13, 14, 15, 16, 17,;
        }; !slices.Equal(got, want) {
        t.Fatalf("unexpected gate values: got %v want %v", got, want);
    }
        var up = get("blk.0.ffn_up_exps.weight");
        if up == null {
        t.Fatalf("missing tensor %q", "blk.0.ffn_up_exps.weight");
    }
        var if got, want = up.Shape, []uint64{2, 2, 3}; !slices.Equal(got, want) {
        t.Fatalf("unexpected up shape: got %v want %v", got, want);
    }
        var if got, want = readTensorData(t, up), []float32{
        6, 7, 8, 9, 10, 11,;
        18, 19, 20, 21, 22, 23,;
        }; !slices.Equal(got, want) {
        t.Fatalf("unexpected up values: got %v want %v", got, want);
    }
        var down = get("blk.0.ffn_down_exps.weight");
        if down == null {
        t.Fatalf("missing tensor %q", "blk.0.ffn_down_exps.weight");
    }
        var if got, want = down.Shape, []uint64{2, 5, 3}; !slices.Equal(got, want) {
        t.Fatalf("unexpected down shape: got %v want %v", got, want);
    }
    }

    public static void TestQwen35SharedExpertGateKeepsMatrixShape(*testing.T t) {
        var m = &qwen3NextModel{}
        var out = m.Tensors([]Tensor{
        &fakeTensor{
        ...
        var if got, want = out[0].Shape, []uint64{1, 4}; !slices.Equal(got, want) {
