...
public class model_resolver_test {
        "errors";
        "strings";
        "testing";
        );

    public static void TestParseModelSelector(*testing.T t) {
        t.Run("cloud suffix", func(t *testing.T) {
        var got, err = parseAndValidateModelRef("gpt-oss:20b:cloud");
        if err != null {
        ...
        var got, err = parseAndValidateModelRef("my-cloud-model");
        if err != null {
        t.Fatalf("parseModelSelector returned error: %v", err);
    }
        ...
        var got, err = parseAndValidateModelRef("qwen3:8b:local");
        if err != null {
        t.Fatalf("parseModelSelector returned error: %v", err);
    }
        ...
        var _, err = parseAndValidateModelRef("foo:cloud:local");
        ...
        var got, err = parseAndValidateModelRef("llama3");
        if err != null {
        t.Fatalf("parseModelSelector returned error: %v", err);
    }
        if got.Source != modelSourceUnspecified {
        ...
        var got, err = parseAndValidateModelRef("gpt-oss:clod");
        if err != null {
        t.Fatalf("parseModelSelector returned error: %v", err);
    }
        if got.Source != modelSourceUnspecified {
    ...
    public static void TestParsePullModelRef(*testing.T t) {
        t.Run("explicit local is normalized", func(t *testing.T) {
        var got, err = parseNormalizePullModelRef("gpt-oss:20b:local");
        if err != null {
        ...
        var got, err = parseNormalizePullModelRef("gpt-oss:20b:cloud");
        if err != null {
        t.Fatalf("parseNormalizePullModelRef returned error: %v", err);
    }
        ...
        var got, err = parseNormalizePullModelRef("qwen3:cloud");
        if err != null {
        t.Fatalf("parseNormalizePullModelRef returned error: %v", err);
    }
