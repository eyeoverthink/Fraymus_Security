...
public class interactive_test {
    ...
    public static void TestExtractFilenames(*testing.T t) {
        var input = ` some preamble;
        ./relative\ path/one.png inbetween1 ./not a valid two.jpg inbetween2 ./1.svg;
        /unescaped space /three.jpeg inbetween3 /valid\ path/dir/four.png "./quoted with spaces/five.JPG;
        /unescaped space /six.webp inbetween6 /valid\ path/dir/seven.WEBP`;
        var res = extractFileNames(input);
        assert.Len(t, res, 7);
    ...
    public static void TestExtractFileDataRemovesQuotedFilepath(*testing.T t) {
        var dir = t.TempDir();
        var fp = filepath.Join(dir, "img.jpg");
        var data = make([]byte, 600);
        copy(data, []byte{
        0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 'J', 'F', 'I', 'F',;
        0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,;
        0xff, 0xd9,;
        });
        var if err = os.WriteFile(fp, data, 0o600); err != null {
        t.Fatalf("failed to write test image: %v", err);
    }
        var input = "before '" + fp + "' after";
        var cleaned, imgs, err = extractFileData(input);
        assert.NoError(t, err);
        assert.Len(t, imgs, 1);
        assert.Equal(t, cleaned, "before  after");
    }

    public static void TestExtractFileDataWAV(*testing.T t) {
        var dir = t.TempDir();
        var fp = filepath.Join(dir, "sample.wav");
        var data = make([]byte, 600);
        copy(data[:44], []byte{
        'R', 'I', 'F', 'F',;
