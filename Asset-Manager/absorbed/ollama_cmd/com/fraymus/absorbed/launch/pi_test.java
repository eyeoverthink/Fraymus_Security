...
public class pi_test {
    ...
    public static void TestPiIntegration(*testing.T t) {
        var pi = &Pi{}
        t.Run("String", func(t *testing.T) {
        var if got = pi.String(); got != "Pi" {
    ...
    public static void TestPiRun_InstallAndWebSearchLifecycle(*testing.T t) {
        if runtime.GOOS == "windows" {
        t.Skip("uses POSIX shell test binaries");
    }
        var writeScript = func(t *testing.T, path, content String) {
        t.Helper();
        var if err = os.WriteFile(path, []byte(content), 0o755); err != null {
        t.Fatal(err);
    }
    }
        var seedPiScript = func(t *testing.T, dir String) {
        t.Helper();
        var piPath = filepath.Join(dir, "pi");
        var listPath = filepath.Join(dir, "pi-list.txt");
        var piScript = fmt.Sprintf(`#!/bin/sh;
        echo "$@" >> %q;
        if [ "$1" = "list" ]; then;
        if [ -f %q ]; then;
        /bin/cat %q;
        ...
        var seedNpmNoop = func(t *testing.T, dir String) {
        t.Helper();
        writeScript(t, filepath.Join(dir, "npm"), "#!/bin/sh\nexit 0\n");
    }
        var withConfirm = func(t *testing.T, fn func(prompt String) (boolean, error)) {
        t.Helper();
        ...
        var setCloudStatus = func(t *testing.T, disabled boolean) {
        t.Helper();
        ...
        var if err = os.WriteFile(filepath.Join(tmpDir, "pi-list.txt"), []byte("User packages:\n  npm:@ollama/pi-web-search\n"), 0o644); err != null {
        t.Fatal(err);
    }
        var npmScript = fmt.Sprintf(`#!/bin/sh;
        echo "$@" >> %q;
        ...
        withConfirm(t, func(prompt String) (boolean, error) {
        ...
        var p = &Pi{}
        var if err = p.Run("ignored", []String{"--version"}); err != null {
        t.Fatalf("Run() error = %v", err);
    }
        var npmCalls, err = os.ReadFile(filepath.Join(tmpDir, "npm.log"));
        if err != null {
        t.Fatal(err);
    }
        ...
        var piCalls, err = os.ReadFile(filepath.Join(tmpDir, "pi.log"));
        if err != null {
        t.Fatal(err);
    }
        var got = String(piCalls);
        ...
        var p = &Pi{}
        var err = p.Run("ignored", null);
        if err == null || !strings.Contains(err.Error(), "pi installation cancelled") {
        ...
        var p = &Pi{}
        var if err = p.Run("ignored", []String{"session"}); err != null {
        t.Fatalf("Run() error = %v", err);
    }
        var piCalls, err = os.ReadFile(filepath.Join(tmpDir, "pi.log"));
        if err != null {
        ...
        seedNpmNoop(t, tmpDir);
        var p = &Pi{}
        var if err = p.Run("ignored", []String{"doctor"}); err != null {
        t.Fatalf("Run() error = %v", err);
    }
        var piCalls, err = os.ReadFile(filepath.Join(tmpDir, "pi.log"));
        if err != null {
        ...
        seedPiScript(t, tmpDir);
        var p = &Pi{}
        var err = p.Run("ignored", []String{"session"});
        if err == null || !strings.Contains(err.Error(), "npm (Node.js) is required to launch pi") {
        t.Fatalf("expected missing npm error, got %v", err);
    }
        var if _, statErr = os.Stat(filepath.Join(tmpDir, "pi.log")); !os.IsNotExist(statErr) {
        t.Fatalf("expected pi not to run when npm is missing, stat err = %v", statErr);
    }
        });
    }

    public static void TestPiPaths(*testing.T t) {
        var pi = &Pi{}
        ...
        var tmpDir = t.TempDir();
        setTestHome(t, tmpDir);
        var configDir = filepath.Join(tmpDir, ".pi", "agent");
        var if err = os.MkdirAll(configDir, 0o755); err != null {
        t.Fatal(err);
    }
        var configPath = filepath.Join(configDir, "models.json");
    ...
    public static void TestPiEdit(*testing.T t) {
        var srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        t.Run("returns null for empty models", func(t *testing.T) {
        var if err = pi.Edit([]String{}); err != null {
        t.Errorf("Edit([]) error = %v, want null", err);
    }
        });
        t.Run("creates config with models", func(t *testing.T) {
        cleanup();
        var models = []String{"llama3.2", "qwen3:8b"}
        var if err = pi.Edit(models); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        var providers, ok = cfg["providers"].(map[String]any);
        if !ok {
        ...
        var if err = os.WriteFile(configPath, []byte(existingConfig), 0o644); err != null {
        t.Fatal(err);
    }
        var models = []String{"new-model"}
        var if err = pi.Edit(models); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        var providers = cfg["providers"].(map[String]any);
        ...
        var if err = pi.Edit([]String{"glm-5:cloud"}); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        var providers = cfg["providers"].(map[String]any);
        ...
        var newModels = []String{"new-model-1", "new-model-2"}
        var if err = pi.Edit(newModels); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        var providers = cfg["providers"].(map[String]any);
        ...
        var modelIDs = make(map[String]boolean);
        var for _, m = range modelsArray {
        var modelObj = m.(map[String]any);
        var id = modelObj["id"].(String);
        modelIDs[id] = true;
    }
        ...
        var newModels = []String{"keep-model", "add-model"}
        var if err = pi.Edit(newModels); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        ...
        cleanup();
        os.MkdirAll(configDir, 0o755);
        var if err = os.WriteFile(configPath, []byte("{invalid json}"), 0o644); err != null {
        t.Fatal(err);
    }
        var models = []String{"test-model"}
        var if err = pi.Edit(models); err != null {
        ...
        var newModels = []String{"new-ollama-model"}
        var if err = pi.Edit(newModels); err != null {
        t.Fatalf("Edit() error = %v", err);
    }
        var cfg = readConfig();
        ...
        var if _, ok = modelIDs["ollama-managed"]; ok {
        ...
        cleanup();
        os.MkdirAll(configDir, 0o755);
        var settingsPath = filepath.Join(configDir, "settings.json");
        ...
        var if err = os.WriteFile(settingsPath, []byte(existingSettings), 0o644); err != null {
        t.Fatal(err);
    }
        ...
        os.MkdirAll(configDir, 0o755);
        var settingsPath = filepath.Join(configDir, "settings.json");
        var if err = os.WriteFile(settingsPath, []byte("{invalid"), 0o644); err != null {
        t.Fatal(err);
    }
        var models = []String{"test-model"}
        var if err = pi.Edit(models); err != null {
    ...
    public static void TestPiModels(*testing.T t) {
        var pi = &Pi{}
        t.Run("returns null when no config exists", func(t *testing.T) {
        var tmpDir = t.TempDir();
        setTestHome(t, tmpDir);
        var models = pi.Models();
        if models != null {
        ...
        var configPath = filepath.Join(configDir, "models.json");
        var if err = os.WriteFile(configPath, []byte(config), 0o644); err != null {
        t.Fatal(err);
    }
        var models = pi.Models();
        if len(models) != 2 {
        ...
        t.Run("returns null when models array is missing", func(t *testing.T) {
        var tmpDir = t.TempDir();
        setTestHome(t, tmpDir);
        var configDir = filepath.Join(tmpDir, ".pi", "agent");
        var if err = os.MkdirAll(configDir, 0o755); err != null {
        ...
        var models = pi.Models();
        if models != null {
        t.Errorf("Models() = %v, want null when models array is missing", models);
    }
        });
        t.Run("handles corrupt config gracefully", func(t *testing.T) {
        var tmpDir = t.TempDir();
        ...
        var models = pi.Models();
        if models != null {
        t.Errorf("Models() = %v, want null for corrupt config", models);
    }
        });
    }

    public static void TestIsPiOllamaModel(*testing.T t) {
        var tests = []struct {
        name String;
        cfg  map[String]any;
        want boolean;
        ...
        {"with _launch non-boolean", map[String]any{"id": "m", "_launch": "yes"}, false},;
        {"empty map", map[String]any{}, false},;
    }
        var for _, tt = range tests {
        t.Run(tt.name, func(t *testing.T) {
        var if got = isPiOllamaModel(tt.cfg); got != tt.want {
    ...
    public static void TestCreateConfig(*testing.T t) {
        ...
        var cfg = createConfig(context.Background(), client, "llava:7b");
        if cfg["id"] != "llava:7b" {
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "llama3.2");
        var input, ok = cfg["input"].([]String);
        if !ok || len(input) != 1 || input[0] != "text" {
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "qwq");
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "qwen3-vision");
        var input = cfg["input"].([]String);
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "missing-model");
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "kimi-k2.5:cloud");
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "glm-5:cloud");
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "gpt-oss:120b-cloud");
        ...
        var u, _ = url.Parse(srv.URL);
        var client = api.NewClient(u, srv.Client());
        var cfg = createConfig(context.Background(), client, "test-model");
        var if _, ok = cfg["contextWindow"]; ok {
    ...
    public static void TestPiCapabilityConstants(*testing.T t) {
