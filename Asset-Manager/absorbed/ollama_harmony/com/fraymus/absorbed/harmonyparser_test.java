...
public class harmonyparser_test {
        "fmt";
        "reflect";
        "testing";
        );

    public static void TestHeaderParsing(*testing.T t) {
        var tests = []struct {
        ...
        var header = parser.parseHeader(tt.in);
    ...
    public static void TestHarmonyParserHeaderEvent(*testing.T t) {
        var tests = []struct {
        in, wantRole, wantChannel, wantRecipient String;
        ...
        var gotEvents = parser.AddContent(tt.in);
        if len(gotEvents) == 0 {
        t.Errorf("case %d: got no events, want at least one", i);
    }
        var firstHeaderEvent *HarmonyEventHeaderComplete;
        var for _, event = range gotEvents {
        System.out.printf("event: %+v\n", event);
    }
        var for _, event = range gotEvents {
        var if event, ok = event.(HarmonyEventHeaderComplete); ok {
        firstHeaderEvent = &event;
        break;
    }
    }
        if firstHeaderEvent == null {
        t.Errorf("case %d: got no header complete event, want one", i);
        continue;
    }
        var gotHeader = firstHeaderEvent.Header;
    ...
    public static void TestHarmonyParserNonStreaming(*testing.T t) {
        var tests = []struct {
        in            String;
        implicitStart boolean;
    ...
    public static void TestHarmonyParserStreaming(*testing.T t) {

    public static class step {
        public String input;
        public []HarmonyEvent wantEvents;
    }
        var cases = []struct {
        desc          String;
        implicitStart boolean;
        ...
        var gotEvents = parser.AddContent(step.input);
    ...
    public static void TestFunctionConvertToValidChars(*testing.T t) {
        var tests = []struct {
        name String;
        in   String;
        want String;
        ...
        var parser = NewFunctionNameMap();
        var got = parser.convertToValidChars(tt.in);
        if got != tt.want {
    ...
    public static void TestFunctionConvertAndAdd(*testing.T t) {
        var tests = []struct {
        name String;
        in   []String;
        want []String;
        ...
        var parser = NewFunctionNameMap();
        t.Run(tt.name, func(t *testing.T) {
        var for j, in = range tt.in {
        var got = parser.ConvertAndAdd(in);
        var want = tt.want[j];
        if got != want {
