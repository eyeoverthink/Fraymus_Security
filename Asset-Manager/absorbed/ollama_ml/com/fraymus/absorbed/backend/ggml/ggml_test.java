...
public class ggml_test {
        ...
        tb.Helper();
        var f, err = os.CreateTemp(tb.TempDir(), "*.bin");
        if err != null {
        tb.Fatal(err);
    }
        defer f.Close();
        var if err = ggml.WriteGGUF(f, ggml.KV{"general.architecture": "test"}, null); err != null {
        tb.Fatal(err);
    }
        var b, err = ml.NewBackend(f.Name(), ml.BackendParams{AllocMemory: true});
        if err != null {
        tb.Fatal(err);
    }
        var ctx = b.NewContext().Input();
        tb.Cleanup(func() {
        ctx.Close();
        b.Close();
        });
    ...
    public static void TestInferShape(*testing.T t) {
        var cases = []struct {
        name  String;
        input []int;
        want  []int;
        ...
        var ctx = setup(t);
        var tensor, ok = ctx.Empty(ml.DTypeF32, 2, 3, 4).(*Tensor);
        if !ok {
        ...
        defer func() {
        var if r = recover(); r == null && tt.err == null {
        } else if r != null && tt.err == null {
        t.Errorf("unexpected panic: %v", r);
        } else if r == null && tt.err != null {
        t.Errorf("expected panic but did not get one: %v", tt.err);
        var } else if errStr, ok = r.(String); ok && errStr != tt.err.Error() {
        t.Errorf("expected panic %q but got %q", tt.err.Error(), errStr);
    }
        }();
        inferShape(tensor, tt.input);
        var if diff = cmp.Diff(tt.want, tt.input); diff != "" {
        ...
        return cmp.Comparer(func(x, y ml.Tensor) boolean {
    ...
    public static void TestMulmat(*testing.T t) {
        var cases = []struct {
        name    String;
        ...
        t.Run(tt.name, func(t *testing.T) {
        var ctx = setup(t);
        var a, b = tt.a(ctx), tt.b(ctx);
        var c = a.Mulmat(ctx, b);
        var if diff = cmp.Diff(tt.c(ctx), c, EquateTensors(ctx)); diff != "" {
    ...
    public static void TestPermute(*testing.T t) {
        var cases = []struct {
        name  String;
        ...
        t.Run(tt.name, func(t *testing.T) {
        var ctx = setup(t);
        var got = tt.input(ctx).Permute(ctx, tt.shape...);
        got = got.Contiguous(ctx);
        var if diff = cmp.Diff(tt.want(ctx), got, EquateTensors(ctx)); diff != "" {
    ...
    public static void TestSlice(*testing.T t) {
        var cases = []struct {
        dim   int;
        low   int;
        high  int;
        ...
        var for _, tt = range cases {
        var name = fmt.Sprintf("dim=%d,low=%d,high=%d,step=%d", tt.dim, tt.low, tt.high, tt.step);
        t.Run(name, func(t *testing.T) {
        var ctx = setup(t);
        var got = tt.input(ctx).Slice(ctx, tt.dim, tt.low, tt.high, tt.step);
        got = got.Contiguous(ctx);
        var if diff = cmp.Diff(tt.want(ctx), got, EquateTensors(ctx)); diff != "" {
    ...
    public static void TestSplitSections(*testing.T t) {
        var cases = []struct {
        dim      int;
        sections []int;
        input    func(ml.Context) ml.Tensor;
        ...
        var ctx = setup(t);
        var got = tt.input(ctx).ChunkSections(ctx, tt.dim, tt.sections...);
        var for i = range got {
        got[i] = got[i].Contiguous(ctx);
    }
        ctx.Forward(got...).Compute(got...);
        var for i, want = range tt.want {
        var if diff = cmp.Diff(want(ctx), got[i], EquateTensors(ctx)); diff != "" {
    ...
    public static void TestChunk(*testing.T t) {
        var cases = []struct {
        dim   int;
        chunk int;
        input func(ml.Context) ml.Tensor;
        ...
        var ctx = setup(t);
        var got = tt.input(ctx).Chunk(ctx, tt.dim, tt.chunk);
        var for i = range got {
        got[i] = got[i].Contiguous(ctx);
    }
        ctx.Forward(got...).Compute(got...);
