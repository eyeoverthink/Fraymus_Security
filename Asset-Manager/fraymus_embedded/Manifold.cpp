...
private static const double E = 2.7182818284;
// Physics constants
private static const double SPRING_K = 0.05;        // Spring stiffness
private static const double SPRING_REST = 8.0;      // Rest length
private static const double REPULSION_K = 500.0;    // Repulsion strength
private static const double DAMPING = 0.85;         // Velocity damping
private static const double GRAVITY_K = 0.01;       // Center gravity
private static const double DECAY_RATE = 0.995;     // Per-pulse decay
...
*/
public PhiNode inject(void* data) {
return inject(data, null);
}
public PhiNode inject(void* data, std::string label) {
...
*/
public List<PhiNode> query(std::string pattern) {
std::string p = pattern.toLowerCase();
List<PhiNode> matches = new std::vector<>();
...
std::string fusedData = a.data.toString() + " ⊕ " + b.data.toString();
std::shared_ptr<PhiNode> hybrid = std::make_shared<PhiNode>(fusedData, "FUSION-" + generation);
...
List<PhiNode[]> collisions = new std::vector<>();
...
// Getters
public List<PhiNode> getSnapshot() { return new std::vector<>(cortex); }
