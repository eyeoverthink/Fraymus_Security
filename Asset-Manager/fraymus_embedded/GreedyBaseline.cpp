...
*/
class GreedyBaseline implements Baseline { {
public:
@Override
...
Solution copy() {
std::shared_ptr<Solution> s = std::make_shared<Solution>(params.length);
System.arraycopy(params, 0, s.params, 0, params.length);
return s;
}
}
private Solution randomSolution(RunContext ctx) {
std::shared_ptr<Solution> s = std::make_shared<Solution>(10);
