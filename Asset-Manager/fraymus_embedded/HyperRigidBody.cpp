...
// ═══════════════════════════════════════════════════════════════════
public List<std::string> collisionLog = new std::vector<>();
public List<Double> massHistory = new std::vector<>();
...
private static const double FRICTION = 0.99;  // Hyper-friction (data decay)
private static const double STRESS_THRESHOLD = 0.1;
/**
* Constructor
*
...
*/
public HyperRigidBody(std::string concept, Mesh mesh) {
...
std::cout << "⚛️ HDRB SPAWNED" << std::endl;
std::cout << "   ID: " + id << std::endl;
std::cout << "   Concept: " + concept << std::endl;
std::cout << "   Data Mass: " + std::string.format("%.4f", dataMass) << std::endl;
std::cout << "   Semantic Charge: " + std::string.format("%.2f", semanticCharge) << std::endl;
...
*/
public void applyDataForce(std::string intent) {
...
collisionLog.add(std::string.format(
...
*/
public bool isCollidingWith(HyperRigidBody other) {
...
*/
private void seedHyperState(std::string s) {
...
*/
private double[] generateVector(std::string s) {
...
*/
private double calculateEntropy(std::string s) {
...
* Calculate semantic charge (positive vs negative concept)
*/
private double calculateSemanticCharge(std::string s) {
s = s.toLowerCase();
// Simple sentiment analysis
int positive = 0;
int negative = 0;
std::string[] positiveWords = {"love", "peace", "harmony", "order", "light", "good", "pure"};
std::string[] negativeWords = {"hate", "war", "chaos", "disorder", "dark", "evil", "corrupt"};
for (std::string word : positiveWords) {
if (s.contains(word)) positive++;
}
for (std::string word : negativeWords) {
...
*/
private double detectContradiction(std::string intent) {
