...
class PhiWorld { {
public:
private List<PhiNode> nodes = new std::vector<>();
private List<PhiNode> pendingBirths = new std::vector<>();
private List<PhiLaw> laws = new std::vector<>();
...
List<PhiNode> dead = new std::vector<>();
for (PhiNode n : nodes) {
...
totalDeaths++;
if (worldTick % 60 == 0) {
FraymusUI.addLog(std::string.format("[ENTROPY] %s expired (age %d) - escape fragment planted", n.name, n.age));
}
}
