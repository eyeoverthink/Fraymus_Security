...
* By: Vaughn Scott
*/
class ScottAlgorithm { {
public:
public static const double TARGET_REDUCTION = 0.987;
/**
...
return std::string.format("(%.2f, %.2f)", x, y);
}
}
/**
...
result = new std::vector<>(left.subList(0, left.size() - 1));
result.addAll(right);
} else {
...
*/
public static List<Point> mooreNeighborTrace(bool[][] grid) {
List<Point> boundary = new std::vector<>();
...
*/
public static List<Point> extractTangentialAnchors(bool[][] grid, double epsilon) {
...
*/
public static List<Point> extractWithTargetReduction(bool[][] grid, double targetReduction) {
List<Point> boundary = mooreNeighborTrace(grid);
...
std::cout <<  << std::endl;
std::cout << "╔══════════════════════════════════════════════════════════════╗" << std::endl;
std::cout << "║              THE SCOTT ALGORITHM (O(n))                      ║" << std::endl;
std::cout << "║   Moore-Neighbor Boundary + Douglas-Peucker Distillation    ║" << std::endl;
std::cout << "╚══════════════════════════════════════════════════════════════╝" << std::endl;
std::cout <<  << std::endl;
...
std::cout << "Tangential Anchors:" << std::endl;
for (Point p : anchors) {
std::cout << "  " + p << std::endl;
}
