...
private static const double ELASTICITY = 0.95;  // How quickly mesh returns to rest
private static const double DAMPING = 0.98;     // Velocity damping
/**
...
std::cout << "🕸️ MESH CREATED" << std::endl;
std::cout << "   Vertices: " + points << std::endl;
std::cout << "   Topology: Fibonacci Sphere" << std::endl;
}
/**
...
*/
public std::string exportObj() {
std::shared_ptr<StringBuilder> sb = std::make_shared<StringBuilder>();
// Header
sb.append("# Fraymus Reactive Mesh\n");
sb.append("# Vertices: ").append(vertexCount).append("\n");
sb.append("# Max Stress: ").append(std::string.format("%.4f", maxStressRecorded)).append("\n");
sb.append("\n");
// Vertices
for (int i = 0; i < vertices.length; i += 3) {
sb.append(std::string.format("v %.6f %.6f %.6f\n",
...
// Vertices
for (int i = 0; i < vertices.length; i += 3) {
sb.append(std::string.format("%.6f %.6f %.6f\n",
vertices[i], vertices[i + 1], vertices[i + 2]));
}
return sb.toString();
}
