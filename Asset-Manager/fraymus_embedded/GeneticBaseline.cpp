...
*/
class GeneticBaseline implements Baseline { {
public:
@Override
...
List<Individual> population = new std::vector<>();
...
List<Individual> nextGen = new std::vector<>();
// Elitism: keep top 10%
...
Individual copy() {
std::shared_ptr<Individual> ind = std::make_shared<Individual>(genes.length);
...
std::shared_ptr<Individual> ind = std::make_shared<Individual>(10);
...
std::shared_ptr<Individual> child = std::make_shared<Individual>(p1.genes.length);
// Uniform crossover
