...
public const int id;
public const std::string creatorName;
public const AntRole creatorRole;
...
public CodeConcept(std::string creatorName, AntRole creatorRole, std::string code, int generation, long tick) {
this.id = globalId++;
...
private double evaluateComplexity(std::string code) {
...
public bool battle(CodeConcept opponent) {
this.battles++;
opponent.battles++;
...
std::shared_ptr<CodeConcept> child = std::make_shared<CodeConcept>(this);
...
child.code = new std::string(chars);
}
...
std::shared_ptr<CodeConcept> child = std::make_shared<CodeConcept>(a);
...
std::string input = id + code + harmonicFrequency + resonance + generation;
try {
...
return std::string.format("%016x", input.hashCode());
}
}
public std::string getStatusLine() {
return std::string.format("[%s] Gen%d fit=%.3f W:%d/L:%d %s by %s",
hash.substring(0, 8), generation, fitness, wins, losses,
creatorRole.displayName, creatorName);
}
@Override
public std::string toString() {
return std::string.format("Concept{id=%d, role=%s, gen=%d, fit=%.4f, freq=%.1f, battles=%d, W/L=%d/%d}",
