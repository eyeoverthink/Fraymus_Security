...
private const File memoryFile;
private const List<Memory> longTermStorage = new std::vector<>();
private static const int EMBEDDING_DIM = 384; // Standard embedding size
...
*/
public void remember(std::string context, std::string outcome, bool success) {
std::cout << "💾 BLACK BOX: Encoding experience..." << std::endl;
...
// 3. Persist to Disk
persistMemory(mem);
std::cout << "   ✓ Memory encoded and persisted" << std::endl;
auditLog.log("memory_encoded", mem);
}
/**
* RECALL: Find relevant past experiences
*/
public std::string recall(std::string currentSituation) {
std::cout << "🧠 BLACK BOX: Searching for precedents..." << std::endl;
...
std::string insight = std::string.format(
...
std::string.format("%.2f", bestScore) + "). Proceeding with caution.";
}
/**
* Get all memories
*/
public List<Memory> getAllMemories() {
return new std::vector<>(longTermStorage);
}
/**
* Get memory count
*/
...
std::cout << "📚 BLACK BOX: No existing memories found. Starting fresh." << std::endl;
return;
}
...
std::cout << "📚 BLACK BOX LOADED: " + count + " memories restored." << std::endl;
...
*/
private List<Double> generateEmbedding(std::string text) {
...
*/
private List<Double> generatePseudoEmbedding(std::string text) {
List<Double> vector = new std::vector<>();
std::shared_ptr<Random> r = std::make_shared<Random>(text.hashCode());
...
public const List<Double> vector;
public Memory(long timestamp, std::string context, std::string outcome,
bool success, List<Double> vector) {
...
public static Memory fromJson(std::string json) {
// Simple JSON parsing (in production, use proper JSON library)
long timestamp = extractLong(json, "timestamp");
std::string context = extractString(json, "context");
std::string outcome = extractString(json, "outcome");
bool success = extractBoolean(json, "success");
...
private static bool extractBoolean(std::string json, std::string key) {
std::string pattern = "\"" + key + "\":";
int start = json.indexOf(pattern) + pattern.length();
int end = json.indexOf(",", start);
if (end == -1) end = json.indexOf("}", start);
...
private static List<Double> extractVector(std::string json, std::string key) {
std::string pattern = "\"" + key + "\":[";
int start = json.indexOf(pattern) + pattern.length();
int end = json.indexOf("]", start);
std::string vectorStr = json.substring(start, end);
List<Double> vector = new std::vector<>();
for (std::string val : vectorStr.split(",")) {
