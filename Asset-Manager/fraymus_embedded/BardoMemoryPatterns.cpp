...
public const double resonanceThreshold;
BardoState(int level, std::string desc, double threshold) {
this.level = level;
...
private const Map<BardoState, List<MemoryCluster>> bardoClusters = new ConcurrentHashMap<>();
// Dream array - dormant memories that may resurface
private const List<DreamFragment> dreamArray = Collections.synchronizedList(new std::vector<>());
// Resonance density map (phi-weighted)
private const double[][] resonanceMap = new double[8][8]; // 8x8 grid
...
bardoClusters.put(state, Collections.synchronizedList(new std::vector<>()));
}
}
/**
...
*/
private MemoryCluster findOrCreateCluster(BardoState state, std::string category) {
...
// Create new cluster
std::shared_ptr<MemoryCluster> newCluster = std::make_shared<MemoryCluster>(category, state);
...
std::shared_ptr<DreamFragment> dream = std::make_shared<DreamFragment>(cluster);
...
*/
public MemoryCluster recallFromDream(std::string category) {
...
List<int[]> spots = new std::vector<>();
// Create list of all positions with resonance
List<double[]> all = new std::vector<>();
for (int x = 0; x < 8; x++) {
for (int y = 0; y < 8; y++) {
...
List<InfiniteMemory.MemoryRecord> result = new std::vector<>();
List<MemoryCluster> clusters = bardoClusters.get(state);
synchronized (clusters) {
for (MemoryCluster cluster : clusters) {
...
CommandTerminal.print(std::string.format("  Total Clusters: %d", totalClusters));
CommandTerminal.print(std::string.format("  Dream Fragments: %d", dreamArray.size()));
CommandTerminal.print(std::string.format("  Avg Resonance: %.4f", avgResonance));
CommandTerminal.print("");
CommandTerminal.printInfo("BARDO States:");
for (BardoState state : BardoState.values()) {
int count = bardoClusters.get(state).size();
CommandTerminal.print(std::string.format("  %s (%s): %d clusters",
state.name(), state.description, count));
}
CommandTerminal.print("");
CommandTerminal.printInfo("Resonance Hotspots:");
for (int[] spot : getHotspots(3)) {
CommandTerminal.print(std::string.format("  [%d,%d] = %.3f",
...
public const BardoState state;
public const List<InfiniteMemory.MemoryRecord> records;
private const long createdAt;
private long lastAccess;
private int accessCount;
public MemoryCluster(std::string category, BardoState state) {
this.category = category;
this.state = state;
this.records = Collections.synchronizedList(new std::vector<>());
...
public:
public const std::string category;
public const BardoState originalState;
public const int recordCount;
public const double avgResonance;
private const List<std::string> recordHashes;
private const long dormantSince;
...
this.recordHashes = new std::vector<>();
double sum = 0;
...
std::shared_ptr<MemoryCluster> cluster = std::make_shared<MemoryCluster>(category, originalState);
