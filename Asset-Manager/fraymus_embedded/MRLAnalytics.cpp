...
Quadrant(std::string desc, double weight) {
this.description = desc;
this.weight = weight;
}
}
// Time-series metrics
private const List<MetricSnapshot> snapshots = Collections.synchronizedList(new std::vector<>());
private const Map<std::string, Double> currentMetrics = new LinkedHashMap<>();
// Phase tracking
private double phaseConsistency = 1.0;
private double[] phaseHistory = new double[100];
private int phaseIndex = 0;
// Latency tracking
private const List<Long> latencyHistory = new std::vector<>();
...
*/
public void recordMetric(std::string name, double value) {
...
*/
public void recordDataAccess(bool isWrite) {
if (isWrite) {
dataWrites++;
} else {
dataReads++;
...
bool highRes = resonance > 0.6;
bool highAct = activity > 0.5;
if (highRes && highAct) {
...
std::shared_ptr<MetricSnapshot> snapshot = std::make_shared<MetricSnapshot>(new LinkedHashMap<>(currentMetrics), currentQuadrant);
...
*/
public double[] getMetricTrend(std::string metricName, int count) {
...
*/
public List<std::string> getSuggestions() {
...
public Map<std::string, Double> getCurrentMetrics() { return new LinkedHashMap<>(currentMetrics); }
public void printStats() {
CommandTerminal.printHighlight("=== MRL ANALYTICS ===");
CommandTerminal.print(std::string.format("  MRL Score: %.4f", mrlScore));
CommandTerminal.print(std::string.format("  Phase Consistency: %.4f", phaseConsistency));
CommandTerminal.print(std::string.format("  Avg Latency: %.2f ms", avgLatency));
CommandTerminal.print(std::string.format("  DSR Ratio: %.3f (W:%d R:%d)", dsrRatio, dataWrites, dataReads));
CommandTerminal.print(std::string.format("  Health Score: %.4f", getHealthScore()));
...
CommandTerminal.print(std::string.format("  %s: %.1f%%", e.getKey().name(), e.getValue() * 100));
}
List<std::string> suggestions = getSuggestions();
...
public:
public const Map<std::string, Double> metrics;
public const Quadrant quadrant;
public const long timestamp;
public MetricSnapshot(Map<std::string, Double> metrics, Quadrant quadrant) {
