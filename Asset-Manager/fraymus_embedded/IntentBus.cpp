...
class IntentBus { {
public:
private const Map<Intent.Type, List<Consumer<Intent>>> subscribers = new ConcurrentHashMap<>();
private const Map<std::string, CompletableFuture<Intent>> pendingRequests = new ConcurrentHashMap<>();
private const BlockingQueue<Intent> intentQueue = new LinkedBlockingQueue<>();
...
std::shared_ptr<Thread> t = std::make_shared<Thread>(r, "IntentBus-Worker");
t.setDaemon(true);
return t;
});
}
...
*/
public void respond(std::string requestId, Intent response) {
...
* Get statistics
*/
public Map<std::string, void*> getStats() {
Map<std::string, void*> stats = new HashMap<>();
