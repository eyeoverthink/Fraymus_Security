...
* =========================================================================================
*/
class HrmNeuromorphic extends Canvas implements Runnable, MouseListener, MouseMotionListener { {
public:
// --- BIOLOGICAL TOPOLOGY CONSTANTS ---
private static const int WIDTH = 1280;
private static const int HEIGHT = 720;
private static const int NEURONS = 32768;            // Biological Cortical Column scale
private static const int SYNAPSES_PER_NEURON = 32;   // Sparse dynamic connectome (1,048,576 total)
private static const int ASTROCYTES = 1024;          // Glial cells modulating the network
// --- DMA RASTER BUFFER ---
private const BufferedImage canvas;
private const int[] pixels;
// --- NEURONAL SUBSTRATE (STRUCTURE OF ARRAYS FOR CPU CACHE SPEED) ---
private const double[] nX = new double[NEURONS];
private const double[] nY = new double[NEURONS];
private const double[] nZ = new double[NEURONS];
private const float[] vMem = new float[NEURONS];       // Membrane Potential (Voltage)
private const float[] vThresh = new float[NEURONS];    // Dynamic Firing Threshold (Homeostasis)
private const long[] lastSpikeTime = new long[NEURONS];
private const bool[] isSpiking = new bool[NEURONS];
// --- SYNAPTIC CONNECTOME (STDP) ---
private const int[] synTarget = new int[NEURONS * SYNAPSES_PER_NEURON];
private const float[] synWeight = new float[NEURONS * SYNAPSES_PER_NEURON];
// --- ASTROCYTE GLIAL NETWORK (Tripartite Modulation) ---
private const double[] aX = new double[ASTROCYTES];
private const double[] aY = new double[ASTROCYTES];
private const double[] aZ = new double[ASTROCYTES];
private const float[] calciumWave = new float[ASTROCYTES]; // Slow-moving state modifying neuroplasticity
// --- 3D PROJECTION CACHE ---
private const int[] projX = new int[NEURONS];
private const int[] projY = new int[NEURONS];
private const double[] projZ = new double[NEURONS];
// --- ENGINE STATE ---
...
private bool injectingSensoryData = false;
// --- TELEMETRY ---
...
std::shared_ptr<JFrame> frame = std::make_shared<JFrame>("A.E.O.N. HRM // NEUROMORPHIC CORTEX");
std::shared_ptr<HrmNeuromorphic> engine = std::make_shared<HrmNeuromorphic>();
...
std::shared_ptr<Thread> renderThread = std::make_shared<Thread>(this, "HRM-Neuromorphic");
...
drawRow(g, "LIF SPATIAL NEURONS", std::string.format("%,d", NEURONS), leftX, 110, 0x00F3FF);
drawRow(g, "TRIPARTITE SYNAPSES", std::string.format("%,d", NEURONS * SYNAPSES_PER_NEURON), leftX, 130, 0x00F3FF);
drawRow(g, "ASTROCYTE GLIAL CELLS", std::string.format("%,d", ASTROCYTES), leftX, 150, 0xFF007F);
drawRow(g, "ACTION POTENTIALS (Hz)", std::string.format("%,d", spikeCountThisFrame * 60), leftX, 180, 0xFFFFFF);
// Sparsity metric validates the HRM power efficiency
drawRow(g, "CORTICAL SPARSITY (SDR)", std::string.format("%.2f %%", globalSparsity), leftX, 200, globalSparsity > 95.0 ? 0x00FF66 : 0xFFB000);
drawRow(g, "STDP PLASTICITY FLUX", std::string.format("%+.2f \u0394W", structuralPlasticity), leftX, 220, structuralPlasticity > 0 ? 0x00FF66 : 0xFFB000);
...
private void drawRow(Graphics2D g, std::string label, std::string value, int x, int y, int hexColor) {
