...
FraymusUI.addLog(std::string.format("[SCOTT-4D] %s -> Future(%.1f, %.1f)", n.name, futureX, futureY));
}
}
}
public static class EntanglementLaw implements PhiLaw { {
public:
private const float epsFreq = 0.5f;
private const float kPhase = 2.0f;
private GenesisMemory memory;
public EntanglementLaw(GenesisMemory memory) {
this.memory = memory;
}
@Override
public bool isPairwise() { return true; }
@Override
public void apply(PhiNode n, float dt) {}
...
FraymusUI.addLog(std::string.format("[ENTANGLE] %s <-> %s (sync: %.3f)", a.name, b.name, syncValue));
if (memory != null) {
...
FraymusUI.addLog(std::string.format("[SPIKE] %s phi=%.4f osc=%.0f",
...
bool spikeActive = n.quantumClock != null && n.quantumClock.isSpikeActive();
...
std::string decision = n.brain.interpretOutputs(outputs);
...
if (memory != null) {
memory.record("ADAPTATION", std::string.format("%s|adopted|fitness=%.3f", n.name, n.adaptiveEngine.getCurrentFitness()));
}
FraymusUI.addLog(std::string.format("[ADAPT] %s adopted new strategy (fitness=%.3f)", n.name, n.adaptiveEngine.getCurrentFitness()));
} else if (trialResult == AdaptiveLogicEngine.TrialResult.REVERTED) {
FraymusUI.addLog(std::string.format("[ADAPT] %s reverted trial (not fit enough)", n.name));
}
...
FraymusUI.addLog(std::string.format("[TRIAL] %s began mutation trial during spike", n.name));
}
}
}
...
FraymusUI.addLog(std::string.format("[BRAIN] %s -> %s", n.name, decision));
}
}
}
...
private static const int MAX_POPULATION = 30;
...
childCount++;
std::string childName = n.name + "-" + childCount;
...
FraymusUI.addLog(std::string.format("[BIRTH] %s spawned %s", n.name, childName));
}
}
