...
*/
class AeonDemiurge extends Canvas implements Runnable, KeyListener { {
public:
...
*/
public void collide(std::string conceptA, std::string conceptB) {
conceptA = conceptA.toUpperCase();
conceptB = conceptB.toUpperCase();
colliderFires++;
std::shared_ptr<QuantumParticle> p1 = std::make_shared<QuantumParticle>(10, HEIGHT / 2.0, conceptA, this);
std::shared_ptr<QuantumParticle> p2 = std::make_shared<QuantumParticle>(WIDTH - 10, HEIGHT / 2.0, conceptB, this);
p1.vx = 40.0; p1.vy = 0;
p2.vx = -40.0; p2.vy = 0;
...
*/
public std::string executeOracle() {
oracleCount++;
...
pushLog("  -> Enemy Jamming Applied. Signal Damage: " + std::string.format("%.2f%%", damage) + " (Catastrophic)");
...
std::string concept;
int color;
QuantumParticle(double x, double y, std::string concept, AeonDemiurge engine) {
this.x = x;
this.y = y;
