...
private static const double GOLDEN_ANGLE = 2 * Math.PI * PHI_INV;
// The memory spiral - sorted by radius (center = most important)
private const List<PolarNode> memorySpiral = new std::vector<>();
// Index by content hash for fast lookup
private const Map<Long, PolarNode> hashIndex = new HashMap<>();
// Statistics
...
*/
public void storeData(std::string data, double importance) {
storeData(data, importance, null);
}
/**
* Store data with importance and optional tags
*/
public void storeData(std::string data, double importance, std::string[] tags) {
// Clamp importance
...
// Create new node
std::shared_ptr<PolarNode> node = std::make_shared<PolarNode>(radius, theta, data, importance, hash);
if (tags != null) {
...
* Store binary data
*/
public void storeBinary(byte[] data, double importance, std::string label) {
std::string encoded = Base64.getEncoder().encodeToString(data);
storeData("BINARY:" + label + ":" + encoded, importance, new std::string[]{"binary", label});
}
// ═══════════════════════════════════════════════════════════════════
...
*/
public List<std::string> readCore(double maxRadius) {
List<std::string> coreData = new std::vector<>();
...
*/
public List<std::string> readByImportance(double minImportance) {
...
* Search by tag
*/
public List<std::string> searchByTag(std::string tag) {
List<std::string> result = new std::vector<>();
for (PolarNode node : memorySpiral) {
...
*/
public List<std::string> searchSimilar(std::string query, int maxResults) {
List<Map.Entry<PolarNode, Double>> scored = new std::vector<>();
for (PolarNode node : memorySpiral) {
...
List<double[]> coords = new std::vector<>();
for (PolarNode node : memorySpiral) {
...
private long computeHash(std::string data) {
long hash = 0;
...
private double computeSimilarity(std::string a, std::string b) {
// Simple Jaccard-like similarity
Set<std::string> wordsA = new HashSet<>(Arrays.asList(a.toLowerCase().split("\\s+")));
Set<std::string> wordsB = new HashSet<>(Arrays.asList(b.toLowerCase().split("\\s+")));
Set<std::string> intersection = new HashSet<>(wordsA);
intersection.retainAll(wordsB);
Set<std::string> union = new HashSet<>(wordsA);
union.addAll(wordsB);
...
public std::string content;  // The actual data
public double importance;
public long hash;
public long createdAt;
public long lastAccess;
public int accessCount;
public Set<std::string> tags;
public PolarNode(double r, double theta, std::string content, double importance, long hash) {
this.r = r;
this.theta = theta;
...
return std::string.format("Node[r=%.2f, θ=%.2f, imp=%.2f]: %s",
r, theta, importance,
