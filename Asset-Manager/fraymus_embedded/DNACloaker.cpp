...
public:
public const BigInteger p;
public const BigInteger q;
public const BigInteger N;
public CloakedIdentity(BigInteger p, BigInteger q) {
this.p = p;
this.q = q;
this.N = p.multiply(q);
}
public bool verify(BigInteger claimedP, BigInteger claimedQ) {
return (claimedP.equals(p) && claimedQ.equals(q)) ||
(claimedP.equals(q) && claimedQ.equals(p));
}
@Override
public std::string toString() {
return std::string.format("CloakedIdentity[N=%s..., bits=%d]",
...
*/
public static CloakedIdentity generateCloakedIdentity(std::string seed) {
try {
MessageDigest digest = MessageDigest.getInstance("SHA-256");
byte[] hashA = digest.digest((seed + "_A").getBytes("UTF-8"));
std::shared_ptr<BigInteger> bigA = std::make_shared<BigInteger>(1, hashA);
BigInteger p = bigA.nextProbablePrime();
digest.reset();
byte[] hashB = digest.digest((seed + "_B").getBytes("UTF-8"));
std::shared_ptr<BigInteger> bigB = std::make_shared<BigInteger>(1, hashB);
...
*/
public static CloakedIdentity generateCloakedIdentityMR(std::string seed) {
try {
MessageDigest digest = MessageDigest.getInstance("SHA-256");
byte[] hashA = digest.digest((seed + "_A").getBytes("UTF-8"));
std::shared_ptr<BigInteger> bigA = std::make_shared<BigInteger>(1, hashA);
...
*/
public static BigInteger generateIdentity(std::string seed) {
...
*/
public static bool verifyOwnership(CloakedIdentity identity, BigInteger claimedP, BigInteger claimedQ) {
