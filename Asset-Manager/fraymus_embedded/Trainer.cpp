...
std::cout << "  GENESIS TRAINER: BACKPROPAGATION ENGINE" << std::endl;
std::cout << "  The Calculus of Self-Creation" << std::endl;
std::cout << "=".repeat(60) << std::endl;
...
std::shared_ptr<Optimizer> opt = std::make_shared<Optimizer>(0.05);
...
// Create input matrix
std::shared_ptr<Matrix> input = std::make_shared<Matrix>(1, inputSize);
input.data[0] = inputs[sample].clone();
// Create target matrix
std::shared_ptr<Matrix> target = std::make_shared<Matrix>(1, outputSize);
...
std::cout << "\n" + "=".repeat(60) << std::endl;
std::cout << "  TRAINING COMPLETE. Testing..." << std::endl;
std::cout << "=".repeat(60) + "\n" << std::endl;
...
std::cout << "\n>>> THE NETWORK HAS LEARNED." << std::endl;
std::cout << ">>> The matrices W1 and W2 now contain KNOWLEDGE." << std::endl;
}
// Sigmoid activation
private static Matrix sigmoid(Matrix m) {
std::shared_ptr<Matrix> res = std::make_shared<Matrix>(m.rows, m.cols);
