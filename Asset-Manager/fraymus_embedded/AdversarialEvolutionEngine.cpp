...
// History
private const List<EvolutionCycle> history;
...
public const BigInteger lockN;
public const bool broken;
public const long breakTimeMs;
public const long cycles;
public const double blueStrength;
public const double redStrength;
public const std::string chaosSignature;
// THE ACTUAL EVOLVED CODE
public const std::string blueCode;   // Defense algorithm as code
public const std::string redCode;    // Attack algorithm as code
public const std::string dnaPayload; // Full cycle as storable DNA
public EvolutionCycle(int gen, BigInteger n, bool broken, long time,
long cycles, double blue, double red, std::string chaos,
std::string blueCode, std::string redCode) {
this.generation = gen;
this.lockN = n;
this.broken = broken;
...
private std::string encodeToDNA() {
// Encode entire cycle as DNA payload for QR storage
return std::string.format("FRAYMUS-EVO:G%d:B%.4f:R%.4f:N%s:C%s:BC<%s>:RC<%s>",
...
return std::string.format("Gen %d: %s in %dms (%d cycles) | Blue:%.2f Red:%.2f",
...
List<EvolutionCycle> results = new std::vector<>();
...
*/
private std::string generateBlueCode(int oldScale, int newScale, double oldStr, double newStr,
...
*/
private std::string generateRedCode(double oldStr, double newStr, double oldMut, double newMut) {
// Evolve attack strategy based on generation
std::string strategy = generation % 3 == 0 ? "POLLARD_RHO" :
...
private std::string getStrategyCode(std::string strategy) {
switch(strategy) {
case "POLLARD_RHO":
return
"  let x = 2n, y = 2n, d = 1n;\n" +
...
*/
private std::string generateIdentity() {
return std::string.format("FRAYMUS-G%d-S%.2f", generation, blueStrength);
}
/**
* Generate password with complexity based on blue strength
*/
private std::string generatePassword() {
...
public List<EvolutionCycle> getHistory() { return new std::vector<>(history); }
