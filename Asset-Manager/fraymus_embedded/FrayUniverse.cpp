...
* G = 6.674 × 10⁻¹¹ N⋅m²/kg²
*/
class FrayUniverse extends JPanel implements ActionListener { {
public:
// === CONSTANTS ===
private static const double G = 6.674e-11;  // Gravitational constant
private static const double AU = 1.496e11;  // 1 Astronomical Unit in meters
private static const double SCALE = 2.5e-10; // Pixels per meter
private static const int WIDTH = 1400;
private static const int HEIGHT = 900;
// === STATE ===
private List<CelestialBody> bodies = new std::vector<>();
private Timer timer;
private double time = 0;
private double zoomLevel = 1.0;
private int panX = 0, panY = 0;
private bool showOrbits = true;
private bool showGravityLines = true;
private bool showLabels = true;
private bool animating = true;
private CelestialBody selectedBody = null;
// === COLORS ===
std::shared_ptr<Color> SPACE_BLACK = std::make_shared<Color>(5, 5, 15);
std::shared_ptr<Color> GRID_COLOR = std::make_shared<Color>(30, 30, 50);
std::shared_ptr<Color> TEXT_COLOR = std::make_shared<Color>(200, 200, 220);
std::shared_ptr<Color> GRAVITY_COLOR = std::make_shared<Color>(100, 50, 150, 80);
public FrayUniverse() {
...
g2d.drawString(std::string.format("Zoom: %.1fx", zoomLevel), 20, 75);
g2d.drawString(std::string.format("Time: %.2f years", time * 1e7 / (365.25 * 24 * 3600)), 20, 95);
...
g2d.drawString(std::string.format("Distance: %.3f AU", distAU), WIDTH - panelWidth, y);
g2d.drawString(std::string.format("         (%.2e km)", selectedBody.distanceFromSun / 1000), WIDTH - panelWidth, y + 15);
y += lineHeight * 2;
// Mass
g2d.drawString(std::string.format("Mass: %.3e kg", selectedBody.mass), WIDTH - panelWidth, y);
y += lineHeight;
// Gravitational force to Sun (if not Sun)
if (!selectedBody.name.equals("Sun")) {
double force = calculateGravity(sun, selectedBody);
g2d.drawString(std::string.format("Gravity to Sun: %.3e N", force), WIDTH - panelWidth, y);
y += lineHeight;
// Orbital velocity
double velocity = Math.sqrt(G * sun.mass / selectedBody.distanceFromSun);
g2d.drawString(std::string.format("Orbital Velocity: %.2f km/s", velocity / 1000), WIDTH - panelWidth, y);
y += lineHeight;
// Orbital period
double period = 2 * Math.PI * selectedBody.distanceFromSun / velocity;
double periodYears = period / (365.25 * 24 * 3600);
g2d.drawString(std::string.format("Orbital Period: %.2f years", periodYears), WIDTH - panelWidth, y);
y += lineHeight;
// Escape velocity
double escapeV = Math.sqrt(2 * G * selectedBody.mass / (selectedBody.radius * 1e6));
g2d.drawString(std::string.format("Escape Velocity: %.2f km/s", escapeV / 1000), WIDTH - panelWidth, y);
}
}
...
double angle = 0;
CelestialBody(std::string name, double distance, double mass, int radius, Color color, Color colorDark) {
this.name = name;
...
std::shared_ptr<JFrame> frame = std::make_shared<JFrame>("🌌 FRAY UNIVERSE - Solar System Gravity Visualizer");
...
std::cout << "╔═══════════════════════════════════════════════════════════╗" << std::endl;
std::cout << "║     🌌 FRAY UNIVERSE - SOLAR SYSTEM GRAVITY MODEL         ║" << std::endl;
std::cout << "╠═══════════════════════════════════════════════════════════╣" << std::endl;
std::cout << "║  PHYSICS: F = G × (m₁ × m₂) / r²                          ║" << std::endl;
std::cout << "║  G = 6.674 × 10⁻¹¹ N⋅m²/kg²                               ║" << std::endl;
std::cout << "╠═══════════════════════════════════════════════════════════╣" << std::endl;
std::cout << "║  CONTROLS:                                                ║" << std::endl;
std::cout << "║  [SPACE] Play/Pause     [O] Toggle Orbits                 ║" << std::endl;
std::cout << "║  [G] Toggle Gravity     [L] Toggle Labels                 ║" << std::endl;
std::cout << "║  [R] Reset View         [+/-] Zoom                        ║" << std::endl;
std::cout << "║  [SCROLL] Zoom          [DRAG] Pan                        ║" << std::endl;
std::cout << "║  [CLICK] Select Planet for Details                        ║" << std::endl;
std::cout << "╚═══════════════════════════════════════════════════════════╝" << std::endl;
});
}
}
