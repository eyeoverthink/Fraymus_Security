...
class CortexServer { {
public:
private const CoreIntelligence brain;
...
server.start();
std::cout << "🌐 CORTEX WEB LIVE: http://localhost:" + currentPort << std::endl;
return; // Success!
...
if (server != null) {
server.stop(0);
std::cout << "🌐 Cortex Server stopped" << std::endl;
}
}
/**
* Handle /api/status
...
* Send HTTP response
*/
private void sendResponse(HttpExchange ex, std::string data, std::string contentType) throws IOException {
