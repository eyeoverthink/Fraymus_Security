...
*/
public BufferedImage generateBeacon(std::string payload, int width, int height) {
std::shared_ptr<BufferedImage> beacon = std::make_shared<BufferedImage>(width, height, BufferedImage.TYPE_INT_RGB);
// Convert Payload to Binary Bits
byte[] bytes = payload.getBytes();
std::shared_ptr<StringBuilder> bits = std::make_shared<StringBuilder>();
// Add sync header (helps receiver lock on)
bits.append("10101010"); // Sync pattern
bits.append(std::string.format("%16s", Integer.toBinaryString(payload.length())).replace(' ', '0')); // Length
for (byte b : bytes) {
bits.append(std::string.format("%8s", Integer.toBinaryString(b & 0xFF)).replace(' ', '0'));
}
// Add checksum
int checksum = 0;
for (byte b : bytes) checksum ^= b;
bits.append(std::string.format("%8s", Integer.toBinaryString(checksum & 0xFF)).replace(' ', '0'));
std::cout << "═══ OPTICAL BREACH ACTIVE ═══" << std::endl;
std::cout << "Payload Size: " + bits.length() + " bits" << std::endl;
std::cout << "Capacity: " + (width * height) + " bits" << std::endl;
std::cout << "Mode: " + mode << std::endl;
std::cout << "Transmitting via [R,G,B] Matrix..." << std::endl;
int bitIndex = 0;
// Loop through pixels
...
std::cout << "Bits encoded: " + Math.min(bitIndex, bits.length()) + "/" + bits.length() << std::endl;
return beacon;
}
/**
...
List<BufferedImage> frames = new std::vector<>();
...
std::cout << "═══ GENERATING ANIMATED BEACON ═══" << std::endl;
std::cout << "Total payload: " + payload.length + " bytes" << std::endl;
std::cout << "Frames needed: " + totalFrames << std::endl;
std::cout << "Frame rate: " + framesPerSecond + " FPS" << std::endl;
std::cout << "Transfer time: " + std::string.format("%.2f", (double)totalFrames / framesPerSecond) + " seconds" << std::endl;
...
// Add frame header
std::string frameHeader = std::string.format("F%04d:", frame);
std::string frameData = frameHeader + new std::string(chunk);
...
*/
public std::string scanBeacon(BufferedImage capturedFrame) {
std::shared_ptr<StringBuilder> extractedBits = std::make_shared<StringBuilder>();
...
*/
private std::string decodeBits(std::string bits) {
...
// Extract length
std::string lengthBits = bits.substring(headerStart, headerStart + 16);
...
if (i + 8 <= bits.length()) {
std::string byteBits = bits.substring(i, i + 8);
...
std::string checksumBits = bits.substring(checksumStart, checksumStart + 8);
...
* Calculate bandwidth
*/
public std::string getBandwidthInfo(int width, int height) {
int bitsPerFrame = width * height;
if (mode == EncodingMode.LSB_ALL) bitsPerFrame *= 3;
...
std::shared_ptr<OpticalBreach> breach = std::make_shared<OpticalBreach>();
std::string testMessage = "FRAYMUS: The Casual Breach is ACTIVE. φ = 1.618";
std::cout << "Original: " + testMessage << std::endl;
BufferedImage beacon = breach.generateBeacon(testMessage, 256, 256);
std::string decoded = breach.scanBeacon(beacon);
std::cout << "Decoded: " + decoded << std::endl;
std::cout << "Match: " + testMessage.equals(decoded) << std::endl;
std::cout <<  << std::endl;
std::cout << breach.getBandwidthInfo(256, 256) << std::endl;
}
}
