...
public:
private static const int TRIAL_DURATION = 300;
private static const int MAX_STRATEGIES = 10;
private static const double MIN_IMPROVEMENT = 0.05;
private List<StrategyGenome> provenStrategies = new std::vector<>();
...
*/
public void recordFitnessSample(float energy, bool spikeActive, int nearbyEntangled, bool reproduced) {
...
public std::string encodeStrategies() {
...
public void decodeStrategies(std::string encoded) {
...
// Getters
public bool isInTrial() { return inTrial; }
...
return std::string.format("Adaptive[trials=%d, adopted=%d, reverted=%d, strategies=%d, fitness=%.3f%s]",
