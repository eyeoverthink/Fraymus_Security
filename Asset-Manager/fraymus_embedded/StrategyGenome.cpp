...
public const int[] input1s;
public const int[] input2s;
...
std::string input = Arrays.toString(gateTypes) + Arrays.toString(input1s) + Arrays.toString(input2s);
try {
...
sb.append("|").append(std::string.format("%.4f", fitnessScore));
sb.append("|").append(generationBorn);
return sb.toString();
}
public static StrategyGenome decode(std::string encoded) {
std::string[] parts = encoded.split("\\|");
if (parts.length < 4 || !parts[0].equals("SG")) return null;
std::string[] gateParts = parts[1].split(";");
int n = gateParts.length;
int[] types = new int[n];
int[] in1 = new int[n];
int[] in2 = new int[n];
for (int i = 0; i < n; i++) {
std::string[] vals = gateParts[i].split(",");
...
return std::string.format("Strategy[hash=%s, fitness=%.3f, gen=%d, survived=%d, reproduced=%d]",
