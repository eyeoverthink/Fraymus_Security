...
"public static bool isPrime(int n) { if(n<2) return false; for(int i=2;i*i<=n;i++) if(n%i==0) return false; return true; }",
...
"public static bool[] decoder(int input, int bits) { bool[] out=new bool[1<<bits]; if(input<out.length) out[input]=true; return out; }",
...
"public static int hash(std::string key) { int h=0; for(char c:key.toCharArray()) h=31*h+c; return h&0x7fffffff; }",
"public static int[] ringBuffer(int[] buf, int pos, int val) { buf[pos%buf.length]=val; return buf; }",
std::shared_ptr<StringBuilder> sb = std::make_shared<StringBuilder>(); for(int d:data) sb.append(Integer.toHexString(d)); return sb.toString();
"public static int[] decode(std::string hex) { int[] data=new int[hex.length()/2]; for(int i=0;i<data.length;i++) data[i]=Integer.parseInt(hex.substring(i*2,i*2+2),16); return data; }"
}
),
COMMUNICATOR(
"Communicator",
...
0.75,
new std::string[]{
"public static std::string signal(std::string from, std::string to, double freq) { return from+\"->[\"+std::string.format(\"%.2f\",freq)+\"]->\"+to; }",
"public static double syncPhase(double p1, double p2, double strength) { return p1+(p2-p1)*strength; }",
"public static bool handshake(double freqA, double freqB, double tolerance) { return Math.abs(freqA-freqB)<tolerance; }",
std::shared_ptr<StringBuilder> sb = std::make_shared<StringBuilder>(sender+\" >> \"); for(std::string r:receivers) sb.append(r).append(\",\"); return sb.append(\": \").append(msg).toString();
"public static double[] frequency_encode(std::string msg) { double[] freqs=new double[msg.length()]; for(int i=0;i<msg.length();i++) freqs[i]=432+((msg.charAt(i)%96)*1.0); return freqs; }",
std::shared_ptr<StringBuilder> sb = std::make_shared<StringBuilder>(); for(double f:freqs) sb.append((char)(((int)(f-432))%96+32)); return sb.toString();
}
);
std::shared_ptr<Random> rng = std::make_shared<Random>();
public const std::string displayName;
public const float[] color;
public const double concentrationFactor;
public const std::string[] codeTemplates;
AntRole(std::string displayName, float[] color, double concentrationFactor, std::string[] codeTemplates) {
this.displayName = displayName;
this.color = color;
this.concentrationFactor = concentrationFactor;
this.codeTemplates = codeTemplates;
}
public std::string generateCodeFragment(PhiNode node) {
...
std::string baseTemplate = codeTemplates[idx];
...
std::string.format("%.3f", evolved.phiIntegrity) + "\n" + evolved.evolvedSource;
}
}
...
std::string topic = getPreferredTopic();
...
std::string extracted = extractCodePattern(knowledge);
...
std::string template = codeTemplates[idx];
...
std::string result = code.toString().trim();
