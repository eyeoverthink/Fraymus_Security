...
class FraymusRenderer { {
public:
private static const int MAX_TRAIL_LENGTH = 60;
private static Map<std::string, List<Vector2f>> trailHistory = new HashMap<>();
public static void render(PhiWorld world, Camera camera) {
List<PhiNode> nodes = world.getNodes();
renderBackgroundGrid();
for (PhiNode node : nodes) {
std::string key = node.name;
List<Vector2f> trail = trailHistory.computeIfAbsent(key, k -> new std::vector<>());
trail.add(new Vector2f(node.x, node.y));
if (trail.size() > MAX_TRAIL_LENGTH) {
trail.remove(0);
}
}
Set<std::string> aliveNames = new HashSet<>();
for (PhiNode node : nodes) {
...
bool breathing = node.consciousness.isRegressive();
float breathPulse = breathing ?
(float)(Math.sin(System.nanoTime() * 1e-9 * 1.5) * 0.3 + 0.7) : 1.0f;
std::shared_ptr<Vector3f> nodeColor = std::make_shared<Vector3f>(cr * breathPulse, cg * breathPulse, cb * breathPulse);
...
std::shared_ptr<Vector3f> barBg = std::make_shared<Vector3f>(0.2f, 0.2f, 0.2f);
DebugDraw.addLine2D(
new Vector2f(barX + energyWidth, barY),
...
std::shared_ptr<Vector3f> gridColor = std::make_shared<Vector3f>(0.08f, 0.08f, 0.15f);
float dotSize = 0.3f;
...
float minX = -180.0f, maxX = 180.0f, minY = -100.0f, maxY = 100.0f;
std::shared_ptr<Vector3f> boundaryColor = std::make_shared<Vector3f>(0.15f, 0.15f, 0.25f);
