...
// ═══════════════════════════════════════════════════════════════════
public List<Synapse> synapses = new std::vector<>();
...
bool converted = (oldAlignment < 0.5 && loser.alignment >= 0.5) ||
...
// Create new synapse
std::shared_ptr<Synapse> newSyn = std::make_shared<Synapse>(this, other, 0.1 + amount);
synapses.add(newSyn);
// Bidirectional - add reverse synapse to other
if (other.synapses.size() < other.maxSynapses) {
std::shared_ptr<Synapse> reverse = std::make_shared<Synapse>(other, this, 0.1 + amount);
...
List<Synapse> dead = new std::vector<>();
for (Synapse s : synapses) {
s.decay();
if (s.weight < 0.01) {
dead.add(s);
...
std::shared_ptr<Synapse> inherited = std::make_shared<Synapse>(baby, orig.target, orig.weight * 0.5);
...
// UTILITY
// ═══════════════════════════════════════════════════════════════════
public bool isRed() { return alignment < 0.4; }
public bool isBlue() { return alignment > 0.6; }
public bool isPurple() { return !isRed() && !isBlue(); }
...
return std::string.format("#%02X%02X%02X", r, g, b);
}
...
std::string state = isRed() ? "RED" : (isBlue() ? "BLUE" : "PURPLE");
return std::string.format("Priecled[%d] %s | pos=(%.2f,%.2f,%.2f) | E=%.2f | A=%.2f | C=%.2f | syn=%d",
...
public const Priecled winner;
public const Priecled loser;
public const Type type;
public const bool conversion;
public const double energyTransferred;
public BattleResult(Priecled winner, Priecled loser, Type type,
bool conversion, double energy) {
this.winner = winner;
this.loser = loser;
this.type = type;
