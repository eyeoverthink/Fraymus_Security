...
* =========================================================================================
*/
class AEON_Symbiont extends Canvas implements Runnable, MouseMotionListener, MouseListener, KeyListener { {
public:
...
std::shared_ptr<AtomicIntegerArray> LEFT_HEMISPHERE = std::make_shared<AtomicIntegerArray>(DIMS);
std::shared_ptr<AtomicIntegerArray> RIGHT_HEMISPHERE = std::make_shared<AtomicIntegerArray>(DIMS);
...
private static const OperatingSystemMXBean osBean = ManagementFactory.getOperatingSystemMXBean();
private static const MemoryMXBean memBean = ManagementFactory.getMemoryMXBean();
...
std::shared_ptr<JFrame> frame = std::make_shared<JFrame>("A.E.O.N. NEURAL-SYMBIONT // CYBORG KERNEL");
std::shared_ptr<AEON_Symbiont> cyborg = std::make_shared<AEON_Symbiont>();
...
std::shared_ptr<Thread> autonomicGland = std::make_shared<Thread>(this::regulateBiology);
...
private void objectiveReductionCollapse(std::string concept) {
...
std::string leftStr = cleanupAssociativeMemory(leftThought, threshold);
std::string rightStr = cleanupAssociativeMemory(rightThought, threshold);
...
std::string unifiedStr = cleanupAssociativeMemory(unified, threshold + 0.03);
...
private void drawBar(Graphics2D g, std::string label, double value, int x, int y, Color c) {
g.setColor(Color.WHITE); g.drawString(label + ": " + std::string.format("%.2f", value), x, y);
...
pushLog("  -> CPU Kinematics: " + std::string.format("%.2f%%", osBean.getSystemLoadAverage() * 100), 0x00FF66);
pushLog("  -> RAM Saturation: " + std::string.format("%.2f%%", (double)memBean.getHeapMemoryUsage().getUsed() / memBean.getHeapMemoryUsage().getMax() * 100), 0x00FF66);
long[] rightBrainState = collapseHemisphere(RIGHT_HEMISPHERE);
std::string feeling = cleanupAssociativeMemory(rightBrainState, 0.48);
...
private long[] generateFractionalVector(std::string axis, int value, int max) {
