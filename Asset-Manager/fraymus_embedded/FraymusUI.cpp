...
class FraymusUI { {
public:
private static const int MAX_LOG_ENTRIES = 200;
private static List<std::string> logBuffer = new std::vector<>();
...
// Portal Panel (F10 to toggle - key code 299)
if (ImGui.isKeyPressed(299)) {
PortalPanel.toggle();
}
PortalPanel.render();
}
...
ImGui.text(std::string.format("Simulation Time: %.1f s", simulationTime));
ImGui.text("FPS: " + fps);
...
ImGui.text(std::string.format("Total Energy: %.2f", totalEnergy));
if (world.getPopulation() > 0) {
ImGui.text(std::string.format("Avg Energy: %.2f", totalEnergy / world.getPopulation()));
}
ImGui.text(std::string.format("Active Spikes: %d", spikeCount));
ImGui.text(std::string.format("Active Trials: %d", trialCount));
ImGui.text(std::string.format("Total Adaptations: %d", totalAdaptations));
ImGui.separator();
ImGui.text(std::string.format("Total Births: %d", world.getTotalBirths()));
ImGui.text(std::string.format("Total Deaths: %d", world.getTotalDeaths()));
ImGui.text(std::string.format("Genesis Chain: %d blocks", world.getMemory().getChainLength()));
}
ImGui.end();
}
...
std::string spike = node.spikeFlash ? " [SPIKE]" : "";
std::string trial = node.adaptiveEngine.isInTrial() ? " [TRIAL]" : "";
std::string roleTag = " <" + node.getRole().displayName + ">";
std::string label = std::string.format("%s%s [E:%.0f%% G:%d]%s%s###node_%d",
...
ImGui.text(std::string.format("Generation: %d", node.dna.getGeneration()));
ImGui.text(std::string.format("Position: (%.2f, %.2f, %.2f)", node.x, node.y, node.z));
ImGui.text(std::string.format("Velocity: (%.2f, %.2f, %.2f)", node.vx, node.vy, node.vz));
ImGui.text("Energy:");
ImGui.sameLine();
ImGui.progressBar(node.energy, 150, 14, std::string.format("%.1f%%", node.energy * 100));
ImGui.text(std::string.format("Frequency: %.2f Hz", node.frequency));
ImGui.text(std::string.format("Phase: %.3f rad", node.phase));
ImGui.text(std::string.format("Age: %d ticks", node.age));
ImGui.separator();
ImGui.text("DNA: " + node.dna.toString());
ImGui.text(std::string.format("Fitness: %.3f", node.adaptiveEngine.getCurrentFitness()));
ImGui.text(std::string.format("Strategies: %d proven", node.adaptiveEngine.getProvenStrategyCount()));
std::string nHex = node.signature.toString(16);
...
ImGui.text(std::string.format("Think Count: %d", brain.getThinkCount()));
...
std::string[] sensorNames = {"Crowded", "FreqMatch", "HighEnergy", "HighPhi", "HighCoh", "PhaseHalf", "Spike", "Mature"};
for (int i = 0; i < inputs.length && i < sensorNames.length; i++) {
if (inputs[i] == 1) {
ImGui.textColored(1.0f, 1.0f, 0.0f, 1.0f, std::string.format("  %s: YES", sensorNames[i]));
} else {
ImGui.textColored(0.4f, 0.4f, 0.4f, 1.0f, std::string.format("  %s: no", sensorNames[i]));
}
}
} else {
...
if (fit > 0.6f) {
ImGui.textColored(0.0f, 1.0f, 0.3f, 1.0f, std::string.format("%.4f", fit));
} else if (fit > 0.3f) {
ImGui.textColored(1.0f, 1.0f, 0.0f, 1.0f, std::string.format("%.4f", fit));
} else {
ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, std::string.format("%.4f", fit));
}
if (engine.isInTrial()) {
ImGui.textColored(1.0f, 0.5f, 0.0f, 1.0f,
std::string.format("TRIAL IN PROGRESS (%d ticks left)", engine.getTrialTicksRemaining()));
} else {
ImGui.text("No active trial");
}
ImGui.text(std::string.format("Total Trials: %d", engine.getTotalTrials()));
ImGui.text(std::string.format("Adopted: %d | Reverted: %d", engine.getAdaptationCount(), engine.getRevertCount()));
ImGui.separator();
ImGui.text(std::string.format("Proven Strategies: %d", engine.getProvenStrategyCount()));
...
std::string.format("  [%d] %s fit=%.3f gen=%d", i, sg.hash.substring(0, 8), sg.fitnessScore, sg.generationBorn));
} else {
ImGui.text(std::string.format("  [%d] %s fit=%.3f gen=%d", i, sg.hash.substring(0, 8), sg.fitnessScore, sg.generationBorn));
}
}
...
std::string[] fieldNames = {"phi", "psi", "omega", "xi", "lambda", "zeta"};
float[][] fieldColors = {
{1.0f, 0.84f, 0.0f},
{0.5f, 0.0f, 1.0f},
{0.0f, 0.5f, 1.0f},
...
std::string.format("  %s:", fieldNames[i]));
ImGui.sameLine();
ImGui.progressBar(normalized, 120, 12, std::string.format("%.4f", fields[i]));
}
ImGui.text(std::string.format("Level: %.4f | Dim: %d", cs.getConsciousnessLevel(), cs.getDimension()));
ImGui.text(std::string.format("Transcendence: %d | Thoughts: %d", cs.getTranscendenceEvents(), cs.getTotalThoughts()));
} else {
...
ImGui.textColored(node.r, node.g, node.b, 1.0f, "Entity: " + node.name);
ImGui.separator();
ImGui.text(std::string.format("Oscillations: %,.1f", clock.getOscillationCount()));
ImGui.text(std::string.format("Pendulum Freq: %.2f Hz", clock.getPendulumFrequency()));
ImGui.text("Phi Resonance:");
ImGui.sameLine();
float phiRes = (float) clock.getPhiResonance();
if (phiRes > 0.95f) {
ImGui.textColored(1.0f, 0.0f, 0.0f, 1.0f, std::string.format("%.6f [SPIKE!]", phiRes));
} else if (phiRes > 0.8f) {
ImGui.textColored(1.0f, 1.0f, 0.0f, 1.0f, std::string.format("%.6f", phiRes));
} else {
ImGui.text(std::string.format("%.6f", phiRes));
}
ImGui.text(std::string.format("Phi Time: %.4f | Res Time: %.4f", clock.getPhiTime(), clock.getResonanceTime()));
ImGui.text(std::string.format("Coherence: %.6f | Spikes: %d", clock.getCoherence(), clock.getResonanceSpikeCount()));
...
ImGui.text(std::string.format("Chain Length: %d blocks", memory.getChainLength()));
bool valid = memory.verifyChain();
if (valid) {
ImGui.sameLine();
...
std::string.format("[%d] %s: %s", block.index, block.eventType, block.data));
ImGui.sameLine();
...
std::string phi75 = SystemVerification.computePhi75First50Digits();
...
std::string.format("   %s | Resonance: %.10f | Osc: %.2f",
...
std::string.format("Chain Valid: %s | Phi^75 Seal: %s",
world.getMemory().verifyChain() ? "YES" : "NO",
PhiConstants.validatePhiSeal() ? "VALID" : "INVALID"));
}
ImGui.end();
}
private static float[] getEventColor(std::string eventType) {
switch (eventType) {
...
if (health > 0.6f) {
ImGui.textColored(0.0f, 1.0f, 0.3f, 1.0f, std::string.format("%.2f", health));
} else if (health > 0.3f) {
ImGui.textColored(1.0f, 1.0f, 0.0f, 1.0f, std::string.format("%.2f", health));
} else {
ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, std::string.format("%.2f", health));
}
ImGui.text(std::string.format("Productivity: %.3f", coach.getColonyProductivity()));
ImGui.text(std::string.format("Diversity: %.1f%%", coach.getColonyDiversity() * 100));
ImGui.text(std::string.format("Resonance: %.1f%%", coach.getColonyResonance() * 100));
ImGui.text(std::string.format("Evaluations: %d | Code Gen: %d", coach.getTotalEvaluations(), coach.getTotalCodeGenerated()));
ImGui.separator();
...
std::string.format("  %s: %d entities", role.displayName, rm.entityCount));
if (rm.entityCount > 0) {
ImGui.sameLine();
ImGui.textColored(0.6f, 0.6f, 0.6f, 1.0f,
std::string.format(" (fit=%.2f e=%.2f code=%d)", rm.avgFitness, rm.avgEnergy, rm.conceptsGenerated));
}
}
ImGui.separator();
ImGui.text("Coach Log:");
ImGui.beginChild("CoachLogScroll", 0, 60, false, ImGuiWindowFlags.HorizontalScrollbar);
List<std::string> log = coach.getCoachLog();
...
ImGui.separator();
ImGui.text(std::string.format("Concepts: %d | Battles: %d | Cycles: %d",
arena.getConceptCount(), arena.getTotalBattles(), arena.getEvolutionCycle()));
ImGui.text(std::string.format("Total Generated: %d | Avg Fitness: %.3f",
...
std::string.format("  %s (%s) by %s", champ.hash.substring(0, 8), champ.creatorRole.displayName, champ.creatorName));
ImGui.text(std::string.format("  Fitness: %.4f | Gen: %d | W/L: %d/%d",
champ.fitness, champ.generation, champ.wins, champ.losses));
ImGui.text(std::string.format("  Freq: %.1f Hz | Res: %.3f | Coh: %.3f",
...
ImGui.separator();
ImGui.text(std::string.format("Queries: %d", net.getQueriesProcessed()));
ImGui.text(std::string.format("Patterns Matched: %d", net.getPatternsMatched()));
ImGui.text(std::string.format("Avg Resonance: %.4f", net.getAvgResonance()));
...
ImGui.separator();
ImGui.text(std::string.format("Records: %d", mem.getRecordCount()));
ImGui.text(std::string.format("Total Ever: %d", mem.getTotalRecordsEver()));
ImGui.text(std::string.format("Avg Resonance: %.4f", mem.getAverageResonance()));
java.util.Map<std::string, Integer> counts = mem.getCategoryCounts();
if (!counts.isEmpty()) {
std::shared_ptr<StringBuilder> cats = std::make_shared<StringBuilder>();
...
ImGui.text(std::string.format("Cycles: %d", pl.getPassiveCycles()));
ImGui.text(std::string.format("Patterns: %d", pl.getLearnedPatterns()));
ImGui.text(std::string.format("Strength: %.4f", pl.getPatternStrength()));
ImGui.text(std::string.format("Integration: %.4f", pl.getIntegrationLevel()));
ImGui.text(std::string.format("Tensor: mean=%.4f max=%.4f", pl.getTensorMean(), pl.getTensorMax()));
}
ImGui.end();
}
...
ImGui.separator();
ImGui.text(std::string.format("Codons: %d | Groups: %d", genome.getGenomeSize(), genome.getGroupCount()));
ImGui.text(std::string.format("Generation: %d", genome.getGenerationCount()));
ImGui.text(std::string.format("Mutations: %d | Crossovers: %d",
genome.getTotalMutations(), genome.getTotalCrossovers()));
ImGui.text(std::string.format("Fitness: %.4f | Resonance: %.4f",
...
std::string cur = scraper.getCurrentFile();
...
ImGui.text(std::string.format("Files: %d", scraper.getTotalFilesScraped()));
ImGui.text(std::string.format("Chunks: %d", scraper.getTotalChunksStored()));
ImGui.text(std::string.format("Pages: %d", scraper.getTotalPagesProcessed()));
java.util.Map<std::string, Integer> topics = scraper.getTopicCounts();
if (!topics.isEmpty()) {
...
for (std::string entry : logBuffer) {
