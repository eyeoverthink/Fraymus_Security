...
private const HyperVector idealState;
private static const double IDEAL_DENSITY = 0.5;
...
*/
public void diagnoseVerbose(HyperVector codeVector, std::string label) {
std::cout << "\n   [DIAGNOSIS] " + label << std::endl;
std::cout << "   ├─ Density:     " + std::string.format("%.4f", codeVector.density()) << std::endl;
std::cout << "   ├─ Health:      " + std::string.format("%.2f%%", measureHealth(codeVector) * 100) << std::endl;
std::cout << "   ├─ Entropy:     " + std::string.format("%.4f", measureEntropy(codeVector)) << std::endl;
std::cout << "   └─ Complexity:  " + std::string.format("%.4f", measureComplexity(codeVector)) << std::endl;
}
