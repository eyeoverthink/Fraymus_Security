...
*/
class OrreryStandalone extends JPanel implements ActionListener { {
public:
// ═══════════════════════════════════════════════════════════════════
...
private List<Point2D.Float> trails = new std::vector<>();
private List<Integer> trailColors = new std::vector<>();
...
// COLORS
// ═══════════════════════════════════════════════════════════════════
std::shared_ptr<Color> SPACE = std::make_shared<Color>(15, 15, 25);
std::shared_ptr<Color> SUN_COLOR = std::make_shared<Color>(255, 200, 50);
std::shared_ptr<Color> MERCURY_COLOR = std::make_shared<Color>(180, 180, 180);
std::shared_ptr<Color> VENUS_COLOR = std::make_shared<Color>(230, 180, 100);
std::shared_ptr<Color> EARTH_COLOR = std::make_shared<Color>(50, 100, 255);
std::shared_ptr<Color> MARS_COLOR = std::make_shared<Color>(200, 80, 50);
std::shared_ptr<Color> JUPITER_COLOR = std::make_shared<Color>(200, 150, 100);
...
// EARTH (Reference)
std::shared_ptr<Planet> earth = std::make_shared<Planet>("🌍 Earth", 10, cx + 200, cy, 0, 1.8f, EARTH_COLOR, false);
earthRef = earth;
planets.add(earth);
// MARS
...
std::cout << "╔═══════════════════════════════════════════════════════════╗" << std::endl;
std::cout << "║      🌌 FRAYMUS ORRERY - STANDALONE VERSION               ║" << std::endl;
std::cout << "╠═══════════════════════════════════════════════════════════╣" << std::endl;
std::cout << "║  PHYSICS: F = G × (m₁ × m₂) / r²                          ║" << std::endl;
std::cout << "║  ENGINE:  Time-Step Verlet Integration                    ║" << std::endl;
std::cout << "╠═══════════════════════════════════════════════════════════╣" << std::endl;
std::cout << "║  CONTROLS:                                                ║" << std::endl;
std::cout << "║  [SPACE] Pause/Resume    [T] Toggle Trails                ║" << std::endl;
std::cout << "║  [F] Toggle Force Vectors    [R] Reset                    ║" << std::endl;
std::cout << "║  [+/-] Time Scale    [CLICK] Add Moon                     ║" << std::endl;
std::cout << "╚═══════════════════════════════════════════════════════════╝" << std::endl;
}
@Override
...
g2d.drawString(std::string.format("%.0f Mm", dist), midX, midY - 8);
}
}
...
g2d.setColor(new Color(200, 255, 200));
g2d.drawString(std::string.format("%8.1f Mm", dist), 130, y);
g2d.setColor(new Color(255, 200, 100));
g2d.drawString(std::string.format("%.4f", force), 220, y);
y += 20;
}
// Formula
g2d.setColor(new Color(100, 100, 100));
...
float radius;
bool isSun;
Planet(std::string n, float m, float x, float y, float vx, float vy, Color c, bool sun) {
this.name = n;
this.mass = m;
this.x = x;
this.y = y;
...
std::shared_ptr<JFrame> frame = std::make_shared<JFrame>("🌌 FRAYMUS ORRERY - Gravitational Simulator");
