...
public:
private const Map<std::string, HyperVector> intentVectors = new HashMap<>();
private const Map<std::string, Consumer<std::string>> actions = new HashMap<>();
/**
...
*/
public void register(std::string name, HyperVector vector, Consumer<std::string> action) {
...
*/
public Consumer<std::string> resolve(HyperVector input, double threshold) {
std::string bestMatch = null;
double maxRes = -1.0;
for (Map.Entry<std::string, HyperVector> entry : intentVectors.entrySet()) {
...
std::cout << "   [ RESONANCE: " + std::string.format("%.4f", maxRes) + " | MATCH: " + bestMatch + " ]" << std::endl;
