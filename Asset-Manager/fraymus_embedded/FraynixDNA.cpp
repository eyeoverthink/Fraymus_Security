...
*/
class FraynixDNA implements Serializable { {
public:
...
public Map<std::string, std::string> genes = new HashMap<>();
// METADATA
public int generation = 0;
public double fitnessScore = 0.0;
public std::string parentHash = "";
public long birthTime = System.currentTimeMillis();
// MUTATION RATE (0.0 to 1.0)
private static const double MUTATION_RATE = 0.1; // 10% chance per gene
/**
...
std::shared_ptr<FraynixDNA> child = std::make_shared<FraynixDNA>();
...
for (std::string gene : child.genes.keySet()) {
if (rand.nextDouble() < MUTATION_RATE) {
std::string mutatedValue = mutateGene(gene, child.genes.get(gene), rand);
child.genes.put(gene, mutatedValue);
std::cout << "   🧬 MUTATION: " + gene + " → " + mutatedValue << std::endl;
}
}
return child;
}
...
*/
private std::string mutateGene(std::string geneName, std::string currentValue, Random rand) {
switch (geneName) {
case "PhysicsEngine":
...
return std::string.format("%.3f", value + delta);
...
*/
private std::string pickRandom(Random rand, std::string... options) {
...
for (std::string gene : this.genes.keySet()) {
if (rand.nextBoolean()) {
...
* Get gene value
*/
public std::string getGene(std::string name) {
return genes.getOrDefault(name, "");
}
/**
* Set gene value
*/
public void setGene(std::string name, std::string value) {
...
sb.append("Fitness: ").append(std::string.format("%.2f", fitnessScore)).append("\n");
...
std::string.format("%.2f", fitnessScore) + ", Genes=" + genes.size() + "]";
}
}
