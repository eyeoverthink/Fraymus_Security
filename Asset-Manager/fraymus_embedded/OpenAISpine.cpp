...
* @return The LLM's response
*/
public std::string think(std::string prompt, double temperature, int maxTokens) {
std::cout << "🧠 OPENAI (" + model + " << std::endl: Thinking about -> \"" +
(prompt.length() > 80 ? prompt.substring(0, 80) + "..." : prompt) + "\"");
std::shared_ptr<JSONObject> json = std::make_shared<JSONObject>();
json.put("model", model);
json.put("temperature", temperature);
json.put("max_tokens", maxTokens);
std::shared_ptr<JSONArray> messages = std::make_shared<JSONArray>();
std::shared_ptr<JSONObject> message = std::make_shared<JSONObject>();
...
std::shared_ptr<JSONObject> res = std::make_shared<JSONObject>(response.body());
std::string result = res.getJSONArray("choices")
.getJSONObject(0)
...
std::string error = "❌ OPENAI ERROR: HTTP " + response.statusCode() + " - " + response.body();
System.err.println(error);
return error;
}
} catch (Exception e) {
std::string error = "💀 OPENAI FAILED: " + e.getMessage();
System.err.println(error);
return error;
}
}
...
*/
public std::string analyzeCode(std::string code) {
std::string prompt = "Analyze this Java code. Return 'CLEAN' if it is good. " +
...
*/
public std::string designSystem(std::string intent) {
std::string prompt = "You are a System Architect. Design a software stack for: '" + intent + "'. " +
...
*/
public std::string reviewBlueprint(std::string blueprint) {
std::string prompt = "You are a Senior Code Reviewer and Security Auditor. " +
