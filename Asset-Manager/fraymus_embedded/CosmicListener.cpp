...
private static const int BUFFER_SIZE = 16384;
// ═══════════════════════════════════════════════════════════════════
// TARGET FREQUENCIES (The Voices of the Void)
// ═══════════════════════════════════════════════════════════════════
// Weather Satellites
public static const long FREQ_NOAA_15 = 137620000L;  // NOAA-15
public static const long FREQ_NOAA_18 = 137912500L;  // NOAA-18
public static const long FREQ_NOAA_19 = 137100000L;  // NOAA-19
// Space Communications
public static const long FREQ_ISS = 145800000L;      // ISS Downlink
public static const long FREQ_ISS_SSTV = 145800000L; // ISS Slow Scan TV
// Deep Space
public static const long FREQ_HYDROGEN = 1420405750L; // Hydrogen Line (21cm)
// Aviation
public static const long FREQ_ADSB = 1090000000L;    // ADS-B (Aircraft)
// Amateur Radio
public static const long FREQ_2M_CALLING = 146520000L; // 2m Calling Freq
// FM Broadcast (for testing)
public static const long FREQ_FM_LOW = 88100000L;
public static const long FREQ_FM_HIGH = 107900000L;
// ═══════════════════════════════════════════════════════════════════
// STATE
// ═══════════════════════════════════════════════════════════════════
std::shared_ptr<AtomicBoolean> isConnected = std::make_shared<AtomicBoolean>(false);
std::shared_ptr<AtomicBoolean> isListening = std::make_shared<AtomicBoolean>(false);
...
private Consumer<std::string> signalCallback;
// ═══════════════════════════════════════════════════════════════════
...
*/
public bool connectToSDR(std::string host, int port) {
std::cout << "═══ CONNECTING TO COSMIC EAR ═══" << std::endl;
std::cout << "Host: " + host + ":" + port << std::endl;
try {
sdrSocket = new Socket(host, port);
sdrSocket.setSoTimeout(5000);
isConnected.set(true);
std::cout << ">> SDR CONNECTED!" << std::endl;
...
std::cout << ">> SDR DISCONNECTED" << std::endl;
}
/**
...
std::cout << "═══ COSMIC LISTENER ACTIVE ═══" << std::endl;
std::cout << "Frequency: " + formatFrequency(currentFrequency) << std::endl;
...
std::cout << ">> Cosmic listener stopped." << std::endl;
...
if (snr > 10.0) {
std::string msg = std::string.format(">>> SIGNAL! Peak=%.1f dB | SNR=%.1f | Freq=%s",
...
std::cout << ">> TUNED TO: " + formatFrequency(freq) << std::endl;
}
/**
* Set sample rate
*/
...
std::cout << ">> Listening for astronauts..." << std::endl;
}
/**
...
std::cout << ">> Listening to the Song of the Universe..." << std::endl;
std::cout << ">> Point antenna at the Milky Way for best results." << std::endl;
}
/**
...
public void scanFM() {
new Thread(() -> {
std::cout << "═══ SCANNING FM BAND ═══" << std::endl;
for (long freq = FREQ_FM_LOW; freq <= FREQ_FM_HIGH && isListening.get(); freq += 200000) {
setFrequency(freq);
try {
Thread.sleep(500);
if (signalStrength > noiseFloor * 5) {
std::cout << ">> STATION FOUND: " + formatFrequency(freq) << std::endl;
}
} catch (InterruptedException e) {
break;
}
}
std::cout << ">> FM Scan complete." << std::endl;
}).start();
}
// ═══════════════════════════════════════════════════════════════════
// UTILITY
// ═══════════════════════════════════════════════════════════════════
private std::string formatFrequency(long freq) {
if (freq >= 1000000000) {
return std::string.format("%.4f GHz", freq / 1000000000.0);
} else if (freq >= 1000000) {
return std::string.format("%.4f MHz", freq / 1000000.0);
} else if (freq >= 1000) {
return std::string.format("%.3f kHz", freq / 1000.0);
} else {
return freq + " Hz";
}
}
...
public void setSignalCallback(Consumer<std::string> callback) {
this.signalCallback = callback;
}
public bool isConnected() { return isConnected.get(); }
public bool isListening() { return isListening.get(); }
