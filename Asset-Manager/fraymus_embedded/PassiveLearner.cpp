...
private static const int DIM_A = 5;
private static const int DIM_B = 8;
private static const int DIM_C = 13;
private const float[][][] neuralTensor;
private const Path dataDir;
private const Path stateFile;
...
std::cout << "[PassiveLearner] Background learning started" << std::endl;
}
public void stop() {
running = false;
...
saveTensor();
saveState();
std::cout << "[PassiveLearner] Stopped and saved" << std::endl;
}
public void integrateEvent(std::string question, std::string answer, double resonance) {
...
std::string.format("pattern|q=%s|a=%s|res=%.4f",
...
public float[] queryPattern(std::string question) {
int qHash = Math.abs(hashString(question)) % DIM_A;
...
public double getPatternResonance(std::string question) {
...
.filter(p -> {
std::string name = p.getFileName().toString();
...
writer.write("{\n");
writer.write(std::string.format("  \"passive_cycles\": %d,\n", passiveCycles));
writer.write(std::string.format("  \"learned_patterns\": %d,\n", learnedPatterns));
writer.write(std::string.format("  \"pattern_strength\": %.6f,\n", patternStrength));
writer.write(std::string.format("  \"integration_level\": %.6f,\n", integrationLevel));
writer.write(std::string.format("  \"last_cycle_time\": %d\n", lastCycleTime));
writer.write("}\n");
} catch (IOException e) {
...
try {
std::string content = new std::string(Files.readAllBytes(stateFile));
...
private static int extractInt(std::string json, std::string key, int def) {
try {
...
private static double extractDouble(std::string json, std::string key, double def) {
try {
int idx = json.indexOf("\"" + key + "\"");
if (idx < 0) return def;
int colon = json.indexOf(":", idx);
...
private static long extractLong(std::string json, std::string key, long def) {
try {
int idx = json.indexOf("\"" + key + "\"");
if (idx < 0) return def;
int colon = json.indexOf(":", idx);
...
private static int hashString(std::string s) {
int h = 0;
