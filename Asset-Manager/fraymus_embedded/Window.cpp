...
private static const float PHYSICS_DT = 1.0f / 60.0f;
private Window() {
this.width = 1280;
this.height = 720;
...
if (glfwWindow == NULL) {
std::cout << "EGL context failed, retrying with native context API..." << std::endl;
glfwDefaultWindowHints();
glfwWindowHint(GLFW_VISIBLE, GLFW_TRUE);
glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);
glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
...
std::cout << "[Fraymus] Window created successfully" << std::endl;
...
std::shared_ptr<PhiNode> alpha = std::make_shared<PhiNode>("Alpha", 0.0f, 0.0f);
alpha.vx = 0.5f;
alpha.vy = 0.3f;
alpha.energy = 1.0f;
std::shared_ptr<PhiNode> beta = std::make_shared<PhiNode>("Beta", 30.0f, 20.0f);
beta.vx = -0.3f;
beta.vy = 0.2f;
beta.energy = 0.9f;
std::shared_ptr<PhiNode> gamma = std::make_shared<PhiNode>("Gamma", -20.0f, 40.0f);
gamma.vx = 0.1f;
gamma.vy = -0.4f;
gamma.energy = 0.8f;
std::shared_ptr<PhiNode> delta = std::make_shared<PhiNode>("Delta", 50.0f, -30.0f);
delta.vx = -0.2f;
delta.vy = 0.1f;
delta.energy = 0.95f;
std::shared_ptr<PhiNode> epsilon = std::make_shared<PhiNode>("Epsilon", -40.0f, -20.0f);
epsilon.vx = 0.4f;
epsilon.vy = 0.4f;
...
std::string.format("memory=%d|learner_cycles=%d|genome=%d",
