...
// Audio configuration
private static const float SAMPLE_RATE = 44100f;
private static const int FREQ_HIGH = 20000;     // Bit '1'
private static const int FREQ_LOW = 19000;      // Bit '0'
private static const int FREQ_SYNC = 18000;     // Sync pulse
private static const int BIT_DURATION_MS = 100; // Speed of transmission
// Phi-resonant frequencies (alternative mode)
private static const int FREQ_PHI_1 = (int)(432 * PHI * PHI * PHI); // ~1829 Hz
private static const int FREQ_PHI_0 = (int)(432 * PHI * PHI);       // ~1130 Hz
// State
...
void onSignalDetected(std::string data, double strength);
...
*/
public void broadcast(std::string data) throws LineUnavailableException {
if (isBroadcasting.get()) {
std::cout << "Already broadcasting!" << std::endl;
return;
}
isBroadcasting.set(true);
std::cout << "═══ SONIC BROADCAST INITIATED ═══" << std::endl;
std::cout << "Data: [" + data + "]" << std::endl;
std::cout << "Freq HIGH: " + FREQ_HIGH + " Hz" << std::endl;
std::cout << "Freq LOW: " + FREQ_LOW + " Hz" << std::endl;
std::cout << "Bit Duration: " + BIT_DURATION_MS + " ms" << std::endl;
try {
...
std::string binary = toBinary(data);
std::cout << ">> Binary: " + binary.substring(0, Math.min(32, binary.length())) + "..." << std::endl;
std::cout << ">> Total bits: " + binary.length() << std::endl;
int bitCount = 0;
...
speakers.close();
std::cout <<  << std::endl;
std::cout << ">> TRANSMISSION COMPLETE. " + bitCount + " bits sent." << std::endl;
} catch (Exception e) {
...
*/
public void broadcastPhiMode(std::string data) throws LineUnavailableException {
std::cout << "═══ PHI-RESONANT SONIC BROADCAST ═══" << std::endl;
std::cout << "Using 432Hz-based φ frequencies" << std::endl;
std::cout << "Freq '1': " + FREQ_PHI_1 + " Hz" << std::endl;
std::cout << "Freq '0': " + FREQ_PHI_0 + " Hz" << std::endl;
SourceDataLine speakers = getSpeakers();
speakers.start();
std::string binary = toBinary(data);
for (char bit : binary.toCharArray()) {
...
speakers.drain();
speakers.close();
std::cout << ">> PHI TRANSMISSION COMPLETE." << std::endl;
}
// ═══════════════════════════════════════════════════════════════════
...
std::cout << "═══ SONIC LISTENER ACTIVATED ═══" << std::endl;
std::cout << "Monitoring frequencies: " + FREQ_LOW + " - " + FREQ_HIGH + " Hz" << std::endl;
...
std::cout << ">> Sonic listener stopped." << std::endl;
}
/**
...
std::cout << "!!! SYNC SIGNAL DETECTED | Strength: " + std::string.format("%.3f", magSync) << std::endl;
...
std::shared_ptr<AudioFormat> format = std::make_shared<AudioFormat>(SAMPLE_RATE, 8, 1, true, true);
...
private std::string toBinary(std::string text) {
std::shared_ptr<StringBuilder> b = std::make_shared<StringBuilder>();
for (char c : text.toCharArray()) {
b.append(std::string.format("%8s", Integer.toBinaryString(c)).replace(' ', '0'));
}
return b.toString();
}
...
* Get bandwidth info
*/
public std::string getBandwidthInfo() {
