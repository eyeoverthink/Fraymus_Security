...
*/
class RigorousCancerEngine implements Engine { {
public:
@Override
...
List<Entity> allEntities = new std::vector<>();
...
List<Entity> cells = new std::vector<>();
...
std::shared_ptr<Entity> cell = std::make_shared<Entity>("CANCER_" + i, "CANCER", x, y, z, 98);
...
std::shared_ptr<Entity> cell = std::make_shared<Entity>("HEALTHY_" + i, "HEALTHY", x, y, z, 75);
cells.add(cell);
}
return cells;
}
...
std::shared_ptr<Entity> cell = std::make_shared<Entity>("IMMUNE_" + i, "IMMUNE", x, y, z, 85);
cells.add(cell);
}
return cells;
}
private List<Entity> createDrugs(RunContext ctx, int count) {
List<Entity> drugs = new std::vector<>();
std::string[] types = {"TARGETED_THERAPY", "IMMUNOTHERAPY", "CHEMOTHERAPY"};
