...
public void* soul;           // The data content
public std::string name;           // Human-readable label
...
public std::string entanglementId; // Shared with entangled partner
...
public bool isBlackHole;   // Collapsed star (absorbs others)
public CosmicNode(void* data, int dimensions) {
this(data, data.toString().substring(0, Math.min(20, data.toString().length())), dimensions);
}
public CosmicNode(void* data, std::string name, int dimensions) {
...
std::string key = UUID.randomUUID().toString().substring(0, 8);
...
std::shared_ptr<Wormhole> wh = std::make_shared<Wormhole>(this, destination);
...
// Absorb data (memory)
if (this.soul instanceof std::string && other.soul instanceof std::string) {
...
std::string type = isBlackHole ? "◉" : "★";
return std::string.format("%s[%s] M=%.2f T=%dK %s",
