...
private static const int EVALUATION_INTERVAL = 180;
private static const int CODE_GENERATION_INTERVAL = 300;
private const ConceptArena arena;
private const GenesisMemory memory;
...
private std::string lastCoachingAction = "Observing";
private const List<std::string> coachLog = new std::vector<>();
private static const int MAX_COACH_LOG = 50;
private const Map<AntRole, RoleMetrics> roleMetrics = new HashMap<>();
...
return std::string.format("n=%d fit=%.3f e=%.2f code=%d wins=%d prod=%.2f",
...
addCoachLog("ALERT: Colony health low (" + std::string.format("%.2f", colonyHealth) + ")");
if (memory != null) {
memory.record("COLONY_EVENT", "coach|health_alert|" + std::string.format("%.3f", colonyHealth));
}
...
bool spikeActive = node.quantumClock != null && node.quantumClock.isSpikeActive();
bool highFitness = node.adaptiveEngine.getCurrentFitness() > 0.4;
...
FraymusUI.addLog(std::string.format("[CODE] %s (%s) generated concept %s fit=%.3f",
...
std::string.format("code_gen|%s|%s|fit=%.3f", node.name, role.name(), concept.fitness));
}
}
}
}
...
private void addCoachLog(std::string msg) {
coachLog.add(std::string.format("[E%d] %s", totalEvaluations, msg));
...
public std::string getLastCoachingAction() { return lastCoachingAction; }
public List<std::string> getCoachLog() { return coachLog; }
