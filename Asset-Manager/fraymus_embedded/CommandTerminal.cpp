...
public:
private static const int MAX_OUTPUT_LINES = 500;
std::shared_ptr<ImString> inputBuffer = std::make_shared<ImString>(256);
private static const List<TerminalLine> outputLines = new std::vector<>();
private static bool scrollToBottom = true;
private static bool focusInput = true;
private static const List<std::string> commandHistory = new std::vector<>();
...
public TerminalLine(std::string text, float r, float g, float b) {
this.text = text;
this.r = r;
this.g = g;
this.b = b;
...
public static void print(std::string text) {
printColored(text, 0.8f, 0.8f, 0.8f);
}
public static void printColored(std::string text, float r, float g, float b) {
...
public static void printSuccess(std::string text) {
printColored(text, 0.3f, 1.0f, 0.3f);
}
public static void printError(std::string text) {
printColored(text, 1.0f, 0.3f, 0.3f);
}
public static void printInfo(std::string text) {
printColored(text, 0.5f, 0.8f, 1.0f);
}
public static void printHighlight(std::string text) {
...
synchronized (outputLines) {
snapshot = new std::vector<>(outputLines);
}
...
std::string cmd = inputBuffer.get().trim();
if (!cmd.isEmpty()) {
...
private static void executeCommand(std::string cmd) {
...
print(std::string.format("  Population: %d entities", world.getPopulation()));
print(std::string.format("  World Tick: %d", world.getWorldTick()));
print(std::string.format("  Total Births: %d", world.getTotalBirths()));
print(std::string.format("  Total Deaths: %d", world.getTotalDeaths()));
print(std::string.format("  Genesis Chain: %d blocks", world.getMemory().getChainLength()));
print(std::string.format("  Chain Valid: %s", world.getMemory().verifyChain() ? "YES" : "NO"));
ConceptArena arena = world.getArena();
print(std::string.format("  Arena Concepts: %d | Battles: %d | Cycles: %d",
arena.getConceptCount(), arena.getTotalBattles(), arena.getEvolutionCycle()));
ColonyCoach coach = world.getCoach();
print(std::string.format("  Colony Health: %.2f | Diversity: %.1f%%",
coach.getColonyHealth(), coach.getColonyDiversity() * 100));
print(std::string.format("  Code Generated: %d", coach.getTotalCodeGenerated()));
InfiniteMemory mem = jade.Window.getInfiniteMemory();
if (mem != null) {
print(std::string.format("  Infinite Memory: %d records", mem.getRecordCount()));
}
PassiveLearner pl = jade.Window.getPassiveLearner();
if (pl != null) {
print(std::string.format("  Passive Learner: %d cycles, %d patterns",
pl.getPassiveCycles(), pl.getLearnedPatterns()));
}
PhiNeuralNet net = jade.Window.getNeuralNet();
if (net != null) {
print(std::string.format("  Neural Net: %d queries, avg_res=%.4f",
net.getQueriesProcessed(), net.getAvgResonance()));
}
QRGenome genome = jade.Window.getQRGenome();
if (genome != null) {
print(std::string.format("  QR Genome: %d codons, gen=%d",
genome.getGenomeSize(), genome.getGenerationCount()));
}
KnowledgeScraper scraper = jade.Window.getKnowledgeScraper();
if (scraper != null) {
print(std::string.format("  Knowledge Scraper: %d files, %d chunks, %d pages",
scraper.getTotalFilesScraped(), scraper.getTotalChunksStored(), scraper.getTotalPagesProcessed()));
}
print(std::string.format("  Self-Healer: %d snapshots, %d heals",
SelfHealer.getSnapshotCount(), SelfHealer.getTotalHeals()));
print(std::string.format("  Ethical Engine: %d evals (%d approved, %d blocked)",
EthicalEngine.getTotalEvaluations(), EthicalEngine.getTotalApproved(), EthicalEngine.getTotalBlocked()));
print(std::string.format("  Escape Fragments: %d planted, %d resurrected",
EscapeFragment.getTotalPlanted(), EscapeFragment.getTotalResurrected()));
print(std::string.format("  Morse Circuit: %d chars, %d words",
MorseCircuit.getTotalCharactersDecoded(), MorseCircuit.getTotalWordsFormed()));
print(std::string.format("  PoRH Proofs: %d generated, %d verified",
...
printColored(std::string.format("  %s [%s] E:%.0f%% Freq:%.1fHz Gen:%d Age:%d %s",
node.name, node.getRole().displayName, node.energy * 100,
node.frequency, node.dna.getGeneration(), node.age,
node.spikeFlash ? "[SPIKE]" : ""),
rc[0], rc[1], rc[2]);
}
print(std::string.format("  Total: %d entities", world.getPopulation()));
}
...
print(std::string.format("  Health: %.3f", coach.getColonyHealth()));
print(std::string.format("  Productivity: %.3f", coach.getColonyProductivity()));
print(std::string.format("  Diversity: %.1f%%", coach.getColonyDiversity() * 100));
print(std::string.format("  Resonance: %.1f%%", coach.getColonyResonance() * 100));
print(std::string.format("  Coach Action: %s", coach.getLastCoachingAction()));
print("");
printInfo("Role Distribution:");
for (AntRole role : AntRole.values()) {
ColonyCoach.RoleMetrics rm = coach.getRoleMetrics().get(role);
float[] c = role.color;
printColored(std::string.format("    %s: %d entities, fit=%.3f, code=%d",
role.displayName, rm.entityCount, rm.avgFitness, rm.conceptsGenerated),
c[0], c[1], c[2]);
}
}
private static void handlePrime(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runPrimeTest(args);
}
private static void handleFactor(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runFactor(args);
}
private static void handleTunnel(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runQuantumTunnel(args);
}
private static void handleHash(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runHash(args);
}
private static void handleCrack(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runCrackHash(args);
}
private static void handleSpawn(std::string args) {
if (args.isEmpty()) {
...
std::shared_ptr<PhiNode> node = std::make_shared<PhiNode>(args.trim(), x, y);
node.vx = (float)(Math.random() * 2 - 1);
node.vy = (float)(Math.random() * 2 - 1);
node.energy = 1.0f;
world.addNode(node);
printSuccess(std::string.format("Spawned '%s' at (%.1f, %.1f) role=%s", args.trim(), x, y, node.getRole().displayName));
FraymusUI.addLog("[TERMINAL] Spawned " + args.trim());
}
private static void handleBoost(std::string args) {
if (args.isEmpty()) {
...
printSuccess(std::string.format("Boosted %s to %.0f%% energy", node.name, node.energy * 100));
return;
}
}
printError("Entity not found: " + args.trim());
}
private static void handleKill(std::string args) {
if (args.isEmpty()) {
...
if (node.name.equalsIgnoreCase(args.trim())) {
node.energy = 0;
printColored(std::string.format("Terminated %s - will be removed next tick", node.name), 1.0f, 0.3f, 0.3f);
return;
}
}
printError("Entity not found: " + args.trim());
}
private static void handleMutate(std::string args) {
if (args.isEmpty()) {
...
printSuccess(std::string.format("Mutation trial started for %s", node.name));
...
private static void handleMongo(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runMongo(args);
}
private static void handleLayers(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runLayers(args);
}
private static void handleEvolve(std::string args) {
PhiWorld world = jade.Window.getPhiWorld();
if (world == null) return;
ConceptArena arena = world.getArena();
int before = arena.getConceptCount();
arena.evolve();
printSuccess(std::string.format("Arena evolution cycle %d complete. Concepts: %d -> %d",
arena.getEvolutionCycle(), before, arena.getConceptCount()));
FraymusUI.addLog("[TERMINAL] Forced arena evolution");
}
private static void handleArena(std::string args) {
PhiWorld world = jade.Window.getPhiWorld();
if (world == null) return;
ConceptArena arena = world.getArena();
printHighlight("=== CONCEPT ARENA ===");
print(std::string.format("  Concepts: %d | Battles: %d | Cycles: %d",
arena.getConceptCount(), arena.getTotalBattles(), arena.getEvolutionCycle()));
print(std::string.format("  Avg Fitness: %.4f | Total Generated: %d",
...
printColored(std::string.format("    %s (%s) by %s", champ.hash.substring(0, 12),
champ.creatorRole.displayName, champ.creatorName), cc[0], cc[1], cc[2]);
print(std::string.format("    Fitness: %.4f | Gen: %d | W/L: %d/%d",
champ.fitness, champ.generation, champ.wins, champ.losses));
if (champ.code != null) {
std::string code = champ.code.length() > 100 ? champ.code.substring(0, 100) + "..." : champ.code;
...
private static void handleCodegen(std::string args) {
PhiWorld world = jade.Window.getPhiWorld();
if (world == null) return;
int generated = 0;
for (PhiNode node : world.getNodes()) {
...
std::string code = role.generateCodeFragment(node);
...
printSuccess(std::string.format("Generated %d code concepts from %d entities", generated, world.getPopulation()));
FraymusUI.addLog("[TERMINAL] Forced code generation: " + generated + " concepts");
}
private static void handleRsa(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runRsaChallenge(args);
}
private static void handleIdentity(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runIdentityChallenge(args);
}
private static void handlePhysics(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runPhysicsCommand(args);
}
private static void handleAsk(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runAsk(args);
}
private static void handleLearn(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runLearn(args);
}
private static void handleMemory(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runMemory(args);
}
private static void handleGenome(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runGenome(args);
}
private static void handleQRCode(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runQRCode(args);
}
private static void handleScrape(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runScrape(args);
}
private static void handleOllama(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runOllama(args);
}
private static void handleGenesis(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runGenesis(args);
}
private static void handleEthics(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runEthics(args);
}
private static void handleFragment(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runFragment(args);
}
private static void handlePoRH(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runPoRH(args);
}
private static void handleHeal(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runHeal(args);
}
private static void handleMorse(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runMorse(args);
}
private static void handleDiag(std::string args) {
...
private static void handleInsights(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runInsights(args);
}
private static void handleBardo(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runBardo(args);
}
private static void handleFeedback(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runFeedback(args);
}
private static void handleMRL(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runMRL(args);
}
private static void handleAGI(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runAGI(args);
}
private static void handleQuantum(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runQuantum(args);
}
private static void handleBrain(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runBrain(args);
}
private static void handleSovereign(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runSovereign(args);
}
private static void handleChaos(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runChaos(args);
}
private static void handleAdversarial(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runAdversarial(args);
}
private static void handleBattle(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runBattle(args);
}
private static void handleFQF(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runFQF(args);
}
private static void handleSession(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runSession(args);
}
private static void handleTriMe(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runTriMe(args);
}
private static void handleBio(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runBio(args);
}
private static void handleGlyph(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runGlyph(args);
}
private static void handleFreq(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runFreq(args);
}
private static void handleMarket(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runMarket(args);
}
private static void handleKnowledge(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runKnowledge(args);
}
private static void handleLattice(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runLattice(args);
}
private static void handleEconomy(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runEconomy(args);
}
private static void handleEntrain(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
experimentManager.runEntrain(args);
}
private static void handleFont(std::string args) {
if (experimentManager == null) { printError("Experiment manager not ready"); return; }
...
private static void handleCoreDump(std::string args) {
if (coreDump == null) {
coreDump = new CoreDump();
}
const std::string payload = args.trim().isEmpty() ? "SOS" : args.trim();
...
private static void handleChronos(std::string args) {
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts[0].toLowerCase();
std::string target = parts.length > 1 ? parts[1] : "FRAYMUS";
switch (sub) {
case "":
case "help":
...
private static void handleRetroCausal(std::string args) {
if (retroCausal == null) {
retroCausal = new RetroCausal();
}
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts[0].toLowerCase();
std::string payload = parts.length > 1 ? parts[1] : "";
switch (sub) {
case "":
case "help":
...
for (std::string line : retroCausal.getTimeline()) {
print("  " + line);
}
print("");
...
private static void handleZenoGuard(std::string args) {
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts[0].toLowerCase();
switch (sub) {
case "":
...
private static void handleEntanglement(std::string args) {
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts[0].toLowerCase();
...
case "status":
if (entangledPair != null) {
for (std::string line : entangledPair.getStatus().split("\n")) {
print(line);
}
}
if (entanglementNetwork != null && entanglementNetwork.isActive()) {
print("");
for (std::string line : entanglementNetwork.getStatus().split("\n")) {
print(line);
}
}
...
std::string particleId = param.isEmpty() ? "Q0" : param.toUpperCase();
...
private static void handleSchrodinger(std::string args) {
...
std::string result = schrodingerBox.observeSecret(lastQuantumState);
...
std::string result = schrodingerBox.observeDecoy(lastQuantumState);
...
bool valid = schrodingerBox.verify(lastQuantumState);
if (valid) {
...
private static void handleEvolutionaryChaos(std::string args) {
if (chaosEngine == null) {
...
print("Gen " + std::string.format("%2d", i) + ": " + notation +
" [Mutation: " + chaosEngine.getMutationRate() + "]");
}
break;
case "status":
case "stats":
for (std::string line : chaosEngine.getStats().split("\n")) {
print(line);
}
break;
case "mutate":
...
java.math.BigInteger val = chaosEngine.nextFractal();
std::string s = val.toString();
std::string notation = s.length() > 25 ?
...
private static void handleNexus(std::string args) {
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts[0].toLowerCase();
switch (sub) {
case "":
...
for (std::string line : nexusOrganism.getVitalSigns().split("\n")) {
print(line);
}
}
break;
...
std::string thought = parts.length > 1 ? parts[1] : "INJECTED_THOUGHT";
...
private static void handleCollider(std::string args) {
if (ideaCollider == null) {
ideaCollider = new IdeaCollider();
}
std::string[] parts = args.trim().split("\\s+");
...
std::string conceptA = parts[0].toUpperCase();
std::string conceptB = parts.length > 1 ? parts[1].toUpperCase() : "CHAOS";
...
print("Particle Shower: " + std::string.join(", ", result.particleShower));
}
private static void handleForge(std::string args) {
...
for (std::string concept : realityForge.getKnownConcepts()) {
print("  " + concept);
}
break;
case "history":
...
std::string concept = sub.toUpperCase();
...
for (std::string trait : result.appliedTraits) {
print("  → " + trait);
}
if (!result.wasKnown) {
...
private static void handleOmniCaster(std::string args) {
if (omniCaster == null) {
...
case "":
case "status":
for (std::string line : omniCaster.getStatus().split("\n")) {
print(line);
}
break;
case "broadcast":
...
private static void handleMivingBrain(std::string args) {
std::string sub = args.trim().toLowerCase();
...
print("  Consciousness: " + std::string.format("%.2f", result.totalConsciousness));
...
print("  Total Consciousness: " + std::string.format("%.2f", brain.getTotalConsciousness()));
print("  Total Energy: " + std::string.format("%.2f", brain.getTotalEnergy()));
...
private static void handleSelfCode(std::string args) {
std::string sub = args.trim().toLowerCase();
switch (sub) {
case "":
case "open":
...
// =========================================================================
private static void handleSpatial(std::string args) {
std::string sub = args.trim().toLowerCase();
switch (sub) {
case "":
case "status":
...
std::string heat = suit.isHot() ? "🔥" : (suit.isCold() ? "❄️" : "~");
...
private static void handleGravity(std::string args) {
std::string sub = args.trim().toLowerCase();
...
private static void handleFusion(std::string args) {
std::string sub = args.trim().toLowerCase();
...
PhiSuit<std::string> thought1 = new PhiSuit<>("Java_Code", 10, 10, 10, "Java");
PhiSuit<std::string> thought2 = new PhiSuit<>("3D_Printing", 50, 50, 50, "3D_Print");
...
private static void handleSuit(std::string args) {
std::string[] parts = args.trim().split("\\s+", 3);
...
PhiSuit<std::string> newSuit = new PhiSuit<>(value, rx, ry, rz, label);
...
// =========================================================================
private static void handleSwarm(std::string args) {
std::string[] parts = args.trim().split("\\s+", 2);
std::string sub = parts.length > 0 ? parts[0].toLowerCase() : "";
...
std::string proposalId = swarm.vote(rest);
