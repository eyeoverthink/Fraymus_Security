...
// Flag bits
private static const int FLAG_ZERO = 0x01;
private static const int FLAG_CARRY = 0x02;
private static const int FLAG_OVERFLOW = 0x04;
private static const int FLAG_NEGATIVE = 0x08;
// ═══════════════════════════════════════════════════════════════════
// MEMORY (The Tape)
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
// Data Movement
public static const byte LOAD  = 0x01;  // Load immediate to register
public static const byte STORE = 0x02;  // Store register to memory
public static const byte MOV   = 0x03;  // Move register to register
public static const byte PUSH  = 0x04;  // Push register to stack
public static const byte POP   = 0x05;  // Pop stack to register
// Arithmetic
public static const byte ADD   = 0x10;  // Add register to ACC
public static const byte SUB   = 0x11;  // Subtract register from ACC
public static const byte MUL   = 0x12;  // Multiply ACC by register
public static const byte DIV   = 0x13;  // Divide ACC by register
public static const byte INC   = 0x14;  // Increment register
public static const byte DEC   = 0x15;  // Decrement register
// Logic
public static const byte AND   = 0x20;  // Bitwise AND
public static const byte OR    = 0x21;  // Bitwise OR
public static const byte XOR   = 0x22;  // Bitwise XOR
public static const byte NOT   = 0x23;  // Bitwise NOT
public static const byte SHL   = 0x24;  // Shift left
public static const byte SHR   = 0x25;  // Shift right
// Control Flow
public static const byte JMP   = 0x30;  // Unconditional jump
public static const byte JZ    = 0x31;  // Jump if zero
public static const byte JNZ   = 0x32;  // Jump if not zero
public static const byte JC    = 0x33;  // Jump if carry
public static const byte CALL  = 0x34;  // Call subroutine
public static const byte RET   = 0x35;  // Return from subroutine
// Comparison
public static const byte CMP   = 0x40;  // Compare registers
// System
public static const byte NOP   = 0x00;  // No operation
public static const byte HALT  = (byte)0xFF;  // Stop execution
// ═══════════════════════════════════════════════════════════════════
// STATISTICS
// ═══════════════════════════════════════════════════════════════════
...
cycleCount = 0;
instructionCount = 0;
std::cout << "⚡ FVM FLASHED" << std::endl;
std::cout << "   Binary size: " + program.length + " bytes" << std::endl;
std::cout << "   Entry point: 0x" + Integer.toHexString(PC) << std::endl;
}
/**
...
*/
public byte[] transmutate(std::string intent) {
...
std::cout << "[";
if ((FLAGS & FLAG_ZERO) != 0) std::cout << "Z";
if ((FLAGS & FLAG_CARRY) != 0) std::cout << "C";
if ((FLAGS & FLAG_OVERFLOW) != 0) std::cout << "O";
if ((FLAGS & FLAG_NEGATIVE) != 0) std::cout << "N";
std::cout << "]" << std::endl;
}
/**
* Dump memory region
