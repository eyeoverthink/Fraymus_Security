...
public class Window {
    ...
    private void initPhiWorld() {
        ...
        phiWorld.setOrganismMemory(infiniteMemory);
        ...
        int loadedFragments = EscapeFragment.loadFromMemory(infiniteMemory, 250);
        List<PhiNode> resurrectedNodes = EscapeFragment.resurrectBest(5, 0.0f, 0.0f);
        if (!resurrectedNodes.isEmpty()) {
            for (PhiNode node : resurrectedNodes) {
                node.energy = Math.max(0.65f, node.energy);
                phiWorld.addNode(node);
                memory.record("BOOT_RESURRECTION", String.format(
                        "%s|gen=%d|energy=%.2f|xor=%.3f",
                        node.name,
            ...
            initialOrganismCount = resurrectedNodes.size();
            FraymusUI.addLog(String.format("[BOOT] Resurrected %d organisms from %d genome records",
                    resurrectedNodes.size(), loadedFragments));
        } else {
            ...
            initialOrganismCount = 5;
            FraymusUI.addLog("[BOOT] No genome records found - seeded genesis organisms");
        }
        ...
        FraymusUI.init(experimentManager);
        
        // Initialize health monitor with adaptive recovery
        fraymus.HealthMonitor.setExperimentManager(experimentManager);
        fraymus.HealthMonitor.getInstance();

        passiveLearner.start();
        
        // NERVE CENTER: Connect Java Brain to HTML Eyes
        fraymus.NerveCenter.setPhiWorld(phiWorld);
        fraymus.NerveCenter.setExperimentManager(experimentManager);
        fraymus.NerveCenter.getInstance();
        FraymusUI.addLog("Nerve Center online - ws://localhost:8887");
        FraymusUI.addLog("HTTP API online - http://localhost:8888/api/engine/*");
        FraymusUI.addLog("Open FraymusArena.html to see living visualization");

        FraymusUI.addLog("World initialized with " + initialOrganismCount + " PhiNode entities");
