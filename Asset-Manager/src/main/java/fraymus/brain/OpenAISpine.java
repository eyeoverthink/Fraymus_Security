...
public class OpenAISpine {
    ...
    /**
     * Default to gpt-4-turbo model, load API key from environment
     */
    public OpenAISpine() {
        this("gpt-4-turbo");
    }
    ...
    /**
     * Use specific model, load API key from environment
     */
    public OpenAISpine(String model) {
        this(model, ConfigManager.getInstance().getOpenAIKey());
    }

    /**
     * Use specific model with explicit API key (for legacy compatibility)
     * Note: Prefer using environment variable OPENAI_API_KEY instead
     */
