...
public class OllamaSpine {
    ...
    public OllamaSpine(String modelName, String ollamaHost) {
        this.modelName = modelName;
        this.baseUrl = ollamaHost + "/api";
        this.generateUrl = ollamaHost + "/api/generate";
        this.http = HttpClient.newBuilder()
            .connectTimeout(Duration.ofSeconds(3))
            .build();
        System.out.println(">>> [OLLAMA SPINE] Initialized with model: " + modelName + " @ " + ollamaHost);
    }
    ...
    public String think(String prompt, String context) {
        ...
        try {
            ...
            conn.setReadTimeout(180000); // 180 seconds (3 minutes) for Gemma 4 generation
            ...
            // Timing tracked internally, not printed to user
    ...
    private String buildJsonPayload(String prompt, String context) {
        // Use proper JSON structure with system field for context
        String escapedPrompt = prompt.replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n");
        
        if (context != null && !context.trim().isEmpty()) {
            String escapedContext = context.replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n");
            // Use system field for context (more reliable than embedding in prompt)
            return String.format(
                "{\"model\": \"%s\", \"system\": \"%s\", \"prompt\": \"%s\", \"stream\": false}",
                modelName, escapedContext, escapedPrompt
            );
        } else {
            return String.format(
