...
public class LogicBrain {
    ...
    public int getActiveXorCount() {
        int count = 0;
        for (LogicGate gate : gates) {
            if (gate.isXor() && gate.isActive()) count++;
        }
        return count;
    }

    public int[] getGateTypeCounts() {
        int[] counts = new int[4];
        for (LogicGate gate : gates) {
            if (gate.type >= 0 && gate.type < counts.length) {
                counts[gate.type]++;
            }
        }
        return counts;
    ...
    public double getLogicDiversity() {
        if (gates.isEmpty()) return 0.0;

        int[] counts = getGateTypeCounts();
        double entropy = 0.0;
        for (int count : counts) {
            if (count == 0) continue;
            double p = count / (double) gates.size();
            entropy -= p * (Math.log(p) / Math.log(2.0));
        }
        return entropy / 2.0; // Four gate types max out at 2 bits of entropy.
    }

    public double getXorActivationRatio() {
        int xorGates = getXorGateCount();
        if (xorGates == 0) return 0.0;
        return getActiveXorCount() / (double) xorGates;
    }

    public double getXorResonance() {
        if (gates.isEmpty()) return 0.0;
        double xorDensity = getXorGateCount() / (double) gates.size();
        double activation = getXorActivationRatio();
        double diversity = getLogicDiversity();
        return Math.min(1.0, xorDensity * 0.45 + activation * 0.35 + diversity * 0.20);
    }
