...
public class AEON_Singularity {
    ...
    static class GenesisDB {
        ...
        static final long EXPECTED_BYTES = (long) HDC.DIMS * HDC.DIMS * Float.BYTES;
        ...
        static void save(Cortex cortex) {
            try (FileChannel fc = new FileOutputStream(FILE_NAME).getChannel()) {
                ...
                System.err.println("[DB] Save failed: " + describe(e));
            }
        }
        ...
        static void load(Cortex cortex) {
            ...
            if (f.length() != EXPECTED_BYTES) {
                System.err.println("[DB] Load refused: " + FILE_NAME + " is " + f.length()
                    + " bytes, expected " + EXPECTED_BYTES + ". Initializing safe vacuum state.");
                return;
            }
            try (FileChannel fc = new FileInputStream(f).getChannel()) {
                ...
                System.err.println("[DB] Load failed: " + describe(e));
            }
        }
        ...
        static void writeFully(FileChannel fc, ByteBuffer buf) throws IOException {
            while (buf.hasRemaining()) fc.write(buf);
        }

        static void readFully(FileChannel fc, ByteBuffer buf) throws IOException {
            while (buf.hasRemaining()) {
                if (fc.read(buf) < 0) throw new EOFException("short Genesis DB read");
            }
        }
