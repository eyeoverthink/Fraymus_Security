    ...
    static class SwarmDaemon extends Thread {
        ...
        public SwarmDaemon() {
            ...
            for (int p = 42000; p <= 42020; p++) {
                try { socket = new DatagramSocket(p); socket.setBroadcast(true); this.port = p; return; } catch (Exception e) {}
            }
        }
        ...
        private void sendUDP(String msg) {
            ...
            for (int p = 42000; p <= 42020; p++) {
                if (p != this.port) {
                    try { socket.send(new DatagramPacket(data, data.length, InetAddress.getByName("255.255.255.255"), p)); } 
                    catch (Exception e) {}
                }
            }
        }
