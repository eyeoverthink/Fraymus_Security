 ...
 */
public class HybridModelManager {
    ...
    public HybridModelManager(ModelManager modelManager) {
        ...
        if (modelManager.hasGemma4()) {
            ...
            System.out.println("⚠️ [HYBRID MANAGER] Gemma 4 not available - running in internal-only mode");
        }

        // Initialize Open Claw (performance optimization)
        try {
            this.openClaw = new OpenClawSpine(new fraymus.compute.FrayCLContext());
            System.out.println("✓ [HYBRID MANAGER] Open Claw initialized");
        } catch (Exception e) {
            System.out.println("⚠️ [HYBRID MANAGER] Open Claw initialization failed: " + e.getMessage());
            this.openClaw = null;
        }
        ...
        try {
            ...
            System.out.println("✓ [HYBRID MANAGER] Neo Claw initialized");
        } catch (Exception e) {
            System.out.println("⚠️ [HYBRID MANAGER] Neo Claw initialization failed: " + e.getMessage());
            this.neoClaw = null;
        }
        ...
        try {
            ...
            System.out.println("⚠️ [HYBRID MANAGER] Claude requires API key - not initialized");
        } catch (Exception e) {
            this.claude = null;
        }
        ...
        try {
            ...
            System.out.println("⚠️ [HYBRID MANAGER] Claude Code not initialized - use setClaudeCode()");
        } catch (Exception e) {
        ...
        try {
            ...
            System.out.println("⚠️ [HYBRID MANAGER] SpeechBrain not initialized - use setSpeechBrain()");
        } catch (Exception e) {
    ...
    public void setClaudeCode(ClaudeCodeIntegration claudeCode) {
        ...
        if (claudeCode != null && claudeCode.isAvailable()) {
            System.out.println("✓ [HYBRID MANAGER] Claude Code initialized for code generation tasks");
        } else {
            System.out.println("⚠️ [HYBRID MANAGER] Claude Code not available");
        }
    }
    ...
    public void setSpeechBrain(SpeechBrainIntegration speechBrain) {
        ...
        if (speechBrain != null && speechBrain.isAvailable()) {
            System.out.println("✓ [HYBRID MANAGER] SpeechBrain initialized for speech processing tasks");
        } else {
            System.out.println("⚠️ [HYBRID MANAGER] SpeechBrain not available");
        }
    }
    ...
    public void setFlyGNN(FlyGNN flyGNN) {
        this.flyGNN = flyGNN;
        if (flyGNN != null) {
            System.out.println("✓ [HYBRID MANAGER] FlyGNN initialized for connectome-based pattern recognition");
        }
    }
     ...
     */
    public TaskType analyzeTask(String prompt) {
    ...
    public String generate(String prompt) {
        ...
        boolean containsMath = CalculatorBackend.containsMath(prompt);
        
        if (!hybridMode) {
            // Internal model only
            try {
                String response = internalModel.generateResponse(prompt);
     ...
     */
    private String synthesizeResponse(String prompt) {
        String internalResponse = internalModel.generateResponse(prompt);
        String externalResponse = externalModel.generateResponse(prompt);
     ...
     */
    public String recursivelyEnhance(String output, int iterations) {
        String current = output;
        
        for (int i = 0; i < iterations; i++) {
            String critique = externalModel.generateResponse(
                "Critique this output and suggest specific improvements: " + current
            );
            
            String improved = externalModel.generateResponse(
     ...
     */
    public String metaAnalyze(String internalOutput) {
    ...
    public String getUsageStatistics() {
        ...
        stats.append("   Complexity Threshold: ").append(String.format("%.2f", complexityThreshold)).append("\n");
        stats.append("\n");
        ...
        stats.append("   Internal Model Errors: ").append(internalModelErrors)
            .append(" (").append(String.format("%.2f%%", internalErrorRate)).append(")\n");
        stats.append("   External Model Errors: ").append(externalModelErrors)
            .append(" (").append(String.format("%.2f%%", externalErrorRate)).append(")\n");
        stats.append("   Open Claw Errors: ").append(openClawErrors)
            .append(" (").append(String.format("%.2f%%", openClawErrorRate)).append(")\n");
        stats.append("   Neo Claw Errors: ").append(neoClawErrors)
            .append(" (").append(String.format("%.2f%%", neoClawErrorRate)).append(")\n");
        stats.append("   Claude Errors: ").append(claudeErrors)
            .append(" (").append(String.format("%.2f%%", claudeErrorRate)).append(")\n");
        stats.append("   Claude Code Errors: ").append(claudeCodeErrors)
            .append(" (").append(String.format("%.2f%%", claudeCodeErrorRate)).append(")\n");
        stats.append("   SpeechBrain Errors: ").append(speechBrainErrors)
            .append(" (").append(String.format("%.2f%%", speechBrainErrorRate)).append(")\n");
        stats.append("   FlyGNN Errors: ").append(flyGNNErrors)
            .append(" (").append(String.format("%.2f%%", flyGNNErrorRate)).append(")\n");
        stats.append("\n");
        ...
        for (TaskType taskType : TaskType.values()) {
            ...
            if (count > 0) {
                String routing = taskTypeRouting.getOrDefault(taskType, "UNKNOWN");
        ...
        stats.append("   Internal: ").append(String.format("%.1f%%", internalPercent)).append("\n");
        stats.append("   External: ").append(String.format("%.1f%%", externalPercent)).append("\n");
        stats.append("   Open Claw: ").append(String.format("%.1f%%", openClawPercent)).append("\n");
        stats.append("   Neo Claw: ").append(String.format("%.1f%%", neoClawPercent)).append("\n");
        stats.append("   Claude: ").append(String.format("%.1f%%", claudePercent)).append("\n");
        stats.append("   Claude Code: ").append(String.format("%.1f%%", claudeCodePercent)).append("\n");
        stats.append("   SpeechBrain: ").append(String.format("%.1f%%", speechBrainPercent)).append("\n");
        stats.append("   FlyGNN: ").append(String.format("%.1f%%", flyGNNPercent)).append("\n");
        stats.append("   Hybrid: ").append(String.format("%.1f%%", hybridPercent)).append("\n");
        stats.append("\n");
     ...
     */
    private String applyCalculatorBackend(String prompt, String response) {
        try {
            // Solve math in both prompt and response
            java.util.Map<String, Double> promptMath = CalculatorBackend.solveMathInText(prompt);
            java.util.Map<String, Double> responseMath = CalculatorBackend.solveMathInText(response);

            // Combine results
            java.util.Map<String, Double> allMath = new java.util.LinkedHashMap<>();
            allMath.putAll(promptMath);
            allMath.putAll(responseMath);

            if (!allMath.isEmpty()) {
                System.out.println("🧮 [CALCULATOR BACKEND] Correcting " + allMath.size() + " mathematical operations");
                
                // Inject results into response
                String correctedResponse = CalculatorBackend.injectResults(response, allMath);
    ...
    private String extractFilePath(String prompt) {
        ...
        String[] patterns = { "file:", "path:", "\"", "'" };
        for (String pattern : patterns) {
            int idx = prompt.indexOf(pattern);
            if (idx != -1) {
                String after = prompt.substring(idx + pattern.length()).trim();
    ...
    private String extractText(String prompt) {
        ...
        if (startIdx != -1) {
            ...
            String after = prompt.substring(startIdx + 5).trim();
     ...
     */
    private String extractOutputPath(String prompt) {
        ...
        if (startIdx != -1) {
            String after = prompt.substring(startIdx + 3).trim();
            // Remove quotes
            if (after.startsWith("\"")) after = after.substring(1).trim();
            if (after.startsWith("'")) after = after.substring(1).trim();
            if (after.endsWith("\"")) after = after.substring(0, after.length() - 1);
