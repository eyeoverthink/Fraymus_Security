...
public class ExperimentManager {
    ...
    private String currentModel = "fraymus-genesis:latest";
    ...
    public OllamaIntegration getOllama() {
        return ollama;
    }
    ...
    public PhiWorld getWorld() { return world; }
    ...
    public void runOllama(String args) {
        ...
        switch (sub) {
            ...
            case "local":
                useCloudOllama = false;
                ollama = new OllamaIntegration(false);
                currentModel = "fraymus-genesis:latest";
    ...
    private void handleOllamaAsk(String question) {
        ...
        context.append("\nCAPABILITIES:\n");
        context.append("- You can generate images using Dreamscape by including [DREAMSCAPE: <description>] in your response\n");
        context.append("- Example: 'I will create [DREAMSCAPE: a cosmic nebula with purple stars] for you'\n");
        ...
        if (response.startsWith("ERROR")) {
            ...
            // Check for Dreamscape image generation request
            if (response.contains("[DREAMSCAPE:")) {
                handleDreamscapeGeneration(response);
            }
    ...
    private void handleDreamscapeGeneration(String response) {
        CommandTerminal.printHighlight("=== DREAMSCAPE GENERATION TRIGGERED ===");
        
        // Extract the Dreamscape description
        int startIdx = response.indexOf("[DREAMSCAPE:");
        if (startIdx == -1) return;
        
        int endIdx = response.indexOf("]", startIdx);
        if (endIdx == -1) {
            CommandTerminal.printError("Invalid Dreamscape format - missing closing bracket");
            return;
        }
        
        String concept = response.substring(startIdx + 12, endIdx).trim();
        CommandTerminal.print("  Concept: " + concept);
        
        // Extract quantum parameters from concept (simplified)
        double entropy = 0.5;
        double phi = 1.618033988749895;
        double consciousness = 0.5;
        
        String conceptLower = concept.toLowerCase();
        if (conceptLower.contains("chaos") || conceptLower.contains("random") || conceptLower.contains("nebula")) {
            entropy = 0.8;
        } else if (conceptLower.contains("order") || conceptLower.contains("crystal") || conceptLower.contains("geometric")) {
            entropy = 0.2;
        }
        
        if (conceptLower.contains("spirit") || conceptLower.contains("soul") || conceptLower.contains("conscious")) {
            consciousness = 0.8;
        } else if (conceptLower.contains("stone") || conceptLower.contains("statue") || conceptLower.contains("inanimate")) {
            consciousness = 0.2;
        }
        
        CommandTerminal.print("  Entropy: " + String.format("%.3f", entropy));
        CommandTerminal.print("  Phi: " + String.format("%.6f", phi));
        CommandTerminal.print("  Consciousness: " + String.format("%.3f", consciousness));
        CommandTerminal.print("  Generating image...");
        
        // Call DreamscapeVisualizer
        fraymus.core.DreamscapeVisualizer.dream(concept, entropy, phi, consciousness);
    }
    ...
    private void handleOllamaChat(String message) {
        ...
        sysPrompt.append("\n\nCAPABILITIES:\n");
        sysPrompt.append("- You can generate images using Dreamscape by including [DREAMSCAPE: <description>] in your response\n");
        sysPrompt.append("- Example: 'I will create [DREAMSCAPE: a cosmic nebula with purple stars] for you'\n");
    ...
    private void handleOllamaStream(String prompt) {
        ...
        System.out.print("φ> ");
        String fullResponse = bridge.speakStreaming(prompt);
        System.out.println();
        
        // Store in memory
        if (fullResponse != null && !fullResponse.startsWith("⚠️")) {
            infiniteMemory.store("OLLAMA_STREAM", 
                String.format("Q: %s | A: %s", prompt, fullResponse.substring(0, Math.min(200, fullResponse.length()))),
                0.85, "STREAM");
