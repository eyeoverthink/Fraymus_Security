...
public class FraymusUI {
    ...
    private static boolean modelCheckPrinted = false;
    private static int modelCheckDelay = 60;
    ...
    public static void render(PhiWorld world) {
        ...
        if (!modelCheckPrinted && modelCheckDelay > 0) {
            modelCheckDelay--;
            if (modelCheckDelay == 0) {
                addLog("Checking Ollama model availability...");
                modelCheckPrinted = true;
            }
        }
        ...
        renderOllamaModelCheck();
        renderHealthMonitorPanel();
    ...
    private static void renderWorldStatus(PhiWorld world) {
        ...
        if (ImGui.begin("World Status")) {
            ...
            int activeXorGates = 0;
            double totalXorResonance = 0.0;
            for (PhiNode node : world.getNodes()) {
                ...
                activeXorGates += node.brain.getActiveXorCount();
                totalXorResonance += node.brain.getXorResonance();
            }
            ...
            if (world.getPopulation() > 0) {
                ...
                ImGui.text(String.format("XOR Resonance: %.3f", totalXorResonance / world.getPopulation()));
            }
            ImGui.text(String.format("Active XOR Gates: %d", activeXorGates));
            ...
            ImGui.textColored(0.6f, 0.8f, 1.0f, 1.0f, "Arena: cyan=entangled magenta=XOR amber=trial");
            ImGui.separator();
        ...
        if (ImGui.begin("Entity Inspector")) {
            ...
            for (int i = 0; i < nodes.size(); i++) {
                ...
                if (ImGui.collapsingHeader(label)) {
                    ...
                    ImGui.text(String.format("XOR: res %.3f | active %d/%d",
                            node.brain.getXorResonance(), node.brain.getActiveXorCount(), node.brain.getXorGateCount()));
                    CodeConcept lastConcept = node.getLastGeneratedConcept();
                    if (lastConcept != null) {
                        ImGui.textColored(0.4f, 1.0f, 0.8f, 1.0f,
                                String.format("Last Concept: %s fit=%.3f", lastConcept.hash.substring(0, 8), lastConcept.fitness));
                    }
    ...
    private static void renderBrainInspector(PhiWorld world) {
        ...
        ImGui.setNextWindowSize(340, 230, ImGuiCond.FirstUseEver);
        ...
        if (ImGui.begin("Brain Inspector")) {
            ...
            if (selectedNodeIndex >= 0 && selectedNodeIndex < nodes.size()) {
                ...
                float xorResonance = (float) brain.getXorResonance();
                int[] gateMix = brain.getGateTypeCounts();
                ImGui.textColored(1.0f, 0.3f, 0.9f, 1.0f, "XOR Logic Engine");
                ImGui.text(String.format("XOR Gates: %d/%d | Active: %d",
                        brain.getXorGateCount(), brain.getGateCount(), brain.getActiveXorCount()));
                ImGui.progressBar(xorResonance, 150, 12, String.format("res %.3f", xorResonance));
                ImGui.text(String.format("Diversity: %.3f | XOR Act: %.3f",
                        brain.getLogicDiversity(), brain.getXorActivationRatio()));
                ImGui.text(String.format("Mix A:%d O:%d X:%d N:%d",
                        gateMix[LogicGate.AND], gateMix[LogicGate.OR], gateMix[LogicGate.XOR], gateMix[LogicGate.NAND]));
                ImGui.separator();
                ...
                for (int i = 0; i < brain.gates.size() && i < LogicBrain.OUTPUT_NAMES.length; i++) {
                    ...
                    String trace = gate.isXor()
                            ? String.format(" [%d^%d]", gate.getLastInputA(), gate.getLastInputB())
                            : "";

                    if (active) {
                        ImGui.textColored(0.0f, 1.0f, 0.3f, 1.0f,
                                String.format("  [%d] %s(in%d,in%d)%s -> %s = ON",
                                        i, gate.getTypeName(), gate.in1, gate.in2, trace, LogicBrain.OUTPUT_NAMES[i]));
                    } else {
                        ImGui.textColored(0.5f, 0.5f, 0.5f, 1.0f,
                                String.format("  [%d] %s(in%d,in%d)%s -> %s = off",
                                        i, gate.getTypeName(), gate.in1, gate.in2, trace, LogicBrain.OUTPUT_NAMES[i]));
                    }
                }
    ...
    private static void renderConceptArenaPanel(PhiWorld world) {
        ...
        if (ImGui.begin("Concept Arena")) {
            ...
            if (champ != null) {
                ...
                ImGui.text(String.format("  Q: corr %.3f | eff %.3f | eleg %.3f | cx %.3f",
                    champ.correctness, champ.efficiency, champ.elegance, champ.complexity));
                ImGui.progressBar((float) champ.fitness, 150, 12, String.format("phi fitness %.3f", champ.fitness));
    ...
    private static void renderOllamaModelCheck() {
        ImGui.setNextWindowPos(10, 10, ImGuiCond.FirstUseEver);
        ImGui.setNextWindowSize(260, 120, ImGuiCond.FirstUseEver);

        if (ImGui.begin("Ollama Model Status")) {
            ImGui.textColored(0.5f, 0.8f, 1.0f, 1.0f, "ENGINE V2 - LLM BACKEND");
            ImGui.separator();

            if (experimentManager != null) {
                String model = experimentManager.getCurrentModel();
                ImGui.text("Default Model:");
                ImGui.sameLine();
                ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, model);
                
                ImGui.text("Endpoint:");
                ImGui.sameLine();
                ImGui.textColored(0.5f, 0.8f, 1.0f, 1.0f, "localhost:11434");
                
                ImGui.separator();
                ImGui.text("Type 'ollama status' in terminal");
                ImGui.text("to check connection.");
            } else {
                ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, "ExperimentManager not ready");
            }
        }
        ImGui.end();
    }

    private static void renderHealthMonitorPanel() {
        ImGui.setNextWindowPos(10, 140, ImGuiCond.FirstUseEver);
        ImGui.setNextWindowSize(280, 200, ImGuiCond.FirstUseEver);

        if (ImGui.begin("Health Monitor")) {
            ImGui.textColored(0.5f, 1.0f, 0.5f, 1.0f, "SYSTEM HEALTH - ADAPTIVE");
            ImGui.separator();

            HealthMonitor monitor = HealthMonitor.getInstance();
            if (monitor != null) {
                // Port status
                HealthMonitor.HealthStatus wsStatus = monitor.getStatus("websocket_port");
                HealthMonitor.HealthStatus httpStatus = monitor.getStatus("http_port");
                HealthMonitor.HealthStatus ollamaStatus = monitor.getStatus("ollama_port");

                ImGui.text("WebSocket (8887):");
                ImGui.sameLine();
                if (wsStatus != null && wsStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, wsStatus != null ? wsStatus.status : "UNKNOWN");
                }

                ImGui.text("HTTP (8888):");
                ImGui.sameLine();
                if (httpStatus != null && httpStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, httpStatus != null ? httpStatus.status : "UNKNOWN");
                }

                ImGui.text("Ollama (11434):");
                ImGui.sameLine();
                if (ollamaStatus != null && ollamaStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, ollamaStatus != null ? ollamaStatus.status : "UNKNOWN");
                }

                ImGui.separator();

                // Model status
                HealthMonitor.HealthStatus modelStatus = monitor.getStatus("default_model");
                ImGui.text("Default Model:");
                ImGui.sameLine();
                if (modelStatus != null && modelStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, modelStatus != null ? modelStatus.status : "UNKNOWN");
                }

                // Memory status
                HealthMonitor.HealthStatus memoryStatus = monitor.getStatus("memory_files");
                ImGui.text("Memory Files:");
                ImGui.sameLine();
                if (memoryStatus != null && memoryStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, memoryStatus != null ? memoryStatus.status : "UNKNOWN");
                }

                // Bridge status
                HealthMonitor.HealthStatus bridgeStatus = monitor.getStatus("websocket_bridge");
                ImGui.text("WebSocket Bridge:");
                ImGui.sameLine();
                if (bridgeStatus != null && bridgeStatus.healthy) {
                    ImGui.textColored(0.3f, 1.0f, 0.3f, 1.0f, "OK");
                } else {
                    ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, bridgeStatus != null ? bridgeStatus.status : "UNKNOWN");
                }

                ImGui.separator();
                ImGui.text("Adaptive Mode: " + (monitor.isSystemHealthy() ? "ACTIVE" : "RECOVERING"));
            } else {
                ImGui.textColored(1.0f, 0.3f, 0.3f, 1.0f, "HealthMonitor not initialized");
            }
        }
        ImGui.end();
    }
